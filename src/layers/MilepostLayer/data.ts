const data = {
  layerDefinition: {
    defaultVisibility: true,
    featureReduction: {
      type: "cluster",
      clusterRadius: 37.5,
      clusterMinSize: 16.125,
      clusterMaxSize: 37.5,
      visibilityInfo: {
        maxScale: 1176.278877,
      },
      disablePopup: false,
      popupInfo: {
        popupElements: [
          {
            type: "text",
            text: "This cluster represents <b>{cluster_count}</b> features.",
          },
          {
            type: "text",
            text: "The predominant value of <b>Direction</b> within this cluster is <b>{expression/cluster_type_Direction}</b>.",
          },
          {
            type: "text",
            text: "The average value of <b>Azimuth</b> within this cluster is <b>{cluster_avg_Azimuth}</b>.",
          },
        ],
        description: "This cluster represents <b>{cluster_count}</b> features.",
        expressionInfos: [
          {
            name: "cluster_type_Direction",
            title: "Predominant Direction",
            expression:
              '\n  var uvInfos = [{\n        "value": "b",\n        "label": "Both"\n      }, {\n        "value": "i",\n        "label": "Increase"\n      }, {\n        "value": "d",\n        "label": "Decrease"\n      }, {\n        "value": "u",\n        "label": "Unknown"\n      }];\n  var predominantType = Text($feature["cluster_type_Direction"]);\n  var label = "None";\n\n  for (var i = 0; i < Count(uvInfos); i++) {\n    if (uvInfos[i].value == predominantType) {\n      label = uvInfos[i].label;\n      break;\n    }\n  }\n\n  return label;\n  ',
            returnType: "string",
          },
        ],
        fieldInfos: [
          {
            fieldName: "cluster_count",
            format: {
              digitSeparator: true,
              places: 0,
            },
            isEditable: true,
            label: "Number of features",
            visible: true,
          },
          {
            fieldName: "expression/cluster_type_Direction",
            isEditable: true,
            visible: true,
          },
          {
            fieldName: "cluster_avg_Azimuth",
            format: {
              digitSeparator: true,
              places: 2,
            },
            isEditable: true,
            label: "Average Azimuth",
            visible: true,
          },
        ],
        title: "Cluster summary",
      },
      drawingInfo: {
        renderer: {
          authoringInfo: {
            isAutoGenerated: true,
          },
          type: "uniqueValue",
          visualVariables: [
            {
              type: "rotationInfo",
              field: "cluster_avg_Azimuth",
              rotationType: "geographic",
            },
          ],
          field1: "cluster_type_Direction",
          drawInClassOrder: true,
          uniqueValueGroups: [
            {
              classes: [
                {
                  label: "Both",
                  symbol: {
                    type: "CIMSymbolReference",
                    symbol: {
                      type: "CIMPointSymbol",
                      symbolLayers: [
                        {
                          type: "CIMVectorMarker",
                          enable: true,
                          colorLocked: true,
                          anchorPointUnits: "Relative",
                          dominantSizeAxis3D: "Y",
                          size: 9.69,
                          billboardMode3D: "FaceNearPlane",
                          frame: {
                            xmin: 0,
                            ymin: 0,
                            xmax: 31.2,
                            ymax: 31.2,
                          },
                          markerGraphics: [
                            {
                              type: "CIMMarkerGraphic",
                              geometry: {
                                rings: [
                                  [
                                    [15.6, 0.5],
                                    [17, 0.6],
                                    [17.6, 0.6],
                                    [18.3, 0.8],
                                    [19.1, 0.9],
                                    [19.8, 1.1],
                                    [20.5, 1.3],
                                    [21.2, 1.6],
                                    [21.9, 1.9],
                                    [22.5, 2.2],
                                    [23.2, 2.6],
                                    [23.9, 3],
                                    [24.5, 3.4],
                                    [25.1, 3.9],
                                    [25.7, 4.4],
                                    [26.3, 4.9],
                                    [26.8, 5.5],
                                    [27.3, 6.1],
                                    [27.8, 6.7],
                                    [28.2, 7.3],
                                    [28.6, 8],
                                    [29, 8.7],
                                    [29.3, 9.3],
                                    [29.6, 10],
                                    [29.9, 10.7],
                                    [30.1, 11.4],
                                    [30.3, 12.1],
                                    [30.4, 12.9],
                                    [30.6, 13.6],
                                    [30.6, 14.2],
                                    [30.7, 14.9],
                                    [30.7, 15.6],
                                    [30.5, 18],
                                    [30, 20.3],
                                    [29.1, 22.5],
                                    [27.8, 24.5],
                                    [26.3, 26.3],
                                    [24.5, 27.8],
                                    [22.5, 29.1],
                                    [20.3, 30],
                                    [18, 30.5],
                                    [15.6, 30.7],
                                    [12.7, 30.4],
                                    [10, 29.6],
                                    [7.4, 28.3],
                                    [5.2, 26.5],
                                    [3.3, 24.3],
                                    [1.8, 21.9],
                                    [0.9, 19.1],
                                    [0.5, 16.3],
                                    [0.5, 15.6],
                                    [0.7, 13.2],
                                    [1.2, 10.9],
                                    [2.1, 8.7],
                                    [3.4, 6.7],
                                    [4.9, 4.9],
                                    [6.7, 3.4],
                                    [8.7, 2.1],
                                    [10.9, 1.2],
                                    [13.2, 0.7],
                                    [15.6, 0.5],
                                  ],
                                ],
                              },
                              symbol: {
                                type: "CIMPolygonSymbol",
                                symbolLayers: [
                                  {
                                    type: "CIMSolidFill",
                                    enable: false,
                                    color: [255, 255, 255, 255],
                                  },
                                ],
                              },
                            },
                            {
                              type: "CIMMarkerGraphic",
                              geometry: {
                                rings: [
                                  [
                                    [0.5, 15.6],
                                    [0.7, 13.2],
                                    [1.2, 10.9],
                                    [2.1, 8.7],
                                    [3.4, 6.7],
                                    [4.9, 4.9],
                                    [6.7, 3.4],
                                    [8.7, 2.1],
                                    [10.9, 1.2],
                                    [13.2, 0.7],
                                    [15.6, 0.5],
                                    [18, 0.7],
                                    [20.3, 1.2],
                                    [22.5, 2.1],
                                    [24.5, 3.4],
                                    [26.3, 4.9],
                                    [27.8, 6.7],
                                    [29.1, 8.7],
                                    [30, 10.9],
                                    [30.5, 13.2],
                                    [30.7, 15.6],
                                    [30.5, 18],
                                    [30, 20.3],
                                    [29.1, 22.5],
                                    [27.8, 24.5],
                                    [26.3, 26.3],
                                    [24.5, 27.8],
                                    [22.5, 29.1],
                                    [20.3, 30],
                                    [18, 30.5],
                                    [15.6, 30.7],
                                    [15.6, 30.7],
                                    [13.2, 30.5],
                                    [10.9, 30],
                                    [8.7, 29.1],
                                    [6.7, 27.8],
                                    [4.9, 26.3],
                                    [3.4, 24.5],
                                    [2.1, 22.5],
                                    [1.2, 20.3],
                                    [0.7, 18],
                                    [0.5, 15.6],
                                  ],
                                ],
                              },
                              symbol: {
                                type: "CIMPolygonSymbol",
                                symbolLayers: [
                                  {
                                    type: "CIMSolidStroke",
                                    enable: true,
                                    capStyle: "Butt",
                                    joinStyle: "Miter",
                                    lineStyle3D: "Strip",
                                    miterLimit: 10,
                                    width: 1,
                                    color: [255, 255, 255, 255],
                                  },
                                ],
                              },
                            },
                            {
                              type: "CIMMarkerGraphic",
                              geometry: {
                                rings: [
                                  [
                                    [17.6, 5.6],
                                    [17.6, 18.8],
                                    [23.6, 12.5],
                                    [23.6, 17.6],
                                    [15.6, 25.6],
                                    [7.6, 17.6],
                                    [7.6, 12.5],
                                    [13.6, 18.7],
                                    [13.6, 5.6],
                                    [17.6, 5.6],
                                  ],
                                ],
                              },
                              symbol: {
                                type: "CIMPolygonSymbol",
                                symbolLayers: [
                                  {
                                    type: "CIMSolidFill",
                                    enable: true,
                                    colorLocked: true,
                                    color: [255, 255, 255, 255],
                                  },
                                ],
                              },
                            },
                          ],
                          scaleSymbolsProportionally: true,
                          respectFrame: true,
                          clippingPath: {
                            type: "CIMClippingPath",
                            clippingType: "Intersect",
                            path: {
                              rings: [
                                [
                                  [0, 0],
                                  [31.2, 0],
                                  [31.2, 31.2],
                                  [0, 31.2],
                                  [0, 0],
                                ],
                              ],
                            },
                          },
                          rotation: 0,
                        },
                        {
                          type: "CIMVectorMarker",
                          enable: true,
                          anchorPointUnits: "Relative",
                          dominantSizeAxis3D: "Y",
                          size: 9.69,
                          billboardMode3D: "FaceNearPlane",
                          frame: {
                            xmin: 0,
                            ymin: 0,
                            xmax: 31.2,
                            ymax: 31.2,
                          },
                          markerGraphics: [
                            {
                              type: "CIMMarkerGraphic",
                              geometry: {
                                rings: [
                                  [
                                    [15.6, 0.5],
                                    [17, 0.6],
                                    [17.6, 0.6],
                                    [18.3, 0.8],
                                    [19.1, 0.9],
                                    [19.8, 1.1],
                                    [20.5, 1.3],
                                    [21.2, 1.6],
                                    [21.9, 1.9],
                                    [22.5, 2.2],
                                    [23.2, 2.6],
                                    [23.9, 3],
                                    [24.5, 3.4],
                                    [25.1, 3.9],
                                    [25.7, 4.4],
                                    [26.3, 4.9],
                                    [26.8, 5.5],
                                    [27.3, 6.1],
                                    [27.8, 6.7],
                                    [28.2, 7.3],
                                    [28.6, 8],
                                    [29, 8.7],
                                    [29.3, 9.3],
                                    [29.6, 10],
                                    [29.9, 10.7],
                                    [30.1, 11.4],
                                    [30.3, 12.1],
                                    [30.4, 12.9],
                                    [30.6, 13.6],
                                    [30.6, 14.2],
                                    [30.7, 14.9],
                                    [30.7, 15.6],
                                    [30.5, 18],
                                    [30, 20.3],
                                    [29.1, 22.5],
                                    [27.8, 24.5],
                                    [26.3, 26.3],
                                    [24.5, 27.8],
                                    [22.5, 29.1],
                                    [20.3, 30],
                                    [18, 30.5],
                                    [15.6, 30.7],
                                    [12.7, 30.4],
                                    [10, 29.6],
                                    [7.4, 28.3],
                                    [5.2, 26.5],
                                    [3.3, 24.3],
                                    [1.8, 21.9],
                                    [0.9, 19.1],
                                    [0.5, 16.3],
                                    [0.5, 15.6],
                                    [0.7, 13.2],
                                    [1.2, 10.9],
                                    [2.1, 8.7],
                                    [3.4, 6.7],
                                    [4.9, 4.9],
                                    [6.7, 3.4],
                                    [8.7, 2.1],
                                    [10.9, 1.2],
                                    [13.2, 0.7],
                                    [15.6, 0.5],
                                  ],
                                ],
                              },
                              symbol: {
                                type: "CIMPolygonSymbol",
                                symbolLayers: [
                                  {
                                    type: "CIMSolidFill",
                                    enable: true,
                                    color: [0, 0, 0, 255],
                                  },
                                ],
                              },
                            },
                            {
                              type: "CIMMarkerGraphic",
                              geometry: {
                                rings: [
                                  [
                                    [0.5, 15.6],
                                    [0.7, 13.2],
                                    [1.2, 10.9],
                                    [2.1, 8.7],
                                    [3.4, 6.7],
                                    [4.9, 4.9],
                                    [6.7, 3.4],
                                    [8.7, 2.1],
                                    [10.9, 1.2],
                                    [13.2, 0.7],
                                    [15.6, 0.5],
                                    [18, 0.7],
                                    [20.3, 1.2],
                                    [22.5, 2.1],
                                    [24.5, 3.4],
                                    [26.3, 4.9],
                                    [27.8, 6.7],
                                    [29.1, 8.7],
                                    [30, 10.9],
                                    [30.5, 13.2],
                                    [30.7, 15.6],
                                    [30.5, 18],
                                    [30, 20.3],
                                    [29.1, 22.5],
                                    [27.8, 24.5],
                                    [26.3, 26.3],
                                    [24.5, 27.8],
                                    [22.5, 29.1],
                                    [20.3, 30],
                                    [18, 30.5],
                                    [15.6, 30.7],
                                    [15.6, 30.7],
                                    [13.2, 30.5],
                                    [10.9, 30],
                                    [8.7, 29.1],
                                    [6.7, 27.8],
                                    [4.9, 26.3],
                                    [3.4, 24.5],
                                    [2.1, 22.5],
                                    [1.2, 20.3],
                                    [0.7, 18],
                                    [0.5, 15.6],
                                  ],
                                ],
                              },
                              symbol: {
                                type: "CIMPolygonSymbol",
                                symbolLayers: [
                                  {
                                    type: "CIMSolidStroke",
                                    enable: false,
                                    capStyle: "Butt",
                                    joinStyle: "Miter",
                                    lineStyle3D: "Strip",
                                    miterLimit: 10,
                                    width: 1,
                                    color: [0, 0, 0, 255],
                                  },
                                ],
                              },
                            },
                            {
                              type: "CIMMarkerGraphic",
                              geometry: {
                                rings: [
                                  [
                                    [17.6, 5.6],
                                    [17.6, 18.8],
                                    [23.6, 12.5],
                                    [23.6, 17.6],
                                    [15.6, 25.6],
                                    [7.6, 17.6],
                                    [7.6, 12.5],
                                    [13.6, 18.7],
                                    [13.6, 5.6],
                                    [17.6, 5.6],
                                  ],
                                ],
                              },
                              symbol: {
                                type: "CIMPolygonSymbol",
                                symbolLayers: [
                                  {
                                    type: "CIMSolidFill",
                                    enable: false,
                                    colorLocked: true,
                                    color: [0, 0, 0, 255],
                                  },
                                ],
                              },
                            },
                          ],
                          scaleSymbolsProportionally: true,
                          respectFrame: true,
                          clippingPath: {
                            type: "CIMClippingPath",
                            clippingType: "Intersect",
                            path: {
                              rings: [
                                [
                                  [0, 0],
                                  [31.2, 0],
                                  [31.2, 31.2],
                                  [0, 31.2],
                                  [0, 0],
                                ],
                              ],
                            },
                          },
                          rotation: 0,
                        },
                      ],
                      haloSize: 1,
                      scaleX: 1,
                      angleAlignment: "Display",
                      angle: 0,
                    },
                  },
                  values: [["b"]],
                },
                {
                  label: "Increase",
                  symbol: {
                    type: "CIMSymbolReference",
                    symbol: {
                      type: "CIMPointSymbol",
                      symbolLayers: [
                        {
                          type: "CIMVectorMarker",
                          enable: true,
                          colorLocked: true,
                          anchorPointUnits: "Relative",
                          dominantSizeAxis3D: "Y",
                          size: 9.69,
                          billboardMode3D: "FaceNearPlane",
                          frame: {
                            xmin: 0,
                            ymin: 0,
                            xmax: 31.2,
                            ymax: 31.2,
                          },
                          markerGraphics: [
                            {
                              type: "CIMMarkerGraphic",
                              geometry: {
                                rings: [
                                  [
                                    [15.6, 0.5],
                                    [17, 0.6],
                                    [17.6, 0.6],
                                    [18.3, 0.8],
                                    [19.1, 0.9],
                                    [19.8, 1.1],
                                    [20.5, 1.3],
                                    [21.2, 1.6],
                                    [21.9, 1.9],
                                    [22.5, 2.2],
                                    [23.2, 2.6],
                                    [23.9, 3],
                                    [24.5, 3.4],
                                    [25.1, 3.9],
                                    [25.7, 4.4],
                                    [26.3, 4.9],
                                    [26.8, 5.5],
                                    [27.3, 6.1],
                                    [27.8, 6.7],
                                    [28.2, 7.3],
                                    [28.6, 8],
                                    [29, 8.7],
                                    [29.3, 9.3],
                                    [29.6, 10],
                                    [29.9, 10.7],
                                    [30.1, 11.4],
                                    [30.3, 12.1],
                                    [30.4, 12.9],
                                    [30.6, 13.6],
                                    [30.6, 14.2],
                                    [30.7, 14.9],
                                    [30.7, 15.6],
                                    [30.5, 18],
                                    [30, 20.3],
                                    [29.1, 22.5],
                                    [27.8, 24.5],
                                    [26.3, 26.3],
                                    [24.5, 27.8],
                                    [22.5, 29.1],
                                    [20.3, 30],
                                    [18, 30.5],
                                    [15.6, 30.7],
                                    [12.7, 30.4],
                                    [10, 29.6],
                                    [7.4, 28.3],
                                    [5.2, 26.5],
                                    [3.3, 24.3],
                                    [1.8, 21.9],
                                    [0.9, 19.1],
                                    [0.5, 16.3],
                                    [0.5, 15.6],
                                    [0.7, 13.2],
                                    [1.2, 10.9],
                                    [2.1, 8.7],
                                    [3.4, 6.7],
                                    [4.9, 4.9],
                                    [6.7, 3.4],
                                    [8.7, 2.1],
                                    [10.9, 1.2],
                                    [13.2, 0.7],
                                    [15.6, 0.5],
                                  ],
                                ],
                              },
                              symbol: {
                                type: "CIMPolygonSymbol",
                                symbolLayers: [
                                  {
                                    type: "CIMSolidFill",
                                    enable: false,
                                    color: [255, 255, 255, 255],
                                  },
                                ],
                              },
                            },
                            {
                              type: "CIMMarkerGraphic",
                              geometry: {
                                rings: [
                                  [
                                    [0.5, 15.6],
                                    [0.7, 13.2],
                                    [1.2, 10.9],
                                    [2.1, 8.7],
                                    [3.4, 6.7],
                                    [4.9, 4.9],
                                    [6.7, 3.4],
                                    [8.7, 2.1],
                                    [10.9, 1.2],
                                    [13.2, 0.7],
                                    [15.6, 0.5],
                                    [18, 0.7],
                                    [20.3, 1.2],
                                    [22.5, 2.1],
                                    [24.5, 3.4],
                                    [26.3, 4.9],
                                    [27.8, 6.7],
                                    [29.1, 8.7],
                                    [30, 10.9],
                                    [30.5, 13.2],
                                    [30.7, 15.6],
                                    [30.5, 18],
                                    [30, 20.3],
                                    [29.1, 22.5],
                                    [27.8, 24.5],
                                    [26.3, 26.3],
                                    [24.5, 27.8],
                                    [22.5, 29.1],
                                    [20.3, 30],
                                    [18, 30.5],
                                    [15.6, 30.7],
                                    [15.6, 30.7],
                                    [13.2, 30.5],
                                    [10.9, 30],
                                    [8.7, 29.1],
                                    [6.7, 27.8],
                                    [4.9, 26.3],
                                    [3.4, 24.5],
                                    [2.1, 22.5],
                                    [1.2, 20.3],
                                    [0.7, 18],
                                    [0.5, 15.6],
                                  ],
                                ],
                              },
                              symbol: {
                                type: "CIMPolygonSymbol",
                                symbolLayers: [
                                  {
                                    type: "CIMSolidStroke",
                                    enable: true,
                                    capStyle: "Butt",
                                    joinStyle: "Miter",
                                    lineStyle3D: "Strip",
                                    miterLimit: 10,
                                    width: 1,
                                    color: [255, 255, 255, 255],
                                  },
                                ],
                              },
                            },
                            {
                              type: "CIMMarkerGraphic",
                              geometry: {
                                rings: [
                                  [
                                    [17.6, 5.6],
                                    [17.6, 18.8],
                                    [23.6, 12.5],
                                    [23.6, 17.6],
                                    [15.6, 25.6],
                                    [7.6, 17.6],
                                    [7.6, 12.5],
                                    [13.6, 18.7],
                                    [13.6, 5.6],
                                    [17.6, 5.6],
                                  ],
                                ],
                              },
                              symbol: {
                                type: "CIMPolygonSymbol",
                                symbolLayers: [
                                  {
                                    type: "CIMSolidFill",
                                    enable: true,
                                    colorLocked: true,
                                    color: [255, 255, 255, 255],
                                  },
                                ],
                              },
                            },
                          ],
                          scaleSymbolsProportionally: true,
                          respectFrame: true,
                          clippingPath: {
                            type: "CIMClippingPath",
                            clippingType: "Intersect",
                            path: {
                              rings: [
                                [
                                  [0, 0],
                                  [31.2, 0],
                                  [31.2, 31.2],
                                  [0, 31.2],
                                  [0, 0],
                                ],
                              ],
                            },
                          },
                          rotation: 0,
                        },
                        {
                          type: "CIMVectorMarker",
                          enable: true,
                          anchorPointUnits: "Relative",
                          dominantSizeAxis3D: "Y",
                          size: 9.69,
                          billboardMode3D: "FaceNearPlane",
                          frame: {
                            xmin: 0,
                            ymin: 0,
                            xmax: 31.2,
                            ymax: 31.2,
                          },
                          markerGraphics: [
                            {
                              type: "CIMMarkerGraphic",
                              geometry: {
                                rings: [
                                  [
                                    [15.6, 0.5],
                                    [17, 0.6],
                                    [17.6, 0.6],
                                    [18.3, 0.8],
                                    [19.1, 0.9],
                                    [19.8, 1.1],
                                    [20.5, 1.3],
                                    [21.2, 1.6],
                                    [21.9, 1.9],
                                    [22.5, 2.2],
                                    [23.2, 2.6],
                                    [23.9, 3],
                                    [24.5, 3.4],
                                    [25.1, 3.9],
                                    [25.7, 4.4],
                                    [26.3, 4.9],
                                    [26.8, 5.5],
                                    [27.3, 6.1],
                                    [27.8, 6.7],
                                    [28.2, 7.3],
                                    [28.6, 8],
                                    [29, 8.7],
                                    [29.3, 9.3],
                                    [29.6, 10],
                                    [29.9, 10.7],
                                    [30.1, 11.4],
                                    [30.3, 12.1],
                                    [30.4, 12.9],
                                    [30.6, 13.6],
                                    [30.6, 14.2],
                                    [30.7, 14.9],
                                    [30.7, 15.6],
                                    [30.5, 18],
                                    [30, 20.3],
                                    [29.1, 22.5],
                                    [27.8, 24.5],
                                    [26.3, 26.3],
                                    [24.5, 27.8],
                                    [22.5, 29.1],
                                    [20.3, 30],
                                    [18, 30.5],
                                    [15.6, 30.7],
                                    [12.7, 30.4],
                                    [10, 29.6],
                                    [7.4, 28.3],
                                    [5.2, 26.5],
                                    [3.3, 24.3],
                                    [1.8, 21.9],
                                    [0.9, 19.1],
                                    [0.5, 16.3],
                                    [0.5, 15.6],
                                    [0.7, 13.2],
                                    [1.2, 10.9],
                                    [2.1, 8.7],
                                    [3.4, 6.7],
                                    [4.9, 4.9],
                                    [6.7, 3.4],
                                    [8.7, 2.1],
                                    [10.9, 1.2],
                                    [13.2, 0.7],
                                    [15.6, 0.5],
                                  ],
                                ],
                              },
                              symbol: {
                                type: "CIMPolygonSymbol",
                                symbolLayers: [
                                  {
                                    type: "CIMSolidFill",
                                    enable: true,
                                    color: [65, 75, 140, 255],
                                  },
                                ],
                              },
                            },
                            {
                              type: "CIMMarkerGraphic",
                              geometry: {
                                rings: [
                                  [
                                    [0.5, 15.6],
                                    [0.7, 13.2],
                                    [1.2, 10.9],
                                    [2.1, 8.7],
                                    [3.4, 6.7],
                                    [4.9, 4.9],
                                    [6.7, 3.4],
                                    [8.7, 2.1],
                                    [10.9, 1.2],
                                    [13.2, 0.7],
                                    [15.6, 0.5],
                                    [18, 0.7],
                                    [20.3, 1.2],
                                    [22.5, 2.1],
                                    [24.5, 3.4],
                                    [26.3, 4.9],
                                    [27.8, 6.7],
                                    [29.1, 8.7],
                                    [30, 10.9],
                                    [30.5, 13.2],
                                    [30.7, 15.6],
                                    [30.5, 18],
                                    [30, 20.3],
                                    [29.1, 22.5],
                                    [27.8, 24.5],
                                    [26.3, 26.3],
                                    [24.5, 27.8],
                                    [22.5, 29.1],
                                    [20.3, 30],
                                    [18, 30.5],
                                    [15.6, 30.7],
                                    [15.6, 30.7],
                                    [13.2, 30.5],
                                    [10.9, 30],
                                    [8.7, 29.1],
                                    [6.7, 27.8],
                                    [4.9, 26.3],
                                    [3.4, 24.5],
                                    [2.1, 22.5],
                                    [1.2, 20.3],
                                    [0.7, 18],
                                    [0.5, 15.6],
                                  ],
                                ],
                              },
                              symbol: {
                                type: "CIMPolygonSymbol",
                                symbolLayers: [
                                  {
                                    type: "CIMSolidStroke",
                                    enable: false,
                                    capStyle: "Butt",
                                    joinStyle: "Miter",
                                    lineStyle3D: "Strip",
                                    miterLimit: 10,
                                    width: 1,
                                    color: [0, 0, 0, 255],
                                  },
                                ],
                              },
                            },
                            {
                              type: "CIMMarkerGraphic",
                              geometry: {
                                rings: [
                                  [
                                    [17.6, 5.6],
                                    [17.6, 18.8],
                                    [23.6, 12.5],
                                    [23.6, 17.6],
                                    [15.6, 25.6],
                                    [7.6, 17.6],
                                    [7.6, 12.5],
                                    [13.6, 18.7],
                                    [13.6, 5.6],
                                    [17.6, 5.6],
                                  ],
                                ],
                              },
                              symbol: {
                                type: "CIMPolygonSymbol",
                                symbolLayers: [
                                  {
                                    type: "CIMSolidFill",
                                    enable: false,
                                    colorLocked: true,
                                    color: [0, 0, 0, 255],
                                  },
                                ],
                              },
                            },
                          ],
                          scaleSymbolsProportionally: true,
                          respectFrame: true,
                          clippingPath: {
                            type: "CIMClippingPath",
                            clippingType: "Intersect",
                            path: {
                              rings: [
                                [
                                  [0, 0],
                                  [31.2, 0],
                                  [31.2, 31.2],
                                  [0, 31.2],
                                  [0, 0],
                                ],
                              ],
                            },
                          },
                          rotation: 0,
                        },
                      ],
                      haloSize: 1,
                      scaleX: 1,
                      angleAlignment: "Display",
                      angle: 0,
                    },
                  },
                  values: [["i"]],
                },
                {
                  label: "Decrease",
                  symbol: {
                    type: "CIMSymbolReference",
                    symbol: {
                      type: "CIMPointSymbol",
                      symbolLayers: [
                        {
                          type: "CIMVectorMarker",
                          enable: true,
                          colorLocked: true,
                          anchorPointUnits: "Relative",
                          dominantSizeAxis3D: "Y",
                          size: 9.69,
                          billboardMode3D: "FaceNearPlane",
                          frame: {
                            xmin: 0,
                            ymin: 0,
                            xmax: 31.2,
                            ymax: 31.2,
                          },
                          markerGraphics: [
                            {
                              type: "CIMMarkerGraphic",
                              geometry: {
                                rings: [
                                  [
                                    [15.6, 0.5],
                                    [17, 0.6],
                                    [17.6, 0.6],
                                    [18.3, 0.8],
                                    [19.1, 0.9],
                                    [19.8, 1.1],
                                    [20.5, 1.3],
                                    [21.2, 1.6],
                                    [21.9, 1.9],
                                    [22.5, 2.2],
                                    [23.2, 2.6],
                                    [23.9, 3],
                                    [24.5, 3.4],
                                    [25.1, 3.9],
                                    [25.7, 4.4],
                                    [26.3, 4.9],
                                    [26.8, 5.5],
                                    [27.3, 6.1],
                                    [27.8, 6.7],
                                    [28.2, 7.3],
                                    [28.6, 8],
                                    [29, 8.7],
                                    [29.3, 9.3],
                                    [29.6, 10],
                                    [29.9, 10.7],
                                    [30.1, 11.4],
                                    [30.3, 12.1],
                                    [30.4, 12.9],
                                    [30.6, 13.6],
                                    [30.6, 14.2],
                                    [30.7, 14.9],
                                    [30.7, 15.6],
                                    [30.5, 18],
                                    [30, 20.3],
                                    [29.1, 22.5],
                                    [27.8, 24.5],
                                    [26.3, 26.3],
                                    [24.5, 27.8],
                                    [22.5, 29.1],
                                    [20.3, 30],
                                    [18, 30.5],
                                    [15.6, 30.7],
                                    [12.7, 30.4],
                                    [10, 29.6],
                                    [7.4, 28.3],
                                    [5.2, 26.5],
                                    [3.3, 24.3],
                                    [1.8, 21.9],
                                    [0.9, 19.1],
                                    [0.5, 16.3],
                                    [0.5, 15.6],
                                    [0.7, 13.2],
                                    [1.2, 10.9],
                                    [2.1, 8.7],
                                    [3.4, 6.7],
                                    [4.9, 4.9],
                                    [6.7, 3.4],
                                    [8.7, 2.1],
                                    [10.9, 1.2],
                                    [13.2, 0.7],
                                    [15.6, 0.5],
                                  ],
                                ],
                              },
                              symbol: {
                                type: "CIMPolygonSymbol",
                                symbolLayers: [
                                  {
                                    type: "CIMSolidFill",
                                    enable: false,
                                    color: [255, 255, 255, 255],
                                  },
                                ],
                              },
                            },
                            {
                              type: "CIMMarkerGraphic",
                              geometry: {
                                rings: [
                                  [
                                    [0.5, 15.6],
                                    [0.7, 13.2],
                                    [1.2, 10.9],
                                    [2.1, 8.7],
                                    [3.4, 6.7],
                                    [4.9, 4.9],
                                    [6.7, 3.4],
                                    [8.7, 2.1],
                                    [10.9, 1.2],
                                    [13.2, 0.7],
                                    [15.6, 0.5],
                                    [18, 0.7],
                                    [20.3, 1.2],
                                    [22.5, 2.1],
                                    [24.5, 3.4],
                                    [26.3, 4.9],
                                    [27.8, 6.7],
                                    [29.1, 8.7],
                                    [30, 10.9],
                                    [30.5, 13.2],
                                    [30.7, 15.6],
                                    [30.5, 18],
                                    [30, 20.3],
                                    [29.1, 22.5],
                                    [27.8, 24.5],
                                    [26.3, 26.3],
                                    [24.5, 27.8],
                                    [22.5, 29.1],
                                    [20.3, 30],
                                    [18, 30.5],
                                    [15.6, 30.7],
                                    [15.6, 30.7],
                                    [13.2, 30.5],
                                    [10.9, 30],
                                    [8.7, 29.1],
                                    [6.7, 27.8],
                                    [4.9, 26.3],
                                    [3.4, 24.5],
                                    [2.1, 22.5],
                                    [1.2, 20.3],
                                    [0.7, 18],
                                    [0.5, 15.6],
                                  ],
                                ],
                              },
                              symbol: {
                                type: "CIMPolygonSymbol",
                                symbolLayers: [
                                  {
                                    type: "CIMSolidStroke",
                                    enable: true,
                                    capStyle: "Butt",
                                    joinStyle: "Miter",
                                    lineStyle3D: "Strip",
                                    miterLimit: 10,
                                    width: 1,
                                    color: [255, 255, 255, 255],
                                  },
                                ],
                              },
                            },
                            {
                              type: "CIMMarkerGraphic",
                              geometry: {
                                rings: [
                                  [
                                    [17.6, 5.6],
                                    [17.6, 18.8],
                                    [23.6, 12.5],
                                    [23.6, 17.6],
                                    [15.6, 25.6],
                                    [7.6, 17.6],
                                    [7.6, 12.5],
                                    [13.6, 18.7],
                                    [13.6, 5.6],
                                    [17.6, 5.6],
                                  ],
                                ],
                              },
                              symbol: {
                                type: "CIMPolygonSymbol",
                                symbolLayers: [
                                  {
                                    type: "CIMSolidFill",
                                    enable: true,
                                    colorLocked: true,
                                    color: [255, 255, 255, 255],
                                  },
                                ],
                              },
                            },
                          ],
                          scaleSymbolsProportionally: true,
                          respectFrame: true,
                          clippingPath: {
                            type: "CIMClippingPath",
                            clippingType: "Intersect",
                            path: {
                              rings: [
                                [
                                  [0, 0],
                                  [31.2, 0],
                                  [31.2, 31.2],
                                  [0, 31.2],
                                  [0, 0],
                                ],
                              ],
                            },
                          },
                          rotation: 180,
                        },
                        {
                          type: "CIMVectorMarker",
                          enable: true,
                          anchorPointUnits: "Relative",
                          dominantSizeAxis3D: "Y",
                          size: 9.69,
                          billboardMode3D: "FaceNearPlane",
                          frame: {
                            xmin: 0,
                            ymin: 0,
                            xmax: 31.2,
                            ymax: 31.2,
                          },
                          markerGraphics: [
                            {
                              type: "CIMMarkerGraphic",
                              geometry: {
                                rings: [
                                  [
                                    [15.6, 0.5],
                                    [17, 0.6],
                                    [17.6, 0.6],
                                    [18.3, 0.8],
                                    [19.1, 0.9],
                                    [19.8, 1.1],
                                    [20.5, 1.3],
                                    [21.2, 1.6],
                                    [21.9, 1.9],
                                    [22.5, 2.2],
                                    [23.2, 2.6],
                                    [23.9, 3],
                                    [24.5, 3.4],
                                    [25.1, 3.9],
                                    [25.7, 4.4],
                                    [26.3, 4.9],
                                    [26.8, 5.5],
                                    [27.3, 6.1],
                                    [27.8, 6.7],
                                    [28.2, 7.3],
                                    [28.6, 8],
                                    [29, 8.7],
                                    [29.3, 9.3],
                                    [29.6, 10],
                                    [29.9, 10.7],
                                    [30.1, 11.4],
                                    [30.3, 12.1],
                                    [30.4, 12.9],
                                    [30.6, 13.6],
                                    [30.6, 14.2],
                                    [30.7, 14.9],
                                    [30.7, 15.6],
                                    [30.5, 18],
                                    [30, 20.3],
                                    [29.1, 22.5],
                                    [27.8, 24.5],
                                    [26.3, 26.3],
                                    [24.5, 27.8],
                                    [22.5, 29.1],
                                    [20.3, 30],
                                    [18, 30.5],
                                    [15.6, 30.7],
                                    [12.7, 30.4],
                                    [10, 29.6],
                                    [7.4, 28.3],
                                    [5.2, 26.5],
                                    [3.3, 24.3],
                                    [1.8, 21.9],
                                    [0.9, 19.1],
                                    [0.5, 16.3],
                                    [0.5, 15.6],
                                    [0.7, 13.2],
                                    [1.2, 10.9],
                                    [2.1, 8.7],
                                    [3.4, 6.7],
                                    [4.9, 4.9],
                                    [6.7, 3.4],
                                    [8.7, 2.1],
                                    [10.9, 1.2],
                                    [13.2, 0.7],
                                    [15.6, 0.5],
                                  ],
                                ],
                              },
                              symbol: {
                                type: "CIMPolygonSymbol",
                                symbolLayers: [
                                  {
                                    type: "CIMSolidFill",
                                    enable: true,
                                    color: [153, 144, 92, 255],
                                  },
                                ],
                              },
                            },
                            {
                              type: "CIMMarkerGraphic",
                              geometry: {
                                rings: [
                                  [
                                    [0.5, 15.6],
                                    [0.7, 13.2],
                                    [1.2, 10.9],
                                    [2.1, 8.7],
                                    [3.4, 6.7],
                                    [4.9, 4.9],
                                    [6.7, 3.4],
                                    [8.7, 2.1],
                                    [10.9, 1.2],
                                    [13.2, 0.7],
                                    [15.6, 0.5],
                                    [18, 0.7],
                                    [20.3, 1.2],
                                    [22.5, 2.1],
                                    [24.5, 3.4],
                                    [26.3, 4.9],
                                    [27.8, 6.7],
                                    [29.1, 8.7],
                                    [30, 10.9],
                                    [30.5, 13.2],
                                    [30.7, 15.6],
                                    [30.5, 18],
                                    [30, 20.3],
                                    [29.1, 22.5],
                                    [27.8, 24.5],
                                    [26.3, 26.3],
                                    [24.5, 27.8],
                                    [22.5, 29.1],
                                    [20.3, 30],
                                    [18, 30.5],
                                    [15.6, 30.7],
                                    [15.6, 30.7],
                                    [13.2, 30.5],
                                    [10.9, 30],
                                    [8.7, 29.1],
                                    [6.7, 27.8],
                                    [4.9, 26.3],
                                    [3.4, 24.5],
                                    [2.1, 22.5],
                                    [1.2, 20.3],
                                    [0.7, 18],
                                    [0.5, 15.6],
                                  ],
                                ],
                              },
                              symbol: {
                                type: "CIMPolygonSymbol",
                                symbolLayers: [
                                  {
                                    type: "CIMSolidStroke",
                                    enable: false,
                                    capStyle: "Butt",
                                    joinStyle: "Miter",
                                    lineStyle3D: "Strip",
                                    miterLimit: 10,
                                    width: 1,
                                    color: [0, 0, 0, 255],
                                  },
                                ],
                              },
                            },
                            {
                              type: "CIMMarkerGraphic",
                              geometry: {
                                rings: [
                                  [
                                    [17.6, 5.6],
                                    [17.6, 18.8],
                                    [23.6, 12.5],
                                    [23.6, 17.6],
                                    [15.6, 25.6],
                                    [7.6, 17.6],
                                    [7.6, 12.5],
                                    [13.6, 18.7],
                                    [13.6, 5.6],
                                    [17.6, 5.6],
                                  ],
                                ],
                              },
                              symbol: {
                                type: "CIMPolygonSymbol",
                                symbolLayers: [
                                  {
                                    type: "CIMSolidFill",
                                    enable: false,
                                    colorLocked: true,
                                    color: [0, 0, 0, 255],
                                  },
                                ],
                              },
                            },
                          ],
                          scaleSymbolsProportionally: true,
                          respectFrame: true,
                          clippingPath: {
                            type: "CIMClippingPath",
                            clippingType: "Intersect",
                            path: {
                              rings: [
                                [
                                  [0, 0],
                                  [31.2, 0],
                                  [31.2, 31.2],
                                  [0, 31.2],
                                  [0, 0],
                                ],
                              ],
                            },
                          },
                          rotation: 180,
                        },
                      ],
                      haloSize: 1,
                      scaleX: 1,
                      angleAlignment: "Display",
                      angle: 180,
                    },
                  },
                  values: [["d"]],
                },
                {
                  label: "Unknown",
                  symbol: {
                    type: "CIMSymbolReference",
                    symbol: {
                      type: "CIMPointSymbol",
                      symbolLayers: [
                        {
                          type: "CIMVectorMarker",
                          enable: true,
                          colorLocked: true,
                          anchorPointUnits: "Relative",
                          dominantSizeAxis3D: "Y",
                          size: 9.69,
                          billboardMode3D: "FaceNearPlane",
                          frame: {
                            xmin: 0,
                            ymin: 0,
                            xmax: 31.2,
                            ymax: 31.2,
                          },
                          markerGraphics: [
                            {
                              type: "CIMMarkerGraphic",
                              geometry: {
                                rings: [
                                  [
                                    [15.6, 0.5],
                                    [17, 0.6],
                                    [17.6, 0.6],
                                    [18.3, 0.8],
                                    [19.1, 0.9],
                                    [19.8, 1.1],
                                    [20.5, 1.3],
                                    [21.2, 1.6],
                                    [21.9, 1.9],
                                    [22.5, 2.2],
                                    [23.2, 2.6],
                                    [23.9, 3],
                                    [24.5, 3.4],
                                    [25.1, 3.9],
                                    [25.7, 4.4],
                                    [26.3, 4.9],
                                    [26.8, 5.5],
                                    [27.3, 6.1],
                                    [27.8, 6.7],
                                    [28.2, 7.3],
                                    [28.6, 8],
                                    [29, 8.7],
                                    [29.3, 9.3],
                                    [29.6, 10],
                                    [29.9, 10.7],
                                    [30.1, 11.4],
                                    [30.3, 12.1],
                                    [30.4, 12.9],
                                    [30.6, 13.6],
                                    [30.6, 14.2],
                                    [30.7, 14.9],
                                    [30.7, 15.6],
                                    [30.5, 18],
                                    [30, 20.3],
                                    [29.1, 22.5],
                                    [27.8, 24.5],
                                    [26.3, 26.3],
                                    [24.5, 27.8],
                                    [22.5, 29.1],
                                    [20.3, 30],
                                    [18, 30.5],
                                    [15.6, 30.7],
                                    [12.7, 30.4],
                                    [10, 29.6],
                                    [7.4, 28.3],
                                    [5.2, 26.5],
                                    [3.3, 24.3],
                                    [1.8, 21.9],
                                    [0.9, 19.1],
                                    [0.5, 16.3],
                                    [0.5, 15.6],
                                    [0.7, 13.2],
                                    [1.2, 10.9],
                                    [2.1, 8.7],
                                    [3.4, 6.7],
                                    [4.9, 4.9],
                                    [6.7, 3.4],
                                    [8.7, 2.1],
                                    [10.9, 1.2],
                                    [13.2, 0.7],
                                    [15.6, 0.5],
                                  ],
                                ],
                              },
                              symbol: {
                                type: "CIMPolygonSymbol",
                                symbolLayers: [
                                  {
                                    type: "CIMSolidFill",
                                    enable: false,
                                    color: [255, 255, 255, 255],
                                  },
                                ],
                              },
                            },
                            {
                              type: "CIMMarkerGraphic",
                              geometry: {
                                rings: [
                                  [
                                    [0.5, 15.6],
                                    [0.7, 13.2],
                                    [1.2, 10.9],
                                    [2.1, 8.7],
                                    [3.4, 6.7],
                                    [4.9, 4.9],
                                    [6.7, 3.4],
                                    [8.7, 2.1],
                                    [10.9, 1.2],
                                    [13.2, 0.7],
                                    [15.6, 0.5],
                                    [18, 0.7],
                                    [20.3, 1.2],
                                    [22.5, 2.1],
                                    [24.5, 3.4],
                                    [26.3, 4.9],
                                    [27.8, 6.7],
                                    [29.1, 8.7],
                                    [30, 10.9],
                                    [30.5, 13.2],
                                    [30.7, 15.6],
                                    [30.5, 18],
                                    [30, 20.3],
                                    [29.1, 22.5],
                                    [27.8, 24.5],
                                    [26.3, 26.3],
                                    [24.5, 27.8],
                                    [22.5, 29.1],
                                    [20.3, 30],
                                    [18, 30.5],
                                    [15.6, 30.7],
                                    [15.6, 30.7],
                                    [13.2, 30.5],
                                    [10.9, 30],
                                    [8.7, 29.1],
                                    [6.7, 27.8],
                                    [4.9, 26.3],
                                    [3.4, 24.5],
                                    [2.1, 22.5],
                                    [1.2, 20.3],
                                    [0.7, 18],
                                    [0.5, 15.6],
                                  ],
                                ],
                              },
                              symbol: {
                                type: "CIMPolygonSymbol",
                                symbolLayers: [
                                  {
                                    type: "CIMSolidStroke",
                                    enable: true,
                                    capStyle: "Butt",
                                    joinStyle: "Miter",
                                    lineStyle3D: "Strip",
                                    miterLimit: 10,
                                    width: 1,
                                    color: [255, 255, 255, 255],
                                  },
                                ],
                              },
                            },
                            {
                              type: "CIMMarkerGraphic",
                              geometry: {
                                rings: [
                                  [
                                    [17.6, 5.6],
                                    [17.6, 18.8],
                                    [23.6, 12.5],
                                    [23.6, 17.6],
                                    [15.6, 25.6],
                                    [7.6, 17.6],
                                    [7.6, 12.5],
                                    [13.6, 18.7],
                                    [13.6, 5.6],
                                    [17.6, 5.6],
                                  ],
                                ],
                              },
                              symbol: {
                                type: "CIMPolygonSymbol",
                                symbolLayers: [
                                  {
                                    type: "CIMSolidFill",
                                    enable: true,
                                    colorLocked: true,
                                    color: [255, 255, 255, 255],
                                  },
                                ],
                              },
                            },
                          ],
                          scaleSymbolsProportionally: true,
                          respectFrame: true,
                          clippingPath: {
                            type: "CIMClippingPath",
                            clippingType: "Intersect",
                            path: {
                              rings: [
                                [
                                  [0, 0],
                                  [31.2, 0],
                                  [31.2, 31.2],
                                  [0, 31.2],
                                  [0, 0],
                                ],
                              ],
                            },
                          },
                          rotation: 0,
                        },
                        {
                          type: "CIMVectorMarker",
                          enable: true,
                          anchorPointUnits: "Relative",
                          dominantSizeAxis3D: "Y",
                          size: 9.69,
                          billboardMode3D: "FaceNearPlane",
                          frame: {
                            xmin: 0,
                            ymin: 0,
                            xmax: 31.2,
                            ymax: 31.2,
                          },
                          markerGraphics: [
                            {
                              type: "CIMMarkerGraphic",
                              geometry: {
                                rings: [
                                  [
                                    [15.6, 0.5],
                                    [17, 0.6],
                                    [17.6, 0.6],
                                    [18.3, 0.8],
                                    [19.1, 0.9],
                                    [19.8, 1.1],
                                    [20.5, 1.3],
                                    [21.2, 1.6],
                                    [21.9, 1.9],
                                    [22.5, 2.2],
                                    [23.2, 2.6],
                                    [23.9, 3],
                                    [24.5, 3.4],
                                    [25.1, 3.9],
                                    [25.7, 4.4],
                                    [26.3, 4.9],
                                    [26.8, 5.5],
                                    [27.3, 6.1],
                                    [27.8, 6.7],
                                    [28.2, 7.3],
                                    [28.6, 8],
                                    [29, 8.7],
                                    [29.3, 9.3],
                                    [29.6, 10],
                                    [29.9, 10.7],
                                    [30.1, 11.4],
                                    [30.3, 12.1],
                                    [30.4, 12.9],
                                    [30.6, 13.6],
                                    [30.6, 14.2],
                                    [30.7, 14.9],
                                    [30.7, 15.6],
                                    [30.5, 18],
                                    [30, 20.3],
                                    [29.1, 22.5],
                                    [27.8, 24.5],
                                    [26.3, 26.3],
                                    [24.5, 27.8],
                                    [22.5, 29.1],
                                    [20.3, 30],
                                    [18, 30.5],
                                    [15.6, 30.7],
                                    [12.7, 30.4],
                                    [10, 29.6],
                                    [7.4, 28.3],
                                    [5.2, 26.5],
                                    [3.3, 24.3],
                                    [1.8, 21.9],
                                    [0.9, 19.1],
                                    [0.5, 16.3],
                                    [0.5, 15.6],
                                    [0.7, 13.2],
                                    [1.2, 10.9],
                                    [2.1, 8.7],
                                    [3.4, 6.7],
                                    [4.9, 4.9],
                                    [6.7, 3.4],
                                    [8.7, 2.1],
                                    [10.9, 1.2],
                                    [13.2, 0.7],
                                    [15.6, 0.5],
                                  ],
                                ],
                              },
                              symbol: {
                                type: "CIMPolygonSymbol",
                                symbolLayers: [
                                  {
                                    type: "CIMSolidFill",
                                    enable: true,
                                    color: [137, 166, 166, 255],
                                  },
                                ],
                              },
                            },
                            {
                              type: "CIMMarkerGraphic",
                              geometry: {
                                rings: [
                                  [
                                    [0.5, 15.6],
                                    [0.7, 13.2],
                                    [1.2, 10.9],
                                    [2.1, 8.7],
                                    [3.4, 6.7],
                                    [4.9, 4.9],
                                    [6.7, 3.4],
                                    [8.7, 2.1],
                                    [10.9, 1.2],
                                    [13.2, 0.7],
                                    [15.6, 0.5],
                                    [18, 0.7],
                                    [20.3, 1.2],
                                    [22.5, 2.1],
                                    [24.5, 3.4],
                                    [26.3, 4.9],
                                    [27.8, 6.7],
                                    [29.1, 8.7],
                                    [30, 10.9],
                                    [30.5, 13.2],
                                    [30.7, 15.6],
                                    [30.5, 18],
                                    [30, 20.3],
                                    [29.1, 22.5],
                                    [27.8, 24.5],
                                    [26.3, 26.3],
                                    [24.5, 27.8],
                                    [22.5, 29.1],
                                    [20.3, 30],
                                    [18, 30.5],
                                    [15.6, 30.7],
                                    [15.6, 30.7],
                                    [13.2, 30.5],
                                    [10.9, 30],
                                    [8.7, 29.1],
                                    [6.7, 27.8],
                                    [4.9, 26.3],
                                    [3.4, 24.5],
                                    [2.1, 22.5],
                                    [1.2, 20.3],
                                    [0.7, 18],
                                    [0.5, 15.6],
                                  ],
                                ],
                              },
                              symbol: {
                                type: "CIMPolygonSymbol",
                                symbolLayers: [
                                  {
                                    type: "CIMSolidStroke",
                                    enable: false,
                                    capStyle: "Butt",
                                    joinStyle: "Miter",
                                    lineStyle3D: "Strip",
                                    miterLimit: 10,
                                    width: 1,
                                    color: [0, 0, 0, 255],
                                  },
                                ],
                              },
                            },
                            {
                              type: "CIMMarkerGraphic",
                              geometry: {
                                rings: [
                                  [
                                    [17.6, 5.6],
                                    [17.6, 18.8],
                                    [23.6, 12.5],
                                    [23.6, 17.6],
                                    [15.6, 25.6],
                                    [7.6, 17.6],
                                    [7.6, 12.5],
                                    [13.6, 18.7],
                                    [13.6, 5.6],
                                    [17.6, 5.6],
                                  ],
                                ],
                              },
                              symbol: {
                                type: "CIMPolygonSymbol",
                                symbolLayers: [
                                  {
                                    type: "CIMSolidFill",
                                    enable: false,
                                    colorLocked: true,
                                    color: [0, 0, 0, 255],
                                  },
                                ],
                              },
                            },
                          ],
                          scaleSymbolsProportionally: true,
                          respectFrame: true,
                          clippingPath: {
                            type: "CIMClippingPath",
                            clippingType: "Intersect",
                            path: {
                              rings: [
                                [
                                  [0, 0],
                                  [31.2, 0],
                                  [31.2, 31.2],
                                  [0, 31.2],
                                  [0, 0],
                                ],
                              ],
                            },
                          },
                          rotation: 0,
                        },
                      ],
                      haloSize: 1,
                      scaleX: 1,
                      angleAlignment: "Display",
                      angle: 0,
                    },
                  },
                  values: [["u"]],
                },
              ],
            },
          ],
          uniqueValueInfos: [
            {
              label: "Both",
              symbol: {
                type: "CIMSymbolReference",
                symbol: {
                  type: "CIMPointSymbol",
                  symbolLayers: [
                    {
                      type: "CIMVectorMarker",
                      enable: true,
                      colorLocked: true,
                      anchorPointUnits: "Relative",
                      dominantSizeAxis3D: "Y",
                      size: 9.69,
                      billboardMode3D: "FaceNearPlane",
                      frame: {
                        xmin: 0,
                        ymin: 0,
                        xmax: 31.2,
                        ymax: 31.2,
                      },
                      markerGraphics: [
                        {
                          type: "CIMMarkerGraphic",
                          geometry: {
                            rings: [
                              [
                                [15.6, 0.5],
                                [17, 0.6],
                                [17.6, 0.6],
                                [18.3, 0.8],
                                [19.1, 0.9],
                                [19.8, 1.1],
                                [20.5, 1.3],
                                [21.2, 1.6],
                                [21.9, 1.9],
                                [22.5, 2.2],
                                [23.2, 2.6],
                                [23.9, 3],
                                [24.5, 3.4],
                                [25.1, 3.9],
                                [25.7, 4.4],
                                [26.3, 4.9],
                                [26.8, 5.5],
                                [27.3, 6.1],
                                [27.8, 6.7],
                                [28.2, 7.3],
                                [28.6, 8],
                                [29, 8.7],
                                [29.3, 9.3],
                                [29.6, 10],
                                [29.9, 10.7],
                                [30.1, 11.4],
                                [30.3, 12.1],
                                [30.4, 12.9],
                                [30.6, 13.6],
                                [30.6, 14.2],
                                [30.7, 14.9],
                                [30.7, 15.6],
                                [30.5, 18],
                                [30, 20.3],
                                [29.1, 22.5],
                                [27.8, 24.5],
                                [26.3, 26.3],
                                [24.5, 27.8],
                                [22.5, 29.1],
                                [20.3, 30],
                                [18, 30.5],
                                [15.6, 30.7],
                                [12.7, 30.4],
                                [10, 29.6],
                                [7.4, 28.3],
                                [5.2, 26.5],
                                [3.3, 24.3],
                                [1.8, 21.9],
                                [0.9, 19.1],
                                [0.5, 16.3],
                                [0.5, 15.6],
                                [0.7, 13.2],
                                [1.2, 10.9],
                                [2.1, 8.7],
                                [3.4, 6.7],
                                [4.9, 4.9],
                                [6.7, 3.4],
                                [8.7, 2.1],
                                [10.9, 1.2],
                                [13.2, 0.7],
                                [15.6, 0.5],
                              ],
                            ],
                          },
                          symbol: {
                            type: "CIMPolygonSymbol",
                            symbolLayers: [
                              {
                                type: "CIMSolidFill",
                                enable: false,
                                color: [255, 255, 255, 255],
                              },
                            ],
                          },
                        },
                        {
                          type: "CIMMarkerGraphic",
                          geometry: {
                            rings: [
                              [
                                [0.5, 15.6],
                                [0.7, 13.2],
                                [1.2, 10.9],
                                [2.1, 8.7],
                                [3.4, 6.7],
                                [4.9, 4.9],
                                [6.7, 3.4],
                                [8.7, 2.1],
                                [10.9, 1.2],
                                [13.2, 0.7],
                                [15.6, 0.5],
                                [18, 0.7],
                                [20.3, 1.2],
                                [22.5, 2.1],
                                [24.5, 3.4],
                                [26.3, 4.9],
                                [27.8, 6.7],
                                [29.1, 8.7],
                                [30, 10.9],
                                [30.5, 13.2],
                                [30.7, 15.6],
                                [30.5, 18],
                                [30, 20.3],
                                [29.1, 22.5],
                                [27.8, 24.5],
                                [26.3, 26.3],
                                [24.5, 27.8],
                                [22.5, 29.1],
                                [20.3, 30],
                                [18, 30.5],
                                [15.6, 30.7],
                                [15.6, 30.7],
                                [13.2, 30.5],
                                [10.9, 30],
                                [8.7, 29.1],
                                [6.7, 27.8],
                                [4.9, 26.3],
                                [3.4, 24.5],
                                [2.1, 22.5],
                                [1.2, 20.3],
                                [0.7, 18],
                                [0.5, 15.6],
                              ],
                            ],
                          },
                          symbol: {
                            type: "CIMPolygonSymbol",
                            symbolLayers: [
                              {
                                type: "CIMSolidStroke",
                                enable: true,
                                capStyle: "Butt",
                                joinStyle: "Miter",
                                lineStyle3D: "Strip",
                                miterLimit: 10,
                                width: 1,
                                color: [255, 255, 255, 255],
                              },
                            ],
                          },
                        },
                        {
                          type: "CIMMarkerGraphic",
                          geometry: {
                            rings: [
                              [
                                [17.6, 5.6],
                                [17.6, 18.8],
                                [23.6, 12.5],
                                [23.6, 17.6],
                                [15.6, 25.6],
                                [7.6, 17.6],
                                [7.6, 12.5],
                                [13.6, 18.7],
                                [13.6, 5.6],
                                [17.6, 5.6],
                              ],
                            ],
                          },
                          symbol: {
                            type: "CIMPolygonSymbol",
                            symbolLayers: [
                              {
                                type: "CIMSolidFill",
                                enable: true,
                                colorLocked: true,
                                color: [255, 255, 255, 255],
                              },
                            ],
                          },
                        },
                      ],
                      scaleSymbolsProportionally: true,
                      respectFrame: true,
                      clippingPath: {
                        type: "CIMClippingPath",
                        clippingType: "Intersect",
                        path: {
                          rings: [
                            [
                              [0, 0],
                              [31.2, 0],
                              [31.2, 31.2],
                              [0, 31.2],
                              [0, 0],
                            ],
                          ],
                        },
                      },
                      rotation: 0,
                    },
                    {
                      type: "CIMVectorMarker",
                      enable: true,
                      anchorPointUnits: "Relative",
                      dominantSizeAxis3D: "Y",
                      size: 9.69,
                      billboardMode3D: "FaceNearPlane",
                      frame: {
                        xmin: 0,
                        ymin: 0,
                        xmax: 31.2,
                        ymax: 31.2,
                      },
                      markerGraphics: [
                        {
                          type: "CIMMarkerGraphic",
                          geometry: {
                            rings: [
                              [
                                [15.6, 0.5],
                                [17, 0.6],
                                [17.6, 0.6],
                                [18.3, 0.8],
                                [19.1, 0.9],
                                [19.8, 1.1],
                                [20.5, 1.3],
                                [21.2, 1.6],
                                [21.9, 1.9],
                                [22.5, 2.2],
                                [23.2, 2.6],
                                [23.9, 3],
                                [24.5, 3.4],
                                [25.1, 3.9],
                                [25.7, 4.4],
                                [26.3, 4.9],
                                [26.8, 5.5],
                                [27.3, 6.1],
                                [27.8, 6.7],
                                [28.2, 7.3],
                                [28.6, 8],
                                [29, 8.7],
                                [29.3, 9.3],
                                [29.6, 10],
                                [29.9, 10.7],
                                [30.1, 11.4],
                                [30.3, 12.1],
                                [30.4, 12.9],
                                [30.6, 13.6],
                                [30.6, 14.2],
                                [30.7, 14.9],
                                [30.7, 15.6],
                                [30.5, 18],
                                [30, 20.3],
                                [29.1, 22.5],
                                [27.8, 24.5],
                                [26.3, 26.3],
                                [24.5, 27.8],
                                [22.5, 29.1],
                                [20.3, 30],
                                [18, 30.5],
                                [15.6, 30.7],
                                [12.7, 30.4],
                                [10, 29.6],
                                [7.4, 28.3],
                                [5.2, 26.5],
                                [3.3, 24.3],
                                [1.8, 21.9],
                                [0.9, 19.1],
                                [0.5, 16.3],
                                [0.5, 15.6],
                                [0.7, 13.2],
                                [1.2, 10.9],
                                [2.1, 8.7],
                                [3.4, 6.7],
                                [4.9, 4.9],
                                [6.7, 3.4],
                                [8.7, 2.1],
                                [10.9, 1.2],
                                [13.2, 0.7],
                                [15.6, 0.5],
                              ],
                            ],
                          },
                          symbol: {
                            type: "CIMPolygonSymbol",
                            symbolLayers: [
                              {
                                type: "CIMSolidFill",
                                enable: true,
                                color: [0, 0, 0, 255],
                              },
                            ],
                          },
                        },
                        {
                          type: "CIMMarkerGraphic",
                          geometry: {
                            rings: [
                              [
                                [0.5, 15.6],
                                [0.7, 13.2],
                                [1.2, 10.9],
                                [2.1, 8.7],
                                [3.4, 6.7],
                                [4.9, 4.9],
                                [6.7, 3.4],
                                [8.7, 2.1],
                                [10.9, 1.2],
                                [13.2, 0.7],
                                [15.6, 0.5],
                                [18, 0.7],
                                [20.3, 1.2],
                                [22.5, 2.1],
                                [24.5, 3.4],
                                [26.3, 4.9],
                                [27.8, 6.7],
                                [29.1, 8.7],
                                [30, 10.9],
                                [30.5, 13.2],
                                [30.7, 15.6],
                                [30.5, 18],
                                [30, 20.3],
                                [29.1, 22.5],
                                [27.8, 24.5],
                                [26.3, 26.3],
                                [24.5, 27.8],
                                [22.5, 29.1],
                                [20.3, 30],
                                [18, 30.5],
                                [15.6, 30.7],
                                [15.6, 30.7],
                                [13.2, 30.5],
                                [10.9, 30],
                                [8.7, 29.1],
                                [6.7, 27.8],
                                [4.9, 26.3],
                                [3.4, 24.5],
                                [2.1, 22.5],
                                [1.2, 20.3],
                                [0.7, 18],
                                [0.5, 15.6],
                              ],
                            ],
                          },
                          symbol: {
                            type: "CIMPolygonSymbol",
                            symbolLayers: [
                              {
                                type: "CIMSolidStroke",
                                enable: false,
                                capStyle: "Butt",
                                joinStyle: "Miter",
                                lineStyle3D: "Strip",
                                miterLimit: 10,
                                width: 1,
                                color: [0, 0, 0, 255],
                              },
                            ],
                          },
                        },
                        {
                          type: "CIMMarkerGraphic",
                          geometry: {
                            rings: [
                              [
                                [17.6, 5.6],
                                [17.6, 18.8],
                                [23.6, 12.5],
                                [23.6, 17.6],
                                [15.6, 25.6],
                                [7.6, 17.6],
                                [7.6, 12.5],
                                [13.6, 18.7],
                                [13.6, 5.6],
                                [17.6, 5.6],
                              ],
                            ],
                          },
                          symbol: {
                            type: "CIMPolygonSymbol",
                            symbolLayers: [
                              {
                                type: "CIMSolidFill",
                                enable: false,
                                colorLocked: true,
                                color: [0, 0, 0, 255],
                              },
                            ],
                          },
                        },
                      ],
                      scaleSymbolsProportionally: true,
                      respectFrame: true,
                      clippingPath: {
                        type: "CIMClippingPath",
                        clippingType: "Intersect",
                        path: {
                          rings: [
                            [
                              [0, 0],
                              [31.2, 0],
                              [31.2, 31.2],
                              [0, 31.2],
                              [0, 0],
                            ],
                          ],
                        },
                      },
                      rotation: 0,
                    },
                  ],
                  haloSize: 1,
                  scaleX: 1,
                  angleAlignment: "Display",
                  angle: 0,
                },
              },
              value: "b",
            },
            {
              label: "Increase",
              symbol: {
                type: "CIMSymbolReference",
                symbol: {
                  type: "CIMPointSymbol",
                  symbolLayers: [
                    {
                      type: "CIMVectorMarker",
                      enable: true,
                      colorLocked: true,
                      anchorPointUnits: "Relative",
                      dominantSizeAxis3D: "Y",
                      size: 9.69,
                      billboardMode3D: "FaceNearPlane",
                      frame: {
                        xmin: 0,
                        ymin: 0,
                        xmax: 31.2,
                        ymax: 31.2,
                      },
                      markerGraphics: [
                        {
                          type: "CIMMarkerGraphic",
                          geometry: {
                            rings: [
                              [
                                [15.6, 0.5],
                                [17, 0.6],
                                [17.6, 0.6],
                                [18.3, 0.8],
                                [19.1, 0.9],
                                [19.8, 1.1],
                                [20.5, 1.3],
                                [21.2, 1.6],
                                [21.9, 1.9],
                                [22.5, 2.2],
                                [23.2, 2.6],
                                [23.9, 3],
                                [24.5, 3.4],
                                [25.1, 3.9],
                                [25.7, 4.4],
                                [26.3, 4.9],
                                [26.8, 5.5],
                                [27.3, 6.1],
                                [27.8, 6.7],
                                [28.2, 7.3],
                                [28.6, 8],
                                [29, 8.7],
                                [29.3, 9.3],
                                [29.6, 10],
                                [29.9, 10.7],
                                [30.1, 11.4],
                                [30.3, 12.1],
                                [30.4, 12.9],
                                [30.6, 13.6],
                                [30.6, 14.2],
                                [30.7, 14.9],
                                [30.7, 15.6],
                                [30.5, 18],
                                [30, 20.3],
                                [29.1, 22.5],
                                [27.8, 24.5],
                                [26.3, 26.3],
                                [24.5, 27.8],
                                [22.5, 29.1],
                                [20.3, 30],
                                [18, 30.5],
                                [15.6, 30.7],
                                [12.7, 30.4],
                                [10, 29.6],
                                [7.4, 28.3],
                                [5.2, 26.5],
                                [3.3, 24.3],
                                [1.8, 21.9],
                                [0.9, 19.1],
                                [0.5, 16.3],
                                [0.5, 15.6],
                                [0.7, 13.2],
                                [1.2, 10.9],
                                [2.1, 8.7],
                                [3.4, 6.7],
                                [4.9, 4.9],
                                [6.7, 3.4],
                                [8.7, 2.1],
                                [10.9, 1.2],
                                [13.2, 0.7],
                                [15.6, 0.5],
                              ],
                            ],
                          },
                          symbol: {
                            type: "CIMPolygonSymbol",
                            symbolLayers: [
                              {
                                type: "CIMSolidFill",
                                enable: false,
                                color: [255, 255, 255, 255],
                              },
                            ],
                          },
                        },
                        {
                          type: "CIMMarkerGraphic",
                          geometry: {
                            rings: [
                              [
                                [0.5, 15.6],
                                [0.7, 13.2],
                                [1.2, 10.9],
                                [2.1, 8.7],
                                [3.4, 6.7],
                                [4.9, 4.9],
                                [6.7, 3.4],
                                [8.7, 2.1],
                                [10.9, 1.2],
                                [13.2, 0.7],
                                [15.6, 0.5],
                                [18, 0.7],
                                [20.3, 1.2],
                                [22.5, 2.1],
                                [24.5, 3.4],
                                [26.3, 4.9],
                                [27.8, 6.7],
                                [29.1, 8.7],
                                [30, 10.9],
                                [30.5, 13.2],
                                [30.7, 15.6],
                                [30.5, 18],
                                [30, 20.3],
                                [29.1, 22.5],
                                [27.8, 24.5],
                                [26.3, 26.3],
                                [24.5, 27.8],
                                [22.5, 29.1],
                                [20.3, 30],
                                [18, 30.5],
                                [15.6, 30.7],
                                [15.6, 30.7],
                                [13.2, 30.5],
                                [10.9, 30],
                                [8.7, 29.1],
                                [6.7, 27.8],
                                [4.9, 26.3],
                                [3.4, 24.5],
                                [2.1, 22.5],
                                [1.2, 20.3],
                                [0.7, 18],
                                [0.5, 15.6],
                              ],
                            ],
                          },
                          symbol: {
                            type: "CIMPolygonSymbol",
                            symbolLayers: [
                              {
                                type: "CIMSolidStroke",
                                enable: true,
                                capStyle: "Butt",
                                joinStyle: "Miter",
                                lineStyle3D: "Strip",
                                miterLimit: 10,
                                width: 1,
                                color: [255, 255, 255, 255],
                              },
                            ],
                          },
                        },
                        {
                          type: "CIMMarkerGraphic",
                          geometry: {
                            rings: [
                              [
                                [17.6, 5.6],
                                [17.6, 18.8],
                                [23.6, 12.5],
                                [23.6, 17.6],
                                [15.6, 25.6],
                                [7.6, 17.6],
                                [7.6, 12.5],
                                [13.6, 18.7],
                                [13.6, 5.6],
                                [17.6, 5.6],
                              ],
                            ],
                          },
                          symbol: {
                            type: "CIMPolygonSymbol",
                            symbolLayers: [
                              {
                                type: "CIMSolidFill",
                                enable: true,
                                colorLocked: true,
                                color: [255, 255, 255, 255],
                              },
                            ],
                          },
                        },
                      ],
                      scaleSymbolsProportionally: true,
                      respectFrame: true,
                      clippingPath: {
                        type: "CIMClippingPath",
                        clippingType: "Intersect",
                        path: {
                          rings: [
                            [
                              [0, 0],
                              [31.2, 0],
                              [31.2, 31.2],
                              [0, 31.2],
                              [0, 0],
                            ],
                          ],
                        },
                      },
                      rotation: 0,
                    },
                    {
                      type: "CIMVectorMarker",
                      enable: true,
                      anchorPointUnits: "Relative",
                      dominantSizeAxis3D: "Y",
                      size: 9.69,
                      billboardMode3D: "FaceNearPlane",
                      frame: {
                        xmin: 0,
                        ymin: 0,
                        xmax: 31.2,
                        ymax: 31.2,
                      },
                      markerGraphics: [
                        {
                          type: "CIMMarkerGraphic",
                          geometry: {
                            rings: [
                              [
                                [15.6, 0.5],
                                [17, 0.6],
                                [17.6, 0.6],
                                [18.3, 0.8],
                                [19.1, 0.9],
                                [19.8, 1.1],
                                [20.5, 1.3],
                                [21.2, 1.6],
                                [21.9, 1.9],
                                [22.5, 2.2],
                                [23.2, 2.6],
                                [23.9, 3],
                                [24.5, 3.4],
                                [25.1, 3.9],
                                [25.7, 4.4],
                                [26.3, 4.9],
                                [26.8, 5.5],
                                [27.3, 6.1],
                                [27.8, 6.7],
                                [28.2, 7.3],
                                [28.6, 8],
                                [29, 8.7],
                                [29.3, 9.3],
                                [29.6, 10],
                                [29.9, 10.7],
                                [30.1, 11.4],
                                [30.3, 12.1],
                                [30.4, 12.9],
                                [30.6, 13.6],
                                [30.6, 14.2],
                                [30.7, 14.9],
                                [30.7, 15.6],
                                [30.5, 18],
                                [30, 20.3],
                                [29.1, 22.5],
                                [27.8, 24.5],
                                [26.3, 26.3],
                                [24.5, 27.8],
                                [22.5, 29.1],
                                [20.3, 30],
                                [18, 30.5],
                                [15.6, 30.7],
                                [12.7, 30.4],
                                [10, 29.6],
                                [7.4, 28.3],
                                [5.2, 26.5],
                                [3.3, 24.3],
                                [1.8, 21.9],
                                [0.9, 19.1],
                                [0.5, 16.3],
                                [0.5, 15.6],
                                [0.7, 13.2],
                                [1.2, 10.9],
                                [2.1, 8.7],
                                [3.4, 6.7],
                                [4.9, 4.9],
                                [6.7, 3.4],
                                [8.7, 2.1],
                                [10.9, 1.2],
                                [13.2, 0.7],
                                [15.6, 0.5],
                              ],
                            ],
                          },
                          symbol: {
                            type: "CIMPolygonSymbol",
                            symbolLayers: [
                              {
                                type: "CIMSolidFill",
                                enable: true,
                                color: [65, 75, 140, 255],
                              },
                            ],
                          },
                        },
                        {
                          type: "CIMMarkerGraphic",
                          geometry: {
                            rings: [
                              [
                                [0.5, 15.6],
                                [0.7, 13.2],
                                [1.2, 10.9],
                                [2.1, 8.7],
                                [3.4, 6.7],
                                [4.9, 4.9],
                                [6.7, 3.4],
                                [8.7, 2.1],
                                [10.9, 1.2],
                                [13.2, 0.7],
                                [15.6, 0.5],
                                [18, 0.7],
                                [20.3, 1.2],
                                [22.5, 2.1],
                                [24.5, 3.4],
                                [26.3, 4.9],
                                [27.8, 6.7],
                                [29.1, 8.7],
                                [30, 10.9],
                                [30.5, 13.2],
                                [30.7, 15.6],
                                [30.5, 18],
                                [30, 20.3],
                                [29.1, 22.5],
                                [27.8, 24.5],
                                [26.3, 26.3],
                                [24.5, 27.8],
                                [22.5, 29.1],
                                [20.3, 30],
                                [18, 30.5],
                                [15.6, 30.7],
                                [15.6, 30.7],
                                [13.2, 30.5],
                                [10.9, 30],
                                [8.7, 29.1],
                                [6.7, 27.8],
                                [4.9, 26.3],
                                [3.4, 24.5],
                                [2.1, 22.5],
                                [1.2, 20.3],
                                [0.7, 18],
                                [0.5, 15.6],
                              ],
                            ],
                          },
                          symbol: {
                            type: "CIMPolygonSymbol",
                            symbolLayers: [
                              {
                                type: "CIMSolidStroke",
                                enable: false,
                                capStyle: "Butt",
                                joinStyle: "Miter",
                                lineStyle3D: "Strip",
                                miterLimit: 10,
                                width: 1,
                                color: [0, 0, 0, 255],
                              },
                            ],
                          },
                        },
                        {
                          type: "CIMMarkerGraphic",
                          geometry: {
                            rings: [
                              [
                                [17.6, 5.6],
                                [17.6, 18.8],
                                [23.6, 12.5],
                                [23.6, 17.6],
                                [15.6, 25.6],
                                [7.6, 17.6],
                                [7.6, 12.5],
                                [13.6, 18.7],
                                [13.6, 5.6],
                                [17.6, 5.6],
                              ],
                            ],
                          },
                          symbol: {
                            type: "CIMPolygonSymbol",
                            symbolLayers: [
                              {
                                type: "CIMSolidFill",
                                enable: false,
                                colorLocked: true,
                                color: [0, 0, 0, 255],
                              },
                            ],
                          },
                        },
                      ],
                      scaleSymbolsProportionally: true,
                      respectFrame: true,
                      clippingPath: {
                        type: "CIMClippingPath",
                        clippingType: "Intersect",
                        path: {
                          rings: [
                            [
                              [0, 0],
                              [31.2, 0],
                              [31.2, 31.2],
                              [0, 31.2],
                              [0, 0],
                            ],
                          ],
                        },
                      },
                      rotation: 0,
                    },
                  ],
                  haloSize: 1,
                  scaleX: 1,
                  angleAlignment: "Display",
                  angle: 0,
                },
              },
              value: "i",
            },
            {
              label: "Decrease",
              symbol: {
                type: "CIMSymbolReference",
                symbol: {
                  type: "CIMPointSymbol",
                  symbolLayers: [
                    {
                      type: "CIMVectorMarker",
                      enable: true,
                      colorLocked: true,
                      anchorPointUnits: "Relative",
                      dominantSizeAxis3D: "Y",
                      size: 9.69,
                      billboardMode3D: "FaceNearPlane",
                      frame: {
                        xmin: 0,
                        ymin: 0,
                        xmax: 31.2,
                        ymax: 31.2,
                      },
                      markerGraphics: [
                        {
                          type: "CIMMarkerGraphic",
                          geometry: {
                            rings: [
                              [
                                [15.6, 0.5],
                                [17, 0.6],
                                [17.6, 0.6],
                                [18.3, 0.8],
                                [19.1, 0.9],
                                [19.8, 1.1],
                                [20.5, 1.3],
                                [21.2, 1.6],
                                [21.9, 1.9],
                                [22.5, 2.2],
                                [23.2, 2.6],
                                [23.9, 3],
                                [24.5, 3.4],
                                [25.1, 3.9],
                                [25.7, 4.4],
                                [26.3, 4.9],
                                [26.8, 5.5],
                                [27.3, 6.1],
                                [27.8, 6.7],
                                [28.2, 7.3],
                                [28.6, 8],
                                [29, 8.7],
                                [29.3, 9.3],
                                [29.6, 10],
                                [29.9, 10.7],
                                [30.1, 11.4],
                                [30.3, 12.1],
                                [30.4, 12.9],
                                [30.6, 13.6],
                                [30.6, 14.2],
                                [30.7, 14.9],
                                [30.7, 15.6],
                                [30.5, 18],
                                [30, 20.3],
                                [29.1, 22.5],
                                [27.8, 24.5],
                                [26.3, 26.3],
                                [24.5, 27.8],
                                [22.5, 29.1],
                                [20.3, 30],
                                [18, 30.5],
                                [15.6, 30.7],
                                [12.7, 30.4],
                                [10, 29.6],
                                [7.4, 28.3],
                                [5.2, 26.5],
                                [3.3, 24.3],
                                [1.8, 21.9],
                                [0.9, 19.1],
                                [0.5, 16.3],
                                [0.5, 15.6],
                                [0.7, 13.2],
                                [1.2, 10.9],
                                [2.1, 8.7],
                                [3.4, 6.7],
                                [4.9, 4.9],
                                [6.7, 3.4],
                                [8.7, 2.1],
                                [10.9, 1.2],
                                [13.2, 0.7],
                                [15.6, 0.5],
                              ],
                            ],
                          },
                          symbol: {
                            type: "CIMPolygonSymbol",
                            symbolLayers: [
                              {
                                type: "CIMSolidFill",
                                enable: false,
                                color: [255, 255, 255, 255],
                              },
                            ],
                          },
                        },
                        {
                          type: "CIMMarkerGraphic",
                          geometry: {
                            rings: [
                              [
                                [0.5, 15.6],
                                [0.7, 13.2],
                                [1.2, 10.9],
                                [2.1, 8.7],
                                [3.4, 6.7],
                                [4.9, 4.9],
                                [6.7, 3.4],
                                [8.7, 2.1],
                                [10.9, 1.2],
                                [13.2, 0.7],
                                [15.6, 0.5],
                                [18, 0.7],
                                [20.3, 1.2],
                                [22.5, 2.1],
                                [24.5, 3.4],
                                [26.3, 4.9],
                                [27.8, 6.7],
                                [29.1, 8.7],
                                [30, 10.9],
                                [30.5, 13.2],
                                [30.7, 15.6],
                                [30.5, 18],
                                [30, 20.3],
                                [29.1, 22.5],
                                [27.8, 24.5],
                                [26.3, 26.3],
                                [24.5, 27.8],
                                [22.5, 29.1],
                                [20.3, 30],
                                [18, 30.5],
                                [15.6, 30.7],
                                [15.6, 30.7],
                                [13.2, 30.5],
                                [10.9, 30],
                                [8.7, 29.1],
                                [6.7, 27.8],
                                [4.9, 26.3],
                                [3.4, 24.5],
                                [2.1, 22.5],
                                [1.2, 20.3],
                                [0.7, 18],
                                [0.5, 15.6],
                              ],
                            ],
                          },
                          symbol: {
                            type: "CIMPolygonSymbol",
                            symbolLayers: [
                              {
                                type: "CIMSolidStroke",
                                enable: true,
                                capStyle: "Butt",
                                joinStyle: "Miter",
                                lineStyle3D: "Strip",
                                miterLimit: 10,
                                width: 1,
                                color: [255, 255, 255, 255],
                              },
                            ],
                          },
                        },
                        {
                          type: "CIMMarkerGraphic",
                          geometry: {
                            rings: [
                              [
                                [17.6, 5.6],
                                [17.6, 18.8],
                                [23.6, 12.5],
                                [23.6, 17.6],
                                [15.6, 25.6],
                                [7.6, 17.6],
                                [7.6, 12.5],
                                [13.6, 18.7],
                                [13.6, 5.6],
                                [17.6, 5.6],
                              ],
                            ],
                          },
                          symbol: {
                            type: "CIMPolygonSymbol",
                            symbolLayers: [
                              {
                                type: "CIMSolidFill",
                                enable: true,
                                colorLocked: true,
                                color: [255, 255, 255, 255],
                              },
                            ],
                          },
                        },
                      ],
                      scaleSymbolsProportionally: true,
                      respectFrame: true,
                      clippingPath: {
                        type: "CIMClippingPath",
                        clippingType: "Intersect",
                        path: {
                          rings: [
                            [
                              [0, 0],
                              [31.2, 0],
                              [31.2, 31.2],
                              [0, 31.2],
                              [0, 0],
                            ],
                          ],
                        },
                      },
                      rotation: 180,
                    },
                    {
                      type: "CIMVectorMarker",
                      enable: true,
                      anchorPointUnits: "Relative",
                      dominantSizeAxis3D: "Y",
                      size: 9.69,
                      billboardMode3D: "FaceNearPlane",
                      frame: {
                        xmin: 0,
                        ymin: 0,
                        xmax: 31.2,
                        ymax: 31.2,
                      },
                      markerGraphics: [
                        {
                          type: "CIMMarkerGraphic",
                          geometry: {
                            rings: [
                              [
                                [15.6, 0.5],
                                [17, 0.6],
                                [17.6, 0.6],
                                [18.3, 0.8],
                                [19.1, 0.9],
                                [19.8, 1.1],
                                [20.5, 1.3],
                                [21.2, 1.6],
                                [21.9, 1.9],
                                [22.5, 2.2],
                                [23.2, 2.6],
                                [23.9, 3],
                                [24.5, 3.4],
                                [25.1, 3.9],
                                [25.7, 4.4],
                                [26.3, 4.9],
                                [26.8, 5.5],
                                [27.3, 6.1],
                                [27.8, 6.7],
                                [28.2, 7.3],
                                [28.6, 8],
                                [29, 8.7],
                                [29.3, 9.3],
                                [29.6, 10],
                                [29.9, 10.7],
                                [30.1, 11.4],
                                [30.3, 12.1],
                                [30.4, 12.9],
                                [30.6, 13.6],
                                [30.6, 14.2],
                                [30.7, 14.9],
                                [30.7, 15.6],
                                [30.5, 18],
                                [30, 20.3],
                                [29.1, 22.5],
                                [27.8, 24.5],
                                [26.3, 26.3],
                                [24.5, 27.8],
                                [22.5, 29.1],
                                [20.3, 30],
                                [18, 30.5],
                                [15.6, 30.7],
                                [12.7, 30.4],
                                [10, 29.6],
                                [7.4, 28.3],
                                [5.2, 26.5],
                                [3.3, 24.3],
                                [1.8, 21.9],
                                [0.9, 19.1],
                                [0.5, 16.3],
                                [0.5, 15.6],
                                [0.7, 13.2],
                                [1.2, 10.9],
                                [2.1, 8.7],
                                [3.4, 6.7],
                                [4.9, 4.9],
                                [6.7, 3.4],
                                [8.7, 2.1],
                                [10.9, 1.2],
                                [13.2, 0.7],
                                [15.6, 0.5],
                              ],
                            ],
                          },
                          symbol: {
                            type: "CIMPolygonSymbol",
                            symbolLayers: [
                              {
                                type: "CIMSolidFill",
                                enable: true,
                                color: [153, 144, 92, 255],
                              },
                            ],
                          },
                        },
                        {
                          type: "CIMMarkerGraphic",
                          geometry: {
                            rings: [
                              [
                                [0.5, 15.6],
                                [0.7, 13.2],
                                [1.2, 10.9],
                                [2.1, 8.7],
                                [3.4, 6.7],
                                [4.9, 4.9],
                                [6.7, 3.4],
                                [8.7, 2.1],
                                [10.9, 1.2],
                                [13.2, 0.7],
                                [15.6, 0.5],
                                [18, 0.7],
                                [20.3, 1.2],
                                [22.5, 2.1],
                                [24.5, 3.4],
                                [26.3, 4.9],
                                [27.8, 6.7],
                                [29.1, 8.7],
                                [30, 10.9],
                                [30.5, 13.2],
                                [30.7, 15.6],
                                [30.5, 18],
                                [30, 20.3],
                                [29.1, 22.5],
                                [27.8, 24.5],
                                [26.3, 26.3],
                                [24.5, 27.8],
                                [22.5, 29.1],
                                [20.3, 30],
                                [18, 30.5],
                                [15.6, 30.7],
                                [15.6, 30.7],
                                [13.2, 30.5],
                                [10.9, 30],
                                [8.7, 29.1],
                                [6.7, 27.8],
                                [4.9, 26.3],
                                [3.4, 24.5],
                                [2.1, 22.5],
                                [1.2, 20.3],
                                [0.7, 18],
                                [0.5, 15.6],
                              ],
                            ],
                          },
                          symbol: {
                            type: "CIMPolygonSymbol",
                            symbolLayers: [
                              {
                                type: "CIMSolidStroke",
                                enable: false,
                                capStyle: "Butt",
                                joinStyle: "Miter",
                                lineStyle3D: "Strip",
                                miterLimit: 10,
                                width: 1,
                                color: [0, 0, 0, 255],
                              },
                            ],
                          },
                        },
                        {
                          type: "CIMMarkerGraphic",
                          geometry: {
                            rings: [
                              [
                                [17.6, 5.6],
                                [17.6, 18.8],
                                [23.6, 12.5],
                                [23.6, 17.6],
                                [15.6, 25.6],
                                [7.6, 17.6],
                                [7.6, 12.5],
                                [13.6, 18.7],
                                [13.6, 5.6],
                                [17.6, 5.6],
                              ],
                            ],
                          },
                          symbol: {
                            type: "CIMPolygonSymbol",
                            symbolLayers: [
                              {
                                type: "CIMSolidFill",
                                enable: false,
                                colorLocked: true,
                                color: [0, 0, 0, 255],
                              },
                            ],
                          },
                        },
                      ],
                      scaleSymbolsProportionally: true,
                      respectFrame: true,
                      clippingPath: {
                        type: "CIMClippingPath",
                        clippingType: "Intersect",
                        path: {
                          rings: [
                            [
                              [0, 0],
                              [31.2, 0],
                              [31.2, 31.2],
                              [0, 31.2],
                              [0, 0],
                            ],
                          ],
                        },
                      },
                      rotation: 180,
                    },
                  ],
                  haloSize: 1,
                  scaleX: 1,
                  angleAlignment: "Display",
                  angle: 180,
                },
              },
              value: "d",
            },
            {
              label: "Unknown",
              symbol: {
                type: "CIMSymbolReference",
                symbol: {
                  type: "CIMPointSymbol",
                  symbolLayers: [
                    {
                      type: "CIMVectorMarker",
                      enable: true,
                      colorLocked: true,
                      anchorPointUnits: "Relative",
                      dominantSizeAxis3D: "Y",
                      size: 9.69,
                      billboardMode3D: "FaceNearPlane",
                      frame: {
                        xmin: 0,
                        ymin: 0,
                        xmax: 31.2,
                        ymax: 31.2,
                      },
                      markerGraphics: [
                        {
                          type: "CIMMarkerGraphic",
                          geometry: {
                            rings: [
                              [
                                [15.6, 0.5],
                                [17, 0.6],
                                [17.6, 0.6],
                                [18.3, 0.8],
                                [19.1, 0.9],
                                [19.8, 1.1],
                                [20.5, 1.3],
                                [21.2, 1.6],
                                [21.9, 1.9],
                                [22.5, 2.2],
                                [23.2, 2.6],
                                [23.9, 3],
                                [24.5, 3.4],
                                [25.1, 3.9],
                                [25.7, 4.4],
                                [26.3, 4.9],
                                [26.8, 5.5],
                                [27.3, 6.1],
                                [27.8, 6.7],
                                [28.2, 7.3],
                                [28.6, 8],
                                [29, 8.7],
                                [29.3, 9.3],
                                [29.6, 10],
                                [29.9, 10.7],
                                [30.1, 11.4],
                                [30.3, 12.1],
                                [30.4, 12.9],
                                [30.6, 13.6],
                                [30.6, 14.2],
                                [30.7, 14.9],
                                [30.7, 15.6],
                                [30.5, 18],
                                [30, 20.3],
                                [29.1, 22.5],
                                [27.8, 24.5],
                                [26.3, 26.3],
                                [24.5, 27.8],
                                [22.5, 29.1],
                                [20.3, 30],
                                [18, 30.5],
                                [15.6, 30.7],
                                [12.7, 30.4],
                                [10, 29.6],
                                [7.4, 28.3],
                                [5.2, 26.5],
                                [3.3, 24.3],
                                [1.8, 21.9],
                                [0.9, 19.1],
                                [0.5, 16.3],
                                [0.5, 15.6],
                                [0.7, 13.2],
                                [1.2, 10.9],
                                [2.1, 8.7],
                                [3.4, 6.7],
                                [4.9, 4.9],
                                [6.7, 3.4],
                                [8.7, 2.1],
                                [10.9, 1.2],
                                [13.2, 0.7],
                                [15.6, 0.5],
                              ],
                            ],
                          },
                          symbol: {
                            type: "CIMPolygonSymbol",
                            symbolLayers: [
                              {
                                type: "CIMSolidFill",
                                enable: false,
                                color: [255, 255, 255, 255],
                              },
                            ],
                          },
                        },
                        {
                          type: "CIMMarkerGraphic",
                          geometry: {
                            rings: [
                              [
                                [0.5, 15.6],
                                [0.7, 13.2],
                                [1.2, 10.9],
                                [2.1, 8.7],
                                [3.4, 6.7],
                                [4.9, 4.9],
                                [6.7, 3.4],
                                [8.7, 2.1],
                                [10.9, 1.2],
                                [13.2, 0.7],
                                [15.6, 0.5],
                                [18, 0.7],
                                [20.3, 1.2],
                                [22.5, 2.1],
                                [24.5, 3.4],
                                [26.3, 4.9],
                                [27.8, 6.7],
                                [29.1, 8.7],
                                [30, 10.9],
                                [30.5, 13.2],
                                [30.7, 15.6],
                                [30.5, 18],
                                [30, 20.3],
                                [29.1, 22.5],
                                [27.8, 24.5],
                                [26.3, 26.3],
                                [24.5, 27.8],
                                [22.5, 29.1],
                                [20.3, 30],
                                [18, 30.5],
                                [15.6, 30.7],
                                [15.6, 30.7],
                                [13.2, 30.5],
                                [10.9, 30],
                                [8.7, 29.1],
                                [6.7, 27.8],
                                [4.9, 26.3],
                                [3.4, 24.5],
                                [2.1, 22.5],
                                [1.2, 20.3],
                                [0.7, 18],
                                [0.5, 15.6],
                              ],
                            ],
                          },
                          symbol: {
                            type: "CIMPolygonSymbol",
                            symbolLayers: [
                              {
                                type: "CIMSolidStroke",
                                enable: true,
                                capStyle: "Butt",
                                joinStyle: "Miter",
                                lineStyle3D: "Strip",
                                miterLimit: 10,
                                width: 1,
                                color: [255, 255, 255, 255],
                              },
                            ],
                          },
                        },
                        {
                          type: "CIMMarkerGraphic",
                          geometry: {
                            rings: [
                              [
                                [17.6, 5.6],
                                [17.6, 18.8],
                                [23.6, 12.5],
                                [23.6, 17.6],
                                [15.6, 25.6],
                                [7.6, 17.6],
                                [7.6, 12.5],
                                [13.6, 18.7],
                                [13.6, 5.6],
                                [17.6, 5.6],
                              ],
                            ],
                          },
                          symbol: {
                            type: "CIMPolygonSymbol",
                            symbolLayers: [
                              {
                                type: "CIMSolidFill",
                                enable: true,
                                colorLocked: true,
                                color: [255, 255, 255, 255],
                              },
                            ],
                          },
                        },
                      ],
                      scaleSymbolsProportionally: true,
                      respectFrame: true,
                      clippingPath: {
                        type: "CIMClippingPath",
                        clippingType: "Intersect",
                        path: {
                          rings: [
                            [
                              [0, 0],
                              [31.2, 0],
                              [31.2, 31.2],
                              [0, 31.2],
                              [0, 0],
                            ],
                          ],
                        },
                      },
                      rotation: 0,
                    },
                    {
                      type: "CIMVectorMarker",
                      enable: true,
                      anchorPointUnits: "Relative",
                      dominantSizeAxis3D: "Y",
                      size: 9.69,
                      billboardMode3D: "FaceNearPlane",
                      frame: {
                        xmin: 0,
                        ymin: 0,
                        xmax: 31.2,
                        ymax: 31.2,
                      },
                      markerGraphics: [
                        {
                          type: "CIMMarkerGraphic",
                          geometry: {
                            rings: [
                              [
                                [15.6, 0.5],
                                [17, 0.6],
                                [17.6, 0.6],
                                [18.3, 0.8],
                                [19.1, 0.9],
                                [19.8, 1.1],
                                [20.5, 1.3],
                                [21.2, 1.6],
                                [21.9, 1.9],
                                [22.5, 2.2],
                                [23.2, 2.6],
                                [23.9, 3],
                                [24.5, 3.4],
                                [25.1, 3.9],
                                [25.7, 4.4],
                                [26.3, 4.9],
                                [26.8, 5.5],
                                [27.3, 6.1],
                                [27.8, 6.7],
                                [28.2, 7.3],
                                [28.6, 8],
                                [29, 8.7],
                                [29.3, 9.3],
                                [29.6, 10],
                                [29.9, 10.7],
                                [30.1, 11.4],
                                [30.3, 12.1],
                                [30.4, 12.9],
                                [30.6, 13.6],
                                [30.6, 14.2],
                                [30.7, 14.9],
                                [30.7, 15.6],
                                [30.5, 18],
                                [30, 20.3],
                                [29.1, 22.5],
                                [27.8, 24.5],
                                [26.3, 26.3],
                                [24.5, 27.8],
                                [22.5, 29.1],
                                [20.3, 30],
                                [18, 30.5],
                                [15.6, 30.7],
                                [12.7, 30.4],
                                [10, 29.6],
                                [7.4, 28.3],
                                [5.2, 26.5],
                                [3.3, 24.3],
                                [1.8, 21.9],
                                [0.9, 19.1],
                                [0.5, 16.3],
                                [0.5, 15.6],
                                [0.7, 13.2],
                                [1.2, 10.9],
                                [2.1, 8.7],
                                [3.4, 6.7],
                                [4.9, 4.9],
                                [6.7, 3.4],
                                [8.7, 2.1],
                                [10.9, 1.2],
                                [13.2, 0.7],
                                [15.6, 0.5],
                              ],
                            ],
                          },
                          symbol: {
                            type: "CIMPolygonSymbol",
                            symbolLayers: [
                              {
                                type: "CIMSolidFill",
                                enable: true,
                                color: [137, 166, 166, 255],
                              },
                            ],
                          },
                        },
                        {
                          type: "CIMMarkerGraphic",
                          geometry: {
                            rings: [
                              [
                                [0.5, 15.6],
                                [0.7, 13.2],
                                [1.2, 10.9],
                                [2.1, 8.7],
                                [3.4, 6.7],
                                [4.9, 4.9],
                                [6.7, 3.4],
                                [8.7, 2.1],
                                [10.9, 1.2],
                                [13.2, 0.7],
                                [15.6, 0.5],
                                [18, 0.7],
                                [20.3, 1.2],
                                [22.5, 2.1],
                                [24.5, 3.4],
                                [26.3, 4.9],
                                [27.8, 6.7],
                                [29.1, 8.7],
                                [30, 10.9],
                                [30.5, 13.2],
                                [30.7, 15.6],
                                [30.5, 18],
                                [30, 20.3],
                                [29.1, 22.5],
                                [27.8, 24.5],
                                [26.3, 26.3],
                                [24.5, 27.8],
                                [22.5, 29.1],
                                [20.3, 30],
                                [18, 30.5],
                                [15.6, 30.7],
                                [15.6, 30.7],
                                [13.2, 30.5],
                                [10.9, 30],
                                [8.7, 29.1],
                                [6.7, 27.8],
                                [4.9, 26.3],
                                [3.4, 24.5],
                                [2.1, 22.5],
                                [1.2, 20.3],
                                [0.7, 18],
                                [0.5, 15.6],
                              ],
                            ],
                          },
                          symbol: {
                            type: "CIMPolygonSymbol",
                            symbolLayers: [
                              {
                                type: "CIMSolidStroke",
                                enable: false,
                                capStyle: "Butt",
                                joinStyle: "Miter",
                                lineStyle3D: "Strip",
                                miterLimit: 10,
                                width: 1,
                                color: [0, 0, 0, 255],
                              },
                            ],
                          },
                        },
                        {
                          type: "CIMMarkerGraphic",
                          geometry: {
                            rings: [
                              [
                                [17.6, 5.6],
                                [17.6, 18.8],
                                [23.6, 12.5],
                                [23.6, 17.6],
                                [15.6, 25.6],
                                [7.6, 17.6],
                                [7.6, 12.5],
                                [13.6, 18.7],
                                [13.6, 5.6],
                                [17.6, 5.6],
                              ],
                            ],
                          },
                          symbol: {
                            type: "CIMPolygonSymbol",
                            symbolLayers: [
                              {
                                type: "CIMSolidFill",
                                enable: false,
                                colorLocked: true,
                                color: [0, 0, 0, 255],
                              },
                            ],
                          },
                        },
                      ],
                      scaleSymbolsProportionally: true,
                      respectFrame: true,
                      clippingPath: {
                        type: "CIMClippingPath",
                        clippingType: "Intersect",
                        path: {
                          rings: [
                            [
                              [0, 0],
                              [31.2, 0],
                              [31.2, 31.2],
                              [0, 31.2],
                              [0, 0],
                            ],
                          ],
                        },
                      },
                      rotation: 0,
                    },
                  ],
                  haloSize: 1,
                  scaleX: 1,
                  angleAlignment: "Display",
                  angle: 0,
                },
              },
              value: "u",
            },
          ],
        },
        labelingInfo: [
          {
            deconflictionStrategy: "none",
            labelExpressionInfo: {
              expression:
                '\n  $feature["cluster_count"];\n  var value = $feature["cluster_count"];\n  var num = Count(Text(Round(value)));\n  var label = When(\n    num < 4, Text(value, "#.#"),\n    num == 4, Text(value / Pow(10, 3), "#.0k"),\n    num <= 6, Text(value / Pow(10, 3), "#k"),\n    num == 7, Text(value / Pow(10, 6), "#.0m"),\n    num > 7, Text(value / Pow(10, 6), "#m"),\n    Text(value, "#,###")\n  );\n  return label;\n  ',
            },
            labelPlacement: "esriServerPointLabelPlacementCenterCenter",
            repeatLabel: true,
            symbol: {
              type: "esriTS",
              color: [240, 240, 240, 255],
              font: {
                family: "Noto Sans",
                size: 9,
                weight: "bold",
              },
              horizontalAlignment: "center",
              kerning: true,
              haloColor: [55, 56, 55, 255],
              haloSize: 0.75,
              rotated: false,
              text: "",
              verticalAlignment: "baseline",
              xoffset: 0,
              yoffset: 0,
              angle: 0,
            },
          },
        ],
      },
      showLabels: true,
      fields: [
        {
          isAutoGenerated: true,
          name: "cluster_count",
          statisticType: "count",
        },
        {
          isAutoGenerated: true,
          name: "cluster_type_Direction",
          onStatisticField: "Direction",
          statisticType: "mode",
        },
      ],
    },
    definitionExpression: null,
    drawingInfo: {
      labelingInfo: [
        {
          labelExpressionInfo: {
            expression:
              '/**\r\n * Creates the label text for an SRMP point.\r\n * - Rounds SRMP to three digits.\r\n * - Appends an "S" for spurs (RRT = "SP")\r\n * - Appends a "B" for back mileage\r\n */\r\nfunction createSrmpLabel() {\r\n  var label = Text($feature.SRMP, "####.###");\r\n\r\n  if ($feature.RelRouteType == "SP") {\r\n    label += Left($feature.RelRouteType, 1)\r\n  }\r\n\r\n  if (Upper($feature.AheadBackInd) == "B") {\r\n    label += "B";\r\n  }\r\n\r\n  return label;\r\n}\r\n\r\ncreateSrmpLabel();',
            title: "SRMP Label",
          },
          labelPlacement: "esriServerPointLabelPlacementAboveCenter",
          maxScale: 0,
          minScale: 623.699391,
          repeatLabel: true,
          symbol: {
            type: "esriTS",
            color: [255, 255, 255, 255],
            font: {
              family: "Arial",
              size: 9.75,
            },
            horizontalAlignment: "center",
            kerning: true,
            haloColor: [0, 123, 95, 255],
            haloSize: 1.5,
            rotated: false,
            text: "",
            verticalAlignment: "baseline",
            xoffset: 0,
            yoffset: 0,
            angle: 0,
          },
        },
      ],
      renderer: {
        type: "uniqueValue",
        visualVariables: [
          {
            type: "rotationInfo",
            field: "Azimuth",
            rotationType: "geographic",
          },
          {
            type: "sizeInfo",
            valueExpression: "$view.scale",
            stops: [
              {
                size: 24.213477325006632,
                value: 70.5310735,
              },
              {
                size: 19.370781860005305,
                value: 564.248588,
              },
              {
                size: 9.685390930002653,
                value: 4513.988705,
              },
              {
                size: 4.842695465001326,
                value: 36111.909643,
              },
            ],
          },
        ],
        field1: "Direction",
        drawInClassOrder: true,
        uniqueValueGroups: [
          {
            classes: [
              {
                label: "Both",
                symbol: {
                  type: "CIMSymbolReference",
                  symbol: {
                    type: "CIMPointSymbol",
                    symbolLayers: [
                      {
                        type: "CIMVectorMarker",
                        enable: true,
                        colorLocked: true,
                        anchorPointUnits: "Relative",
                        dominantSizeAxis3D: "Y",
                        size: 9.69,
                        billboardMode3D: "FaceNearPlane",
                        frame: {
                          xmin: 0,
                          ymin: 0,
                          xmax: 31.2,
                          ymax: 31.2,
                        },
                        markerGraphics: [
                          {
                            type: "CIMMarkerGraphic",
                            geometry: {
                              rings: [
                                [
                                  [15.6, 0.5],
                                  [17, 0.6],
                                  [17.6, 0.6],
                                  [18.3, 0.8],
                                  [19.1, 0.9],
                                  [19.8, 1.1],
                                  [20.5, 1.3],
                                  [21.2, 1.6],
                                  [21.9, 1.9],
                                  [22.5, 2.2],
                                  [23.2, 2.6],
                                  [23.9, 3],
                                  [24.5, 3.4],
                                  [25.1, 3.9],
                                  [25.7, 4.4],
                                  [26.3, 4.9],
                                  [26.8, 5.5],
                                  [27.3, 6.1],
                                  [27.8, 6.7],
                                  [28.2, 7.3],
                                  [28.6, 8],
                                  [29, 8.7],
                                  [29.3, 9.3],
                                  [29.6, 10],
                                  [29.9, 10.7],
                                  [30.1, 11.4],
                                  [30.3, 12.1],
                                  [30.4, 12.9],
                                  [30.6, 13.6],
                                  [30.6, 14.2],
                                  [30.7, 14.9],
                                  [30.7, 15.6],
                                  [30.5, 18],
                                  [30, 20.3],
                                  [29.1, 22.5],
                                  [27.8, 24.5],
                                  [26.3, 26.3],
                                  [24.5, 27.8],
                                  [22.5, 29.1],
                                  [20.3, 30],
                                  [18, 30.5],
                                  [15.6, 30.7],
                                  [12.7, 30.4],
                                  [10, 29.6],
                                  [7.4, 28.3],
                                  [5.2, 26.5],
                                  [3.3, 24.3],
                                  [1.8, 21.9],
                                  [0.9, 19.1],
                                  [0.5, 16.3],
                                  [0.5, 15.6],
                                  [0.7, 13.2],
                                  [1.2, 10.9],
                                  [2.1, 8.7],
                                  [3.4, 6.7],
                                  [4.9, 4.9],
                                  [6.7, 3.4],
                                  [8.7, 2.1],
                                  [10.9, 1.2],
                                  [13.2, 0.7],
                                  [15.6, 0.5],
                                ],
                              ],
                            },
                            symbol: {
                              type: "CIMPolygonSymbol",
                              symbolLayers: [
                                {
                                  type: "CIMSolidFill",
                                  enable: false,
                                  color: [255, 255, 255, 255],
                                },
                              ],
                            },
                          },
                          {
                            type: "CIMMarkerGraphic",
                            geometry: {
                              rings: [
                                [
                                  [0.5, 15.6],
                                  [0.7, 13.2],
                                  [1.2, 10.9],
                                  [2.1, 8.7],
                                  [3.4, 6.7],
                                  [4.9, 4.9],
                                  [6.7, 3.4],
                                  [8.7, 2.1],
                                  [10.9, 1.2],
                                  [13.2, 0.7],
                                  [15.6, 0.5],
                                  [18, 0.7],
                                  [20.3, 1.2],
                                  [22.5, 2.1],
                                  [24.5, 3.4],
                                  [26.3, 4.9],
                                  [27.8, 6.7],
                                  [29.1, 8.7],
                                  [30, 10.9],
                                  [30.5, 13.2],
                                  [30.7, 15.6],
                                  [30.5, 18],
                                  [30, 20.3],
                                  [29.1, 22.5],
                                  [27.8, 24.5],
                                  [26.3, 26.3],
                                  [24.5, 27.8],
                                  [22.5, 29.1],
                                  [20.3, 30],
                                  [18, 30.5],
                                  [15.6, 30.7],
                                  [15.6, 30.7],
                                  [13.2, 30.5],
                                  [10.9, 30],
                                  [8.7, 29.1],
                                  [6.7, 27.8],
                                  [4.9, 26.3],
                                  [3.4, 24.5],
                                  [2.1, 22.5],
                                  [1.2, 20.3],
                                  [0.7, 18],
                                  [0.5, 15.6],
                                ],
                              ],
                            },
                            symbol: {
                              type: "CIMPolygonSymbol",
                              symbolLayers: [
                                {
                                  type: "CIMSolidStroke",
                                  enable: true,
                                  capStyle: "Butt",
                                  joinStyle: "Miter",
                                  lineStyle3D: "Strip",
                                  miterLimit: 10,
                                  width: 1,
                                  color: [255, 255, 255, 255],
                                },
                              ],
                            },
                          },
                          {
                            type: "CIMMarkerGraphic",
                            geometry: {
                              rings: [
                                [
                                  [17.6, 5.6],
                                  [17.6, 18.8],
                                  [23.6, 12.5],
                                  [23.6, 17.6],
                                  [15.6, 25.6],
                                  [7.6, 17.6],
                                  [7.6, 12.5],
                                  [13.6, 18.7],
                                  [13.6, 5.6],
                                  [17.6, 5.6],
                                ],
                              ],
                            },
                            symbol: {
                              type: "CIMPolygonSymbol",
                              symbolLayers: [
                                {
                                  type: "CIMSolidFill",
                                  enable: true,
                                  colorLocked: true,
                                  color: [255, 255, 255, 255],
                                },
                              ],
                            },
                          },
                        ],
                        scaleSymbolsProportionally: true,
                        respectFrame: true,
                        clippingPath: {
                          type: "CIMClippingPath",
                          clippingType: "Intersect",
                          path: {
                            rings: [
                              [
                                [0, 0],
                                [31.2, 0],
                                [31.2, 31.2],
                                [0, 31.2],
                                [0, 0],
                              ],
                            ],
                          },
                        },
                        rotation: 0,
                      },
                      {
                        type: "CIMVectorMarker",
                        enable: true,
                        anchorPointUnits: "Relative",
                        dominantSizeAxis3D: "Y",
                        size: 9.69,
                        billboardMode3D: "FaceNearPlane",
                        frame: {
                          xmin: 0,
                          ymin: 0,
                          xmax: 31.2,
                          ymax: 31.2,
                        },
                        markerGraphics: [
                          {
                            type: "CIMMarkerGraphic",
                            geometry: {
                              rings: [
                                [
                                  [15.6, 0.5],
                                  [17, 0.6],
                                  [17.6, 0.6],
                                  [18.3, 0.8],
                                  [19.1, 0.9],
                                  [19.8, 1.1],
                                  [20.5, 1.3],
                                  [21.2, 1.6],
                                  [21.9, 1.9],
                                  [22.5, 2.2],
                                  [23.2, 2.6],
                                  [23.9, 3],
                                  [24.5, 3.4],
                                  [25.1, 3.9],
                                  [25.7, 4.4],
                                  [26.3, 4.9],
                                  [26.8, 5.5],
                                  [27.3, 6.1],
                                  [27.8, 6.7],
                                  [28.2, 7.3],
                                  [28.6, 8],
                                  [29, 8.7],
                                  [29.3, 9.3],
                                  [29.6, 10],
                                  [29.9, 10.7],
                                  [30.1, 11.4],
                                  [30.3, 12.1],
                                  [30.4, 12.9],
                                  [30.6, 13.6],
                                  [30.6, 14.2],
                                  [30.7, 14.9],
                                  [30.7, 15.6],
                                  [30.5, 18],
                                  [30, 20.3],
                                  [29.1, 22.5],
                                  [27.8, 24.5],
                                  [26.3, 26.3],
                                  [24.5, 27.8],
                                  [22.5, 29.1],
                                  [20.3, 30],
                                  [18, 30.5],
                                  [15.6, 30.7],
                                  [12.7, 30.4],
                                  [10, 29.6],
                                  [7.4, 28.3],
                                  [5.2, 26.5],
                                  [3.3, 24.3],
                                  [1.8, 21.9],
                                  [0.9, 19.1],
                                  [0.5, 16.3],
                                  [0.5, 15.6],
                                  [0.7, 13.2],
                                  [1.2, 10.9],
                                  [2.1, 8.7],
                                  [3.4, 6.7],
                                  [4.9, 4.9],
                                  [6.7, 3.4],
                                  [8.7, 2.1],
                                  [10.9, 1.2],
                                  [13.2, 0.7],
                                  [15.6, 0.5],
                                ],
                              ],
                            },
                            symbol: {
                              type: "CIMPolygonSymbol",
                              symbolLayers: [
                                {
                                  type: "CIMSolidFill",
                                  enable: true,
                                  color: [0, 0, 0, 255],
                                },
                              ],
                            },
                          },
                          {
                            type: "CIMMarkerGraphic",
                            geometry: {
                              rings: [
                                [
                                  [0.5, 15.6],
                                  [0.7, 13.2],
                                  [1.2, 10.9],
                                  [2.1, 8.7],
                                  [3.4, 6.7],
                                  [4.9, 4.9],
                                  [6.7, 3.4],
                                  [8.7, 2.1],
                                  [10.9, 1.2],
                                  [13.2, 0.7],
                                  [15.6, 0.5],
                                  [18, 0.7],
                                  [20.3, 1.2],
                                  [22.5, 2.1],
                                  [24.5, 3.4],
                                  [26.3, 4.9],
                                  [27.8, 6.7],
                                  [29.1, 8.7],
                                  [30, 10.9],
                                  [30.5, 13.2],
                                  [30.7, 15.6],
                                  [30.5, 18],
                                  [30, 20.3],
                                  [29.1, 22.5],
                                  [27.8, 24.5],
                                  [26.3, 26.3],
                                  [24.5, 27.8],
                                  [22.5, 29.1],
                                  [20.3, 30],
                                  [18, 30.5],
                                  [15.6, 30.7],
                                  [15.6, 30.7],
                                  [13.2, 30.5],
                                  [10.9, 30],
                                  [8.7, 29.1],
                                  [6.7, 27.8],
                                  [4.9, 26.3],
                                  [3.4, 24.5],
                                  [2.1, 22.5],
                                  [1.2, 20.3],
                                  [0.7, 18],
                                  [0.5, 15.6],
                                ],
                              ],
                            },
                            symbol: {
                              type: "CIMPolygonSymbol",
                              symbolLayers: [
                                {
                                  type: "CIMSolidStroke",
                                  enable: false,
                                  capStyle: "Butt",
                                  joinStyle: "Miter",
                                  lineStyle3D: "Strip",
                                  miterLimit: 10,
                                  width: 1,
                                  color: [0, 0, 0, 255],
                                },
                              ],
                            },
                          },
                          {
                            type: "CIMMarkerGraphic",
                            geometry: {
                              rings: [
                                [
                                  [17.6, 5.6],
                                  [17.6, 18.8],
                                  [23.6, 12.5],
                                  [23.6, 17.6],
                                  [15.6, 25.6],
                                  [7.6, 17.6],
                                  [7.6, 12.5],
                                  [13.6, 18.7],
                                  [13.6, 5.6],
                                  [17.6, 5.6],
                                ],
                              ],
                            },
                            symbol: {
                              type: "CIMPolygonSymbol",
                              symbolLayers: [
                                {
                                  type: "CIMSolidFill",
                                  enable: false,
                                  colorLocked: true,
                                  color: [0, 0, 0, 255],
                                },
                              ],
                            },
                          },
                        ],
                        scaleSymbolsProportionally: true,
                        respectFrame: true,
                        clippingPath: {
                          type: "CIMClippingPath",
                          clippingType: "Intersect",
                          path: {
                            rings: [
                              [
                                [0, 0],
                                [31.2, 0],
                                [31.2, 31.2],
                                [0, 31.2],
                                [0, 0],
                              ],
                            ],
                          },
                        },
                        rotation: 0,
                      },
                    ],
                    haloSize: 1,
                    scaleX: 1,
                    angleAlignment: "Display",
                    angle: 0,
                  },
                },
                values: [["b"]],
              },
              {
                label: "Increase",
                symbol: {
                  type: "CIMSymbolReference",
                  symbol: {
                    type: "CIMPointSymbol",
                    symbolLayers: [
                      {
                        type: "CIMVectorMarker",
                        enable: true,
                        colorLocked: true,
                        anchorPointUnits: "Relative",
                        dominantSizeAxis3D: "Y",
                        size: 9.69,
                        billboardMode3D: "FaceNearPlane",
                        frame: {
                          xmin: 0,
                          ymin: 0,
                          xmax: 31.2,
                          ymax: 31.2,
                        },
                        markerGraphics: [
                          {
                            type: "CIMMarkerGraphic",
                            geometry: {
                              rings: [
                                [
                                  [15.6, 0.5],
                                  [17, 0.6],
                                  [17.6, 0.6],
                                  [18.3, 0.8],
                                  [19.1, 0.9],
                                  [19.8, 1.1],
                                  [20.5, 1.3],
                                  [21.2, 1.6],
                                  [21.9, 1.9],
                                  [22.5, 2.2],
                                  [23.2, 2.6],
                                  [23.9, 3],
                                  [24.5, 3.4],
                                  [25.1, 3.9],
                                  [25.7, 4.4],
                                  [26.3, 4.9],
                                  [26.8, 5.5],
                                  [27.3, 6.1],
                                  [27.8, 6.7],
                                  [28.2, 7.3],
                                  [28.6, 8],
                                  [29, 8.7],
                                  [29.3, 9.3],
                                  [29.6, 10],
                                  [29.9, 10.7],
                                  [30.1, 11.4],
                                  [30.3, 12.1],
                                  [30.4, 12.9],
                                  [30.6, 13.6],
                                  [30.6, 14.2],
                                  [30.7, 14.9],
                                  [30.7, 15.6],
                                  [30.5, 18],
                                  [30, 20.3],
                                  [29.1, 22.5],
                                  [27.8, 24.5],
                                  [26.3, 26.3],
                                  [24.5, 27.8],
                                  [22.5, 29.1],
                                  [20.3, 30],
                                  [18, 30.5],
                                  [15.6, 30.7],
                                  [12.7, 30.4],
                                  [10, 29.6],
                                  [7.4, 28.3],
                                  [5.2, 26.5],
                                  [3.3, 24.3],
                                  [1.8, 21.9],
                                  [0.9, 19.1],
                                  [0.5, 16.3],
                                  [0.5, 15.6],
                                  [0.7, 13.2],
                                  [1.2, 10.9],
                                  [2.1, 8.7],
                                  [3.4, 6.7],
                                  [4.9, 4.9],
                                  [6.7, 3.4],
                                  [8.7, 2.1],
                                  [10.9, 1.2],
                                  [13.2, 0.7],
                                  [15.6, 0.5],
                                ],
                              ],
                            },
                            symbol: {
                              type: "CIMPolygonSymbol",
                              symbolLayers: [
                                {
                                  type: "CIMSolidFill",
                                  enable: false,
                                  color: [255, 255, 255, 255],
                                },
                              ],
                            },
                          },
                          {
                            type: "CIMMarkerGraphic",
                            geometry: {
                              rings: [
                                [
                                  [0.5, 15.6],
                                  [0.7, 13.2],
                                  [1.2, 10.9],
                                  [2.1, 8.7],
                                  [3.4, 6.7],
                                  [4.9, 4.9],
                                  [6.7, 3.4],
                                  [8.7, 2.1],
                                  [10.9, 1.2],
                                  [13.2, 0.7],
                                  [15.6, 0.5],
                                  [18, 0.7],
                                  [20.3, 1.2],
                                  [22.5, 2.1],
                                  [24.5, 3.4],
                                  [26.3, 4.9],
                                  [27.8, 6.7],
                                  [29.1, 8.7],
                                  [30, 10.9],
                                  [30.5, 13.2],
                                  [30.7, 15.6],
                                  [30.5, 18],
                                  [30, 20.3],
                                  [29.1, 22.5],
                                  [27.8, 24.5],
                                  [26.3, 26.3],
                                  [24.5, 27.8],
                                  [22.5, 29.1],
                                  [20.3, 30],
                                  [18, 30.5],
                                  [15.6, 30.7],
                                  [15.6, 30.7],
                                  [13.2, 30.5],
                                  [10.9, 30],
                                  [8.7, 29.1],
                                  [6.7, 27.8],
                                  [4.9, 26.3],
                                  [3.4, 24.5],
                                  [2.1, 22.5],
                                  [1.2, 20.3],
                                  [0.7, 18],
                                  [0.5, 15.6],
                                ],
                              ],
                            },
                            symbol: {
                              type: "CIMPolygonSymbol",
                              symbolLayers: [
                                {
                                  type: "CIMSolidStroke",
                                  enable: true,
                                  capStyle: "Butt",
                                  joinStyle: "Miter",
                                  lineStyle3D: "Strip",
                                  miterLimit: 10,
                                  width: 1,
                                  color: [255, 255, 255, 255],
                                },
                              ],
                            },
                          },
                          {
                            type: "CIMMarkerGraphic",
                            geometry: {
                              rings: [
                                [
                                  [17.6, 5.6],
                                  [17.6, 18.8],
                                  [23.6, 12.5],
                                  [23.6, 17.6],
                                  [15.6, 25.6],
                                  [7.6, 17.6],
                                  [7.6, 12.5],
                                  [13.6, 18.7],
                                  [13.6, 5.6],
                                  [17.6, 5.6],
                                ],
                              ],
                            },
                            symbol: {
                              type: "CIMPolygonSymbol",
                              symbolLayers: [
                                {
                                  type: "CIMSolidFill",
                                  enable: true,
                                  colorLocked: true,
                                  color: [255, 255, 255, 255],
                                },
                              ],
                            },
                          },
                        ],
                        scaleSymbolsProportionally: true,
                        respectFrame: true,
                        clippingPath: {
                          type: "CIMClippingPath",
                          clippingType: "Intersect",
                          path: {
                            rings: [
                              [
                                [0, 0],
                                [31.2, 0],
                                [31.2, 31.2],
                                [0, 31.2],
                                [0, 0],
                              ],
                            ],
                          },
                        },
                        rotation: 0,
                      },
                      {
                        type: "CIMVectorMarker",
                        enable: true,
                        anchorPointUnits: "Relative",
                        dominantSizeAxis3D: "Y",
                        size: 9.69,
                        billboardMode3D: "FaceNearPlane",
                        frame: {
                          xmin: 0,
                          ymin: 0,
                          xmax: 31.2,
                          ymax: 31.2,
                        },
                        markerGraphics: [
                          {
                            type: "CIMMarkerGraphic",
                            geometry: {
                              rings: [
                                [
                                  [15.6, 0.5],
                                  [17, 0.6],
                                  [17.6, 0.6],
                                  [18.3, 0.8],
                                  [19.1, 0.9],
                                  [19.8, 1.1],
                                  [20.5, 1.3],
                                  [21.2, 1.6],
                                  [21.9, 1.9],
                                  [22.5, 2.2],
                                  [23.2, 2.6],
                                  [23.9, 3],
                                  [24.5, 3.4],
                                  [25.1, 3.9],
                                  [25.7, 4.4],
                                  [26.3, 4.9],
                                  [26.8, 5.5],
                                  [27.3, 6.1],
                                  [27.8, 6.7],
                                  [28.2, 7.3],
                                  [28.6, 8],
                                  [29, 8.7],
                                  [29.3, 9.3],
                                  [29.6, 10],
                                  [29.9, 10.7],
                                  [30.1, 11.4],
                                  [30.3, 12.1],
                                  [30.4, 12.9],
                                  [30.6, 13.6],
                                  [30.6, 14.2],
                                  [30.7, 14.9],
                                  [30.7, 15.6],
                                  [30.5, 18],
                                  [30, 20.3],
                                  [29.1, 22.5],
                                  [27.8, 24.5],
                                  [26.3, 26.3],
                                  [24.5, 27.8],
                                  [22.5, 29.1],
                                  [20.3, 30],
                                  [18, 30.5],
                                  [15.6, 30.7],
                                  [12.7, 30.4],
                                  [10, 29.6],
                                  [7.4, 28.3],
                                  [5.2, 26.5],
                                  [3.3, 24.3],
                                  [1.8, 21.9],
                                  [0.9, 19.1],
                                  [0.5, 16.3],
                                  [0.5, 15.6],
                                  [0.7, 13.2],
                                  [1.2, 10.9],
                                  [2.1, 8.7],
                                  [3.4, 6.7],
                                  [4.9, 4.9],
                                  [6.7, 3.4],
                                  [8.7, 2.1],
                                  [10.9, 1.2],
                                  [13.2, 0.7],
                                  [15.6, 0.5],
                                ],
                              ],
                            },
                            symbol: {
                              type: "CIMPolygonSymbol",
                              symbolLayers: [
                                {
                                  type: "CIMSolidFill",
                                  enable: true,
                                  color: [65, 75, 140, 255],
                                },
                              ],
                            },
                          },
                          {
                            type: "CIMMarkerGraphic",
                            geometry: {
                              rings: [
                                [
                                  [0.5, 15.6],
                                  [0.7, 13.2],
                                  [1.2, 10.9],
                                  [2.1, 8.7],
                                  [3.4, 6.7],
                                  [4.9, 4.9],
                                  [6.7, 3.4],
                                  [8.7, 2.1],
                                  [10.9, 1.2],
                                  [13.2, 0.7],
                                  [15.6, 0.5],
                                  [18, 0.7],
                                  [20.3, 1.2],
                                  [22.5, 2.1],
                                  [24.5, 3.4],
                                  [26.3, 4.9],
                                  [27.8, 6.7],
                                  [29.1, 8.7],
                                  [30, 10.9],
                                  [30.5, 13.2],
                                  [30.7, 15.6],
                                  [30.5, 18],
                                  [30, 20.3],
                                  [29.1, 22.5],
                                  [27.8, 24.5],
                                  [26.3, 26.3],
                                  [24.5, 27.8],
                                  [22.5, 29.1],
                                  [20.3, 30],
                                  [18, 30.5],
                                  [15.6, 30.7],
                                  [15.6, 30.7],
                                  [13.2, 30.5],
                                  [10.9, 30],
                                  [8.7, 29.1],
                                  [6.7, 27.8],
                                  [4.9, 26.3],
                                  [3.4, 24.5],
                                  [2.1, 22.5],
                                  [1.2, 20.3],
                                  [0.7, 18],
                                  [0.5, 15.6],
                                ],
                              ],
                            },
                            symbol: {
                              type: "CIMPolygonSymbol",
                              symbolLayers: [
                                {
                                  type: "CIMSolidStroke",
                                  enable: false,
                                  capStyle: "Butt",
                                  joinStyle: "Miter",
                                  lineStyle3D: "Strip",
                                  miterLimit: 10,
                                  width: 1,
                                  color: [0, 0, 0, 255],
                                },
                              ],
                            },
                          },
                          {
                            type: "CIMMarkerGraphic",
                            geometry: {
                              rings: [
                                [
                                  [17.6, 5.6],
                                  [17.6, 18.8],
                                  [23.6, 12.5],
                                  [23.6, 17.6],
                                  [15.6, 25.6],
                                  [7.6, 17.6],
                                  [7.6, 12.5],
                                  [13.6, 18.7],
                                  [13.6, 5.6],
                                  [17.6, 5.6],
                                ],
                              ],
                            },
                            symbol: {
                              type: "CIMPolygonSymbol",
                              symbolLayers: [
                                {
                                  type: "CIMSolidFill",
                                  enable: false,
                                  colorLocked: true,
                                  color: [0, 0, 0, 255],
                                },
                              ],
                            },
                          },
                        ],
                        scaleSymbolsProportionally: true,
                        respectFrame: true,
                        clippingPath: {
                          type: "CIMClippingPath",
                          clippingType: "Intersect",
                          path: {
                            rings: [
                              [
                                [0, 0],
                                [31.2, 0],
                                [31.2, 31.2],
                                [0, 31.2],
                                [0, 0],
                              ],
                            ],
                          },
                        },
                        rotation: 0,
                      },
                    ],
                    haloSize: 1,
                    scaleX: 1,
                    angleAlignment: "Display",
                    angle: 0,
                  },
                },
                values: [["i"]],
              },
              {
                label: "Decrease",
                symbol: {
                  type: "CIMSymbolReference",
                  symbol: {
                    type: "CIMPointSymbol",
                    symbolLayers: [
                      {
                        type: "CIMVectorMarker",
                        enable: true,
                        colorLocked: true,
                        anchorPointUnits: "Relative",
                        dominantSizeAxis3D: "Y",
                        size: 9.69,
                        billboardMode3D: "FaceNearPlane",
                        frame: {
                          xmin: 0,
                          ymin: 0,
                          xmax: 31.2,
                          ymax: 31.2,
                        },
                        markerGraphics: [
                          {
                            type: "CIMMarkerGraphic",
                            geometry: {
                              rings: [
                                [
                                  [15.6, 0.5],
                                  [17, 0.6],
                                  [17.6, 0.6],
                                  [18.3, 0.8],
                                  [19.1, 0.9],
                                  [19.8, 1.1],
                                  [20.5, 1.3],
                                  [21.2, 1.6],
                                  [21.9, 1.9],
                                  [22.5, 2.2],
                                  [23.2, 2.6],
                                  [23.9, 3],
                                  [24.5, 3.4],
                                  [25.1, 3.9],
                                  [25.7, 4.4],
                                  [26.3, 4.9],
                                  [26.8, 5.5],
                                  [27.3, 6.1],
                                  [27.8, 6.7],
                                  [28.2, 7.3],
                                  [28.6, 8],
                                  [29, 8.7],
                                  [29.3, 9.3],
                                  [29.6, 10],
                                  [29.9, 10.7],
                                  [30.1, 11.4],
                                  [30.3, 12.1],
                                  [30.4, 12.9],
                                  [30.6, 13.6],
                                  [30.6, 14.2],
                                  [30.7, 14.9],
                                  [30.7, 15.6],
                                  [30.5, 18],
                                  [30, 20.3],
                                  [29.1, 22.5],
                                  [27.8, 24.5],
                                  [26.3, 26.3],
                                  [24.5, 27.8],
                                  [22.5, 29.1],
                                  [20.3, 30],
                                  [18, 30.5],
                                  [15.6, 30.7],
                                  [12.7, 30.4],
                                  [10, 29.6],
                                  [7.4, 28.3],
                                  [5.2, 26.5],
                                  [3.3, 24.3],
                                  [1.8, 21.9],
                                  [0.9, 19.1],
                                  [0.5, 16.3],
                                  [0.5, 15.6],
                                  [0.7, 13.2],
                                  [1.2, 10.9],
                                  [2.1, 8.7],
                                  [3.4, 6.7],
                                  [4.9, 4.9],
                                  [6.7, 3.4],
                                  [8.7, 2.1],
                                  [10.9, 1.2],
                                  [13.2, 0.7],
                                  [15.6, 0.5],
                                ],
                              ],
                            },
                            symbol: {
                              type: "CIMPolygonSymbol",
                              symbolLayers: [
                                {
                                  type: "CIMSolidFill",
                                  enable: false,
                                  color: [255, 255, 255, 255],
                                },
                              ],
                            },
                          },
                          {
                            type: "CIMMarkerGraphic",
                            geometry: {
                              rings: [
                                [
                                  [0.5, 15.6],
                                  [0.7, 13.2],
                                  [1.2, 10.9],
                                  [2.1, 8.7],
                                  [3.4, 6.7],
                                  [4.9, 4.9],
                                  [6.7, 3.4],
                                  [8.7, 2.1],
                                  [10.9, 1.2],
                                  [13.2, 0.7],
                                  [15.6, 0.5],
                                  [18, 0.7],
                                  [20.3, 1.2],
                                  [22.5, 2.1],
                                  [24.5, 3.4],
                                  [26.3, 4.9],
                                  [27.8, 6.7],
                                  [29.1, 8.7],
                                  [30, 10.9],
                                  [30.5, 13.2],
                                  [30.7, 15.6],
                                  [30.5, 18],
                                  [30, 20.3],
                                  [29.1, 22.5],
                                  [27.8, 24.5],
                                  [26.3, 26.3],
                                  [24.5, 27.8],
                                  [22.5, 29.1],
                                  [20.3, 30],
                                  [18, 30.5],
                                  [15.6, 30.7],
                                  [15.6, 30.7],
                                  [13.2, 30.5],
                                  [10.9, 30],
                                  [8.7, 29.1],
                                  [6.7, 27.8],
                                  [4.9, 26.3],
                                  [3.4, 24.5],
                                  [2.1, 22.5],
                                  [1.2, 20.3],
                                  [0.7, 18],
                                  [0.5, 15.6],
                                ],
                              ],
                            },
                            symbol: {
                              type: "CIMPolygonSymbol",
                              symbolLayers: [
                                {
                                  type: "CIMSolidStroke",
                                  enable: true,
                                  capStyle: "Butt",
                                  joinStyle: "Miter",
                                  lineStyle3D: "Strip",
                                  miterLimit: 10,
                                  width: 1,
                                  color: [255, 255, 255, 255],
                                },
                              ],
                            },
                          },
                          {
                            type: "CIMMarkerGraphic",
                            geometry: {
                              rings: [
                                [
                                  [17.6, 5.6],
                                  [17.6, 18.8],
                                  [23.6, 12.5],
                                  [23.6, 17.6],
                                  [15.6, 25.6],
                                  [7.6, 17.6],
                                  [7.6, 12.5],
                                  [13.6, 18.7],
                                  [13.6, 5.6],
                                  [17.6, 5.6],
                                ],
                              ],
                            },
                            symbol: {
                              type: "CIMPolygonSymbol",
                              symbolLayers: [
                                {
                                  type: "CIMSolidFill",
                                  enable: true,
                                  colorLocked: true,
                                  color: [255, 255, 255, 255],
                                },
                              ],
                            },
                          },
                        ],
                        scaleSymbolsProportionally: true,
                        respectFrame: true,
                        clippingPath: {
                          type: "CIMClippingPath",
                          clippingType: "Intersect",
                          path: {
                            rings: [
                              [
                                [0, 0],
                                [31.2, 0],
                                [31.2, 31.2],
                                [0, 31.2],
                                [0, 0],
                              ],
                            ],
                          },
                        },
                        rotation: 180,
                      },
                      {
                        type: "CIMVectorMarker",
                        enable: true,
                        anchorPointUnits: "Relative",
                        dominantSizeAxis3D: "Y",
                        size: 9.69,
                        billboardMode3D: "FaceNearPlane",
                        frame: {
                          xmin: 0,
                          ymin: 0,
                          xmax: 31.2,
                          ymax: 31.2,
                        },
                        markerGraphics: [
                          {
                            type: "CIMMarkerGraphic",
                            geometry: {
                              rings: [
                                [
                                  [15.6, 0.5],
                                  [17, 0.6],
                                  [17.6, 0.6],
                                  [18.3, 0.8],
                                  [19.1, 0.9],
                                  [19.8, 1.1],
                                  [20.5, 1.3],
                                  [21.2, 1.6],
                                  [21.9, 1.9],
                                  [22.5, 2.2],
                                  [23.2, 2.6],
                                  [23.9, 3],
                                  [24.5, 3.4],
                                  [25.1, 3.9],
                                  [25.7, 4.4],
                                  [26.3, 4.9],
                                  [26.8, 5.5],
                                  [27.3, 6.1],
                                  [27.8, 6.7],
                                  [28.2, 7.3],
                                  [28.6, 8],
                                  [29, 8.7],
                                  [29.3, 9.3],
                                  [29.6, 10],
                                  [29.9, 10.7],
                                  [30.1, 11.4],
                                  [30.3, 12.1],
                                  [30.4, 12.9],
                                  [30.6, 13.6],
                                  [30.6, 14.2],
                                  [30.7, 14.9],
                                  [30.7, 15.6],
                                  [30.5, 18],
                                  [30, 20.3],
                                  [29.1, 22.5],
                                  [27.8, 24.5],
                                  [26.3, 26.3],
                                  [24.5, 27.8],
                                  [22.5, 29.1],
                                  [20.3, 30],
                                  [18, 30.5],
                                  [15.6, 30.7],
                                  [12.7, 30.4],
                                  [10, 29.6],
                                  [7.4, 28.3],
                                  [5.2, 26.5],
                                  [3.3, 24.3],
                                  [1.8, 21.9],
                                  [0.9, 19.1],
                                  [0.5, 16.3],
                                  [0.5, 15.6],
                                  [0.7, 13.2],
                                  [1.2, 10.9],
                                  [2.1, 8.7],
                                  [3.4, 6.7],
                                  [4.9, 4.9],
                                  [6.7, 3.4],
                                  [8.7, 2.1],
                                  [10.9, 1.2],
                                  [13.2, 0.7],
                                  [15.6, 0.5],
                                ],
                              ],
                            },
                            symbol: {
                              type: "CIMPolygonSymbol",
                              symbolLayers: [
                                {
                                  type: "CIMSolidFill",
                                  enable: true,
                                  color: [153, 144, 92, 255],
                                },
                              ],
                            },
                          },
                          {
                            type: "CIMMarkerGraphic",
                            geometry: {
                              rings: [
                                [
                                  [0.5, 15.6],
                                  [0.7, 13.2],
                                  [1.2, 10.9],
                                  [2.1, 8.7],
                                  [3.4, 6.7],
                                  [4.9, 4.9],
                                  [6.7, 3.4],
                                  [8.7, 2.1],
                                  [10.9, 1.2],
                                  [13.2, 0.7],
                                  [15.6, 0.5],
                                  [18, 0.7],
                                  [20.3, 1.2],
                                  [22.5, 2.1],
                                  [24.5, 3.4],
                                  [26.3, 4.9],
                                  [27.8, 6.7],
                                  [29.1, 8.7],
                                  [30, 10.9],
                                  [30.5, 13.2],
                                  [30.7, 15.6],
                                  [30.5, 18],
                                  [30, 20.3],
                                  [29.1, 22.5],
                                  [27.8, 24.5],
                                  [26.3, 26.3],
                                  [24.5, 27.8],
                                  [22.5, 29.1],
                                  [20.3, 30],
                                  [18, 30.5],
                                  [15.6, 30.7],
                                  [15.6, 30.7],
                                  [13.2, 30.5],
                                  [10.9, 30],
                                  [8.7, 29.1],
                                  [6.7, 27.8],
                                  [4.9, 26.3],
                                  [3.4, 24.5],
                                  [2.1, 22.5],
                                  [1.2, 20.3],
                                  [0.7, 18],
                                  [0.5, 15.6],
                                ],
                              ],
                            },
                            symbol: {
                              type: "CIMPolygonSymbol",
                              symbolLayers: [
                                {
                                  type: "CIMSolidStroke",
                                  enable: false,
                                  capStyle: "Butt",
                                  joinStyle: "Miter",
                                  lineStyle3D: "Strip",
                                  miterLimit: 10,
                                  width: 1,
                                  color: [0, 0, 0, 255],
                                },
                              ],
                            },
                          },
                          {
                            type: "CIMMarkerGraphic",
                            geometry: {
                              rings: [
                                [
                                  [17.6, 5.6],
                                  [17.6, 18.8],
                                  [23.6, 12.5],
                                  [23.6, 17.6],
                                  [15.6, 25.6],
                                  [7.6, 17.6],
                                  [7.6, 12.5],
                                  [13.6, 18.7],
                                  [13.6, 5.6],
                                  [17.6, 5.6],
                                ],
                              ],
                            },
                            symbol: {
                              type: "CIMPolygonSymbol",
                              symbolLayers: [
                                {
                                  type: "CIMSolidFill",
                                  enable: false,
                                  colorLocked: true,
                                  color: [0, 0, 0, 255],
                                },
                              ],
                            },
                          },
                        ],
                        scaleSymbolsProportionally: true,
                        respectFrame: true,
                        clippingPath: {
                          type: "CIMClippingPath",
                          clippingType: "Intersect",
                          path: {
                            rings: [
                              [
                                [0, 0],
                                [31.2, 0],
                                [31.2, 31.2],
                                [0, 31.2],
                                [0, 0],
                              ],
                            ],
                          },
                        },
                        rotation: 180,
                      },
                    ],
                    haloSize: 1,
                    scaleX: 1,
                    angleAlignment: "Display",
                    angle: 180,
                  },
                },
                values: [["d"]],
              },
              {
                label: "Unknown",
                symbol: {
                  type: "CIMSymbolReference",
                  symbol: {
                    type: "CIMPointSymbol",
                    symbolLayers: [
                      {
                        type: "CIMVectorMarker",
                        enable: true,
                        colorLocked: true,
                        anchorPointUnits: "Relative",
                        dominantSizeAxis3D: "Y",
                        size: 9.69,
                        billboardMode3D: "FaceNearPlane",
                        frame: {
                          xmin: 0,
                          ymin: 0,
                          xmax: 31.2,
                          ymax: 31.2,
                        },
                        markerGraphics: [
                          {
                            type: "CIMMarkerGraphic",
                            geometry: {
                              rings: [
                                [
                                  [15.6, 0.5],
                                  [17, 0.6],
                                  [17.6, 0.6],
                                  [18.3, 0.8],
                                  [19.1, 0.9],
                                  [19.8, 1.1],
                                  [20.5, 1.3],
                                  [21.2, 1.6],
                                  [21.9, 1.9],
                                  [22.5, 2.2],
                                  [23.2, 2.6],
                                  [23.9, 3],
                                  [24.5, 3.4],
                                  [25.1, 3.9],
                                  [25.7, 4.4],
                                  [26.3, 4.9],
                                  [26.8, 5.5],
                                  [27.3, 6.1],
                                  [27.8, 6.7],
                                  [28.2, 7.3],
                                  [28.6, 8],
                                  [29, 8.7],
                                  [29.3, 9.3],
                                  [29.6, 10],
                                  [29.9, 10.7],
                                  [30.1, 11.4],
                                  [30.3, 12.1],
                                  [30.4, 12.9],
                                  [30.6, 13.6],
                                  [30.6, 14.2],
                                  [30.7, 14.9],
                                  [30.7, 15.6],
                                  [30.5, 18],
                                  [30, 20.3],
                                  [29.1, 22.5],
                                  [27.8, 24.5],
                                  [26.3, 26.3],
                                  [24.5, 27.8],
                                  [22.5, 29.1],
                                  [20.3, 30],
                                  [18, 30.5],
                                  [15.6, 30.7],
                                  [12.7, 30.4],
                                  [10, 29.6],
                                  [7.4, 28.3],
                                  [5.2, 26.5],
                                  [3.3, 24.3],
                                  [1.8, 21.9],
                                  [0.9, 19.1],
                                  [0.5, 16.3],
                                  [0.5, 15.6],
                                  [0.7, 13.2],
                                  [1.2, 10.9],
                                  [2.1, 8.7],
                                  [3.4, 6.7],
                                  [4.9, 4.9],
                                  [6.7, 3.4],
                                  [8.7, 2.1],
                                  [10.9, 1.2],
                                  [13.2, 0.7],
                                  [15.6, 0.5],
                                ],
                              ],
                            },
                            symbol: {
                              type: "CIMPolygonSymbol",
                              symbolLayers: [
                                {
                                  type: "CIMSolidFill",
                                  enable: false,
                                  color: [255, 255, 255, 255],
                                },
                              ],
                            },
                          },
                          {
                            type: "CIMMarkerGraphic",
                            geometry: {
                              rings: [
                                [
                                  [0.5, 15.6],
                                  [0.7, 13.2],
                                  [1.2, 10.9],
                                  [2.1, 8.7],
                                  [3.4, 6.7],
                                  [4.9, 4.9],
                                  [6.7, 3.4],
                                  [8.7, 2.1],
                                  [10.9, 1.2],
                                  [13.2, 0.7],
                                  [15.6, 0.5],
                                  [18, 0.7],
                                  [20.3, 1.2],
                                  [22.5, 2.1],
                                  [24.5, 3.4],
                                  [26.3, 4.9],
                                  [27.8, 6.7],
                                  [29.1, 8.7],
                                  [30, 10.9],
                                  [30.5, 13.2],
                                  [30.7, 15.6],
                                  [30.5, 18],
                                  [30, 20.3],
                                  [29.1, 22.5],
                                  [27.8, 24.5],
                                  [26.3, 26.3],
                                  [24.5, 27.8],
                                  [22.5, 29.1],
                                  [20.3, 30],
                                  [18, 30.5],
                                  [15.6, 30.7],
                                  [15.6, 30.7],
                                  [13.2, 30.5],
                                  [10.9, 30],
                                  [8.7, 29.1],
                                  [6.7, 27.8],
                                  [4.9, 26.3],
                                  [3.4, 24.5],
                                  [2.1, 22.5],
                                  [1.2, 20.3],
                                  [0.7, 18],
                                  [0.5, 15.6],
                                ],
                              ],
                            },
                            symbol: {
                              type: "CIMPolygonSymbol",
                              symbolLayers: [
                                {
                                  type: "CIMSolidStroke",
                                  enable: true,
                                  capStyle: "Butt",
                                  joinStyle: "Miter",
                                  lineStyle3D: "Strip",
                                  miterLimit: 10,
                                  width: 1,
                                  color: [255, 255, 255, 255],
                                },
                              ],
                            },
                          },
                          {
                            type: "CIMMarkerGraphic",
                            geometry: {
                              rings: [
                                [
                                  [17.6, 5.6],
                                  [17.6, 18.8],
                                  [23.6, 12.5],
                                  [23.6, 17.6],
                                  [15.6, 25.6],
                                  [7.6, 17.6],
                                  [7.6, 12.5],
                                  [13.6, 18.7],
                                  [13.6, 5.6],
                                  [17.6, 5.6],
                                ],
                              ],
                            },
                            symbol: {
                              type: "CIMPolygonSymbol",
                              symbolLayers: [
                                {
                                  type: "CIMSolidFill",
                                  enable: true,
                                  colorLocked: true,
                                  color: [255, 255, 255, 255],
                                },
                              ],
                            },
                          },
                        ],
                        scaleSymbolsProportionally: true,
                        respectFrame: true,
                        clippingPath: {
                          type: "CIMClippingPath",
                          clippingType: "Intersect",
                          path: {
                            rings: [
                              [
                                [0, 0],
                                [31.2, 0],
                                [31.2, 31.2],
                                [0, 31.2],
                                [0, 0],
                              ],
                            ],
                          },
                        },
                        rotation: 0,
                      },
                      {
                        type: "CIMVectorMarker",
                        enable: true,
                        anchorPointUnits: "Relative",
                        dominantSizeAxis3D: "Y",
                        size: 9.69,
                        billboardMode3D: "FaceNearPlane",
                        frame: {
                          xmin: 0,
                          ymin: 0,
                          xmax: 31.2,
                          ymax: 31.2,
                        },
                        markerGraphics: [
                          {
                            type: "CIMMarkerGraphic",
                            geometry: {
                              rings: [
                                [
                                  [15.6, 0.5],
                                  [17, 0.6],
                                  [17.6, 0.6],
                                  [18.3, 0.8],
                                  [19.1, 0.9],
                                  [19.8, 1.1],
                                  [20.5, 1.3],
                                  [21.2, 1.6],
                                  [21.9, 1.9],
                                  [22.5, 2.2],
                                  [23.2, 2.6],
                                  [23.9, 3],
                                  [24.5, 3.4],
                                  [25.1, 3.9],
                                  [25.7, 4.4],
                                  [26.3, 4.9],
                                  [26.8, 5.5],
                                  [27.3, 6.1],
                                  [27.8, 6.7],
                                  [28.2, 7.3],
                                  [28.6, 8],
                                  [29, 8.7],
                                  [29.3, 9.3],
                                  [29.6, 10],
                                  [29.9, 10.7],
                                  [30.1, 11.4],
                                  [30.3, 12.1],
                                  [30.4, 12.9],
                                  [30.6, 13.6],
                                  [30.6, 14.2],
                                  [30.7, 14.9],
                                  [30.7, 15.6],
                                  [30.5, 18],
                                  [30, 20.3],
                                  [29.1, 22.5],
                                  [27.8, 24.5],
                                  [26.3, 26.3],
                                  [24.5, 27.8],
                                  [22.5, 29.1],
                                  [20.3, 30],
                                  [18, 30.5],
                                  [15.6, 30.7],
                                  [12.7, 30.4],
                                  [10, 29.6],
                                  [7.4, 28.3],
                                  [5.2, 26.5],
                                  [3.3, 24.3],
                                  [1.8, 21.9],
                                  [0.9, 19.1],
                                  [0.5, 16.3],
                                  [0.5, 15.6],
                                  [0.7, 13.2],
                                  [1.2, 10.9],
                                  [2.1, 8.7],
                                  [3.4, 6.7],
                                  [4.9, 4.9],
                                  [6.7, 3.4],
                                  [8.7, 2.1],
                                  [10.9, 1.2],
                                  [13.2, 0.7],
                                  [15.6, 0.5],
                                ],
                              ],
                            },
                            symbol: {
                              type: "CIMPolygonSymbol",
                              symbolLayers: [
                                {
                                  type: "CIMSolidFill",
                                  enable: true,
                                  color: [137, 166, 166, 255],
                                },
                              ],
                            },
                          },
                          {
                            type: "CIMMarkerGraphic",
                            geometry: {
                              rings: [
                                [
                                  [0.5, 15.6],
                                  [0.7, 13.2],
                                  [1.2, 10.9],
                                  [2.1, 8.7],
                                  [3.4, 6.7],
                                  [4.9, 4.9],
                                  [6.7, 3.4],
                                  [8.7, 2.1],
                                  [10.9, 1.2],
                                  [13.2, 0.7],
                                  [15.6, 0.5],
                                  [18, 0.7],
                                  [20.3, 1.2],
                                  [22.5, 2.1],
                                  [24.5, 3.4],
                                  [26.3, 4.9],
                                  [27.8, 6.7],
                                  [29.1, 8.7],
                                  [30, 10.9],
                                  [30.5, 13.2],
                                  [30.7, 15.6],
                                  [30.5, 18],
                                  [30, 20.3],
                                  [29.1, 22.5],
                                  [27.8, 24.5],
                                  [26.3, 26.3],
                                  [24.5, 27.8],
                                  [22.5, 29.1],
                                  [20.3, 30],
                                  [18, 30.5],
                                  [15.6, 30.7],
                                  [15.6, 30.7],
                                  [13.2, 30.5],
                                  [10.9, 30],
                                  [8.7, 29.1],
                                  [6.7, 27.8],
                                  [4.9, 26.3],
                                  [3.4, 24.5],
                                  [2.1, 22.5],
                                  [1.2, 20.3],
                                  [0.7, 18],
                                  [0.5, 15.6],
                                ],
                              ],
                            },
                            symbol: {
                              type: "CIMPolygonSymbol",
                              symbolLayers: [
                                {
                                  type: "CIMSolidStroke",
                                  enable: false,
                                  capStyle: "Butt",
                                  joinStyle: "Miter",
                                  lineStyle3D: "Strip",
                                  miterLimit: 10,
                                  width: 1,
                                  color: [0, 0, 0, 255],
                                },
                              ],
                            },
                          },
                          {
                            type: "CIMMarkerGraphic",
                            geometry: {
                              rings: [
                                [
                                  [17.6, 5.6],
                                  [17.6, 18.8],
                                  [23.6, 12.5],
                                  [23.6, 17.6],
                                  [15.6, 25.6],
                                  [7.6, 17.6],
                                  [7.6, 12.5],
                                  [13.6, 18.7],
                                  [13.6, 5.6],
                                  [17.6, 5.6],
                                ],
                              ],
                            },
                            symbol: {
                              type: "CIMPolygonSymbol",
                              symbolLayers: [
                                {
                                  type: "CIMSolidFill",
                                  enable: false,
                                  colorLocked: true,
                                  color: [0, 0, 0, 255],
                                },
                              ],
                            },
                          },
                        ],
                        scaleSymbolsProportionally: true,
                        respectFrame: true,
                        clippingPath: {
                          type: "CIMClippingPath",
                          clippingType: "Intersect",
                          path: {
                            rings: [
                              [
                                [0, 0],
                                [31.2, 0],
                                [31.2, 31.2],
                                [0, 31.2],
                                [0, 0],
                              ],
                            ],
                          },
                        },
                        rotation: 0,
                      },
                    ],
                    haloSize: 1,
                    scaleX: 1,
                    angleAlignment: "Display",
                    angle: 0,
                  },
                },
                values: [["u"]],
              },
            ],
          },
        ],
        uniqueValueInfos: [
          {
            label: "Both",
            symbol: {
              type: "CIMSymbolReference",
              symbol: {
                type: "CIMPointSymbol",
                symbolLayers: [
                  {
                    type: "CIMVectorMarker",
                    enable: true,
                    colorLocked: true,
                    anchorPointUnits: "Relative",
                    dominantSizeAxis3D: "Y",
                    size: 9.69,
                    billboardMode3D: "FaceNearPlane",
                    frame: {
                      xmin: 0,
                      ymin: 0,
                      xmax: 31.2,
                      ymax: 31.2,
                    },
                    markerGraphics: [
                      {
                        type: "CIMMarkerGraphic",
                        geometry: {
                          rings: [
                            [
                              [15.6, 0.5],
                              [17, 0.6],
                              [17.6, 0.6],
                              [18.3, 0.8],
                              [19.1, 0.9],
                              [19.8, 1.1],
                              [20.5, 1.3],
                              [21.2, 1.6],
                              [21.9, 1.9],
                              [22.5, 2.2],
                              [23.2, 2.6],
                              [23.9, 3],
                              [24.5, 3.4],
                              [25.1, 3.9],
                              [25.7, 4.4],
                              [26.3, 4.9],
                              [26.8, 5.5],
                              [27.3, 6.1],
                              [27.8, 6.7],
                              [28.2, 7.3],
                              [28.6, 8],
                              [29, 8.7],
                              [29.3, 9.3],
                              [29.6, 10],
                              [29.9, 10.7],
                              [30.1, 11.4],
                              [30.3, 12.1],
                              [30.4, 12.9],
                              [30.6, 13.6],
                              [30.6, 14.2],
                              [30.7, 14.9],
                              [30.7, 15.6],
                              [30.5, 18],
                              [30, 20.3],
                              [29.1, 22.5],
                              [27.8, 24.5],
                              [26.3, 26.3],
                              [24.5, 27.8],
                              [22.5, 29.1],
                              [20.3, 30],
                              [18, 30.5],
                              [15.6, 30.7],
                              [12.7, 30.4],
                              [10, 29.6],
                              [7.4, 28.3],
                              [5.2, 26.5],
                              [3.3, 24.3],
                              [1.8, 21.9],
                              [0.9, 19.1],
                              [0.5, 16.3],
                              [0.5, 15.6],
                              [0.7, 13.2],
                              [1.2, 10.9],
                              [2.1, 8.7],
                              [3.4, 6.7],
                              [4.9, 4.9],
                              [6.7, 3.4],
                              [8.7, 2.1],
                              [10.9, 1.2],
                              [13.2, 0.7],
                              [15.6, 0.5],
                            ],
                          ],
                        },
                        symbol: {
                          type: "CIMPolygonSymbol",
                          symbolLayers: [
                            {
                              type: "CIMSolidFill",
                              enable: false,
                              color: [255, 255, 255, 255],
                            },
                          ],
                        },
                      },
                      {
                        type: "CIMMarkerGraphic",
                        geometry: {
                          rings: [
                            [
                              [0.5, 15.6],
                              [0.7, 13.2],
                              [1.2, 10.9],
                              [2.1, 8.7],
                              [3.4, 6.7],
                              [4.9, 4.9],
                              [6.7, 3.4],
                              [8.7, 2.1],
                              [10.9, 1.2],
                              [13.2, 0.7],
                              [15.6, 0.5],
                              [18, 0.7],
                              [20.3, 1.2],
                              [22.5, 2.1],
                              [24.5, 3.4],
                              [26.3, 4.9],
                              [27.8, 6.7],
                              [29.1, 8.7],
                              [30, 10.9],
                              [30.5, 13.2],
                              [30.7, 15.6],
                              [30.5, 18],
                              [30, 20.3],
                              [29.1, 22.5],
                              [27.8, 24.5],
                              [26.3, 26.3],
                              [24.5, 27.8],
                              [22.5, 29.1],
                              [20.3, 30],
                              [18, 30.5],
                              [15.6, 30.7],
                              [15.6, 30.7],
                              [13.2, 30.5],
                              [10.9, 30],
                              [8.7, 29.1],
                              [6.7, 27.8],
                              [4.9, 26.3],
                              [3.4, 24.5],
                              [2.1, 22.5],
                              [1.2, 20.3],
                              [0.7, 18],
                              [0.5, 15.6],
                            ],
                          ],
                        },
                        symbol: {
                          type: "CIMPolygonSymbol",
                          symbolLayers: [
                            {
                              type: "CIMSolidStroke",
                              enable: true,
                              capStyle: "Butt",
                              joinStyle: "Miter",
                              lineStyle3D: "Strip",
                              miterLimit: 10,
                              width: 1,
                              color: [255, 255, 255, 255],
                            },
                          ],
                        },
                      },
                      {
                        type: "CIMMarkerGraphic",
                        geometry: {
                          rings: [
                            [
                              [17.6, 5.6],
                              [17.6, 18.8],
                              [23.6, 12.5],
                              [23.6, 17.6],
                              [15.6, 25.6],
                              [7.6, 17.6],
                              [7.6, 12.5],
                              [13.6, 18.7],
                              [13.6, 5.6],
                              [17.6, 5.6],
                            ],
                          ],
                        },
                        symbol: {
                          type: "CIMPolygonSymbol",
                          symbolLayers: [
                            {
                              type: "CIMSolidFill",
                              enable: true,
                              colorLocked: true,
                              color: [255, 255, 255, 255],
                            },
                          ],
                        },
                      },
                    ],
                    scaleSymbolsProportionally: true,
                    respectFrame: true,
                    clippingPath: {
                      type: "CIMClippingPath",
                      clippingType: "Intersect",
                      path: {
                        rings: [
                          [
                            [0, 0],
                            [31.2, 0],
                            [31.2, 31.2],
                            [0, 31.2],
                            [0, 0],
                          ],
                        ],
                      },
                    },
                    rotation: 0,
                  },
                  {
                    type: "CIMVectorMarker",
                    enable: true,
                    anchorPointUnits: "Relative",
                    dominantSizeAxis3D: "Y",
                    size: 9.69,
                    billboardMode3D: "FaceNearPlane",
                    frame: {
                      xmin: 0,
                      ymin: 0,
                      xmax: 31.2,
                      ymax: 31.2,
                    },
                    markerGraphics: [
                      {
                        type: "CIMMarkerGraphic",
                        geometry: {
                          rings: [
                            [
                              [15.6, 0.5],
                              [17, 0.6],
                              [17.6, 0.6],
                              [18.3, 0.8],
                              [19.1, 0.9],
                              [19.8, 1.1],
                              [20.5, 1.3],
                              [21.2, 1.6],
                              [21.9, 1.9],
                              [22.5, 2.2],
                              [23.2, 2.6],
                              [23.9, 3],
                              [24.5, 3.4],
                              [25.1, 3.9],
                              [25.7, 4.4],
                              [26.3, 4.9],
                              [26.8, 5.5],
                              [27.3, 6.1],
                              [27.8, 6.7],
                              [28.2, 7.3],
                              [28.6, 8],
                              [29, 8.7],
                              [29.3, 9.3],
                              [29.6, 10],
                              [29.9, 10.7],
                              [30.1, 11.4],
                              [30.3, 12.1],
                              [30.4, 12.9],
                              [30.6, 13.6],
                              [30.6, 14.2],
                              [30.7, 14.9],
                              [30.7, 15.6],
                              [30.5, 18],
                              [30, 20.3],
                              [29.1, 22.5],
                              [27.8, 24.5],
                              [26.3, 26.3],
                              [24.5, 27.8],
                              [22.5, 29.1],
                              [20.3, 30],
                              [18, 30.5],
                              [15.6, 30.7],
                              [12.7, 30.4],
                              [10, 29.6],
                              [7.4, 28.3],
                              [5.2, 26.5],
                              [3.3, 24.3],
                              [1.8, 21.9],
                              [0.9, 19.1],
                              [0.5, 16.3],
                              [0.5, 15.6],
                              [0.7, 13.2],
                              [1.2, 10.9],
                              [2.1, 8.7],
                              [3.4, 6.7],
                              [4.9, 4.9],
                              [6.7, 3.4],
                              [8.7, 2.1],
                              [10.9, 1.2],
                              [13.2, 0.7],
                              [15.6, 0.5],
                            ],
                          ],
                        },
                        symbol: {
                          type: "CIMPolygonSymbol",
                          symbolLayers: [
                            {
                              type: "CIMSolidFill",
                              enable: true,
                              color: [0, 0, 0, 255],
                            },
                          ],
                        },
                      },
                      {
                        type: "CIMMarkerGraphic",
                        geometry: {
                          rings: [
                            [
                              [0.5, 15.6],
                              [0.7, 13.2],
                              [1.2, 10.9],
                              [2.1, 8.7],
                              [3.4, 6.7],
                              [4.9, 4.9],
                              [6.7, 3.4],
                              [8.7, 2.1],
                              [10.9, 1.2],
                              [13.2, 0.7],
                              [15.6, 0.5],
                              [18, 0.7],
                              [20.3, 1.2],
                              [22.5, 2.1],
                              [24.5, 3.4],
                              [26.3, 4.9],
                              [27.8, 6.7],
                              [29.1, 8.7],
                              [30, 10.9],
                              [30.5, 13.2],
                              [30.7, 15.6],
                              [30.5, 18],
                              [30, 20.3],
                              [29.1, 22.5],
                              [27.8, 24.5],
                              [26.3, 26.3],
                              [24.5, 27.8],
                              [22.5, 29.1],
                              [20.3, 30],
                              [18, 30.5],
                              [15.6, 30.7],
                              [15.6, 30.7],
                              [13.2, 30.5],
                              [10.9, 30],
                              [8.7, 29.1],
                              [6.7, 27.8],
                              [4.9, 26.3],
                              [3.4, 24.5],
                              [2.1, 22.5],
                              [1.2, 20.3],
                              [0.7, 18],
                              [0.5, 15.6],
                            ],
                          ],
                        },
                        symbol: {
                          type: "CIMPolygonSymbol",
                          symbolLayers: [
                            {
                              type: "CIMSolidStroke",
                              enable: false,
                              capStyle: "Butt",
                              joinStyle: "Miter",
                              lineStyle3D: "Strip",
                              miterLimit: 10,
                              width: 1,
                              color: [0, 0, 0, 255],
                            },
                          ],
                        },
                      },
                      {
                        type: "CIMMarkerGraphic",
                        geometry: {
                          rings: [
                            [
                              [17.6, 5.6],
                              [17.6, 18.8],
                              [23.6, 12.5],
                              [23.6, 17.6],
                              [15.6, 25.6],
                              [7.6, 17.6],
                              [7.6, 12.5],
                              [13.6, 18.7],
                              [13.6, 5.6],
                              [17.6, 5.6],
                            ],
                          ],
                        },
                        symbol: {
                          type: "CIMPolygonSymbol",
                          symbolLayers: [
                            {
                              type: "CIMSolidFill",
                              enable: false,
                              colorLocked: true,
                              color: [0, 0, 0, 255],
                            },
                          ],
                        },
                      },
                    ],
                    scaleSymbolsProportionally: true,
                    respectFrame: true,
                    clippingPath: {
                      type: "CIMClippingPath",
                      clippingType: "Intersect",
                      path: {
                        rings: [
                          [
                            [0, 0],
                            [31.2, 0],
                            [31.2, 31.2],
                            [0, 31.2],
                            [0, 0],
                          ],
                        ],
                      },
                    },
                    rotation: 0,
                  },
                ],
                haloSize: 1,
                scaleX: 1,
                angleAlignment: "Display",
                angle: 0,
              },
            },
            value: "b",
          },
          {
            label: "Increase",
            symbol: {
              type: "CIMSymbolReference",
              symbol: {
                type: "CIMPointSymbol",
                symbolLayers: [
                  {
                    type: "CIMVectorMarker",
                    enable: true,
                    colorLocked: true,
                    anchorPointUnits: "Relative",
                    dominantSizeAxis3D: "Y",
                    size: 9.69,
                    billboardMode3D: "FaceNearPlane",
                    frame: {
                      xmin: 0,
                      ymin: 0,
                      xmax: 31.2,
                      ymax: 31.2,
                    },
                    markerGraphics: [
                      {
                        type: "CIMMarkerGraphic",
                        geometry: {
                          rings: [
                            [
                              [15.6, 0.5],
                              [17, 0.6],
                              [17.6, 0.6],
                              [18.3, 0.8],
                              [19.1, 0.9],
                              [19.8, 1.1],
                              [20.5, 1.3],
                              [21.2, 1.6],
                              [21.9, 1.9],
                              [22.5, 2.2],
                              [23.2, 2.6],
                              [23.9, 3],
                              [24.5, 3.4],
                              [25.1, 3.9],
                              [25.7, 4.4],
                              [26.3, 4.9],
                              [26.8, 5.5],
                              [27.3, 6.1],
                              [27.8, 6.7],
                              [28.2, 7.3],
                              [28.6, 8],
                              [29, 8.7],
                              [29.3, 9.3],
                              [29.6, 10],
                              [29.9, 10.7],
                              [30.1, 11.4],
                              [30.3, 12.1],
                              [30.4, 12.9],
                              [30.6, 13.6],
                              [30.6, 14.2],
                              [30.7, 14.9],
                              [30.7, 15.6],
                              [30.5, 18],
                              [30, 20.3],
                              [29.1, 22.5],
                              [27.8, 24.5],
                              [26.3, 26.3],
                              [24.5, 27.8],
                              [22.5, 29.1],
                              [20.3, 30],
                              [18, 30.5],
                              [15.6, 30.7],
                              [12.7, 30.4],
                              [10, 29.6],
                              [7.4, 28.3],
                              [5.2, 26.5],
                              [3.3, 24.3],
                              [1.8, 21.9],
                              [0.9, 19.1],
                              [0.5, 16.3],
                              [0.5, 15.6],
                              [0.7, 13.2],
                              [1.2, 10.9],
                              [2.1, 8.7],
                              [3.4, 6.7],
                              [4.9, 4.9],
                              [6.7, 3.4],
                              [8.7, 2.1],
                              [10.9, 1.2],
                              [13.2, 0.7],
                              [15.6, 0.5],
                            ],
                          ],
                        },
                        symbol: {
                          type: "CIMPolygonSymbol",
                          symbolLayers: [
                            {
                              type: "CIMSolidFill",
                              enable: false,
                              color: [255, 255, 255, 255],
                            },
                          ],
                        },
                      },
                      {
                        type: "CIMMarkerGraphic",
                        geometry: {
                          rings: [
                            [
                              [0.5, 15.6],
                              [0.7, 13.2],
                              [1.2, 10.9],
                              [2.1, 8.7],
                              [3.4, 6.7],
                              [4.9, 4.9],
                              [6.7, 3.4],
                              [8.7, 2.1],
                              [10.9, 1.2],
                              [13.2, 0.7],
                              [15.6, 0.5],
                              [18, 0.7],
                              [20.3, 1.2],
                              [22.5, 2.1],
                              [24.5, 3.4],
                              [26.3, 4.9],
                              [27.8, 6.7],
                              [29.1, 8.7],
                              [30, 10.9],
                              [30.5, 13.2],
                              [30.7, 15.6],
                              [30.5, 18],
                              [30, 20.3],
                              [29.1, 22.5],
                              [27.8, 24.5],
                              [26.3, 26.3],
                              [24.5, 27.8],
                              [22.5, 29.1],
                              [20.3, 30],
                              [18, 30.5],
                              [15.6, 30.7],
                              [15.6, 30.7],
                              [13.2, 30.5],
                              [10.9, 30],
                              [8.7, 29.1],
                              [6.7, 27.8],
                              [4.9, 26.3],
                              [3.4, 24.5],
                              [2.1, 22.5],
                              [1.2, 20.3],
                              [0.7, 18],
                              [0.5, 15.6],
                            ],
                          ],
                        },
                        symbol: {
                          type: "CIMPolygonSymbol",
                          symbolLayers: [
                            {
                              type: "CIMSolidStroke",
                              enable: true,
                              capStyle: "Butt",
                              joinStyle: "Miter",
                              lineStyle3D: "Strip",
                              miterLimit: 10,
                              width: 1,
                              color: [255, 255, 255, 255],
                            },
                          ],
                        },
                      },
                      {
                        type: "CIMMarkerGraphic",
                        geometry: {
                          rings: [
                            [
                              [17.6, 5.6],
                              [17.6, 18.8],
                              [23.6, 12.5],
                              [23.6, 17.6],
                              [15.6, 25.6],
                              [7.6, 17.6],
                              [7.6, 12.5],
                              [13.6, 18.7],
                              [13.6, 5.6],
                              [17.6, 5.6],
                            ],
                          ],
                        },
                        symbol: {
                          type: "CIMPolygonSymbol",
                          symbolLayers: [
                            {
                              type: "CIMSolidFill",
                              enable: true,
                              colorLocked: true,
                              color: [255, 255, 255, 255],
                            },
                          ],
                        },
                      },
                    ],
                    scaleSymbolsProportionally: true,
                    respectFrame: true,
                    clippingPath: {
                      type: "CIMClippingPath",
                      clippingType: "Intersect",
                      path: {
                        rings: [
                          [
                            [0, 0],
                            [31.2, 0],
                            [31.2, 31.2],
                            [0, 31.2],
                            [0, 0],
                          ],
                        ],
                      },
                    },
                    rotation: 0,
                  },
                  {
                    type: "CIMVectorMarker",
                    enable: true,
                    anchorPointUnits: "Relative",
                    dominantSizeAxis3D: "Y",
                    size: 9.69,
                    billboardMode3D: "FaceNearPlane",
                    frame: {
                      xmin: 0,
                      ymin: 0,
                      xmax: 31.2,
                      ymax: 31.2,
                    },
                    markerGraphics: [
                      {
                        type: "CIMMarkerGraphic",
                        geometry: {
                          rings: [
                            [
                              [15.6, 0.5],
                              [17, 0.6],
                              [17.6, 0.6],
                              [18.3, 0.8],
                              [19.1, 0.9],
                              [19.8, 1.1],
                              [20.5, 1.3],
                              [21.2, 1.6],
                              [21.9, 1.9],
                              [22.5, 2.2],
                              [23.2, 2.6],
                              [23.9, 3],
                              [24.5, 3.4],
                              [25.1, 3.9],
                              [25.7, 4.4],
                              [26.3, 4.9],
                              [26.8, 5.5],
                              [27.3, 6.1],
                              [27.8, 6.7],
                              [28.2, 7.3],
                              [28.6, 8],
                              [29, 8.7],
                              [29.3, 9.3],
                              [29.6, 10],
                              [29.9, 10.7],
                              [30.1, 11.4],
                              [30.3, 12.1],
                              [30.4, 12.9],
                              [30.6, 13.6],
                              [30.6, 14.2],
                              [30.7, 14.9],
                              [30.7, 15.6],
                              [30.5, 18],
                              [30, 20.3],
                              [29.1, 22.5],
                              [27.8, 24.5],
                              [26.3, 26.3],
                              [24.5, 27.8],
                              [22.5, 29.1],
                              [20.3, 30],
                              [18, 30.5],
                              [15.6, 30.7],
                              [12.7, 30.4],
                              [10, 29.6],
                              [7.4, 28.3],
                              [5.2, 26.5],
                              [3.3, 24.3],
                              [1.8, 21.9],
                              [0.9, 19.1],
                              [0.5, 16.3],
                              [0.5, 15.6],
                              [0.7, 13.2],
                              [1.2, 10.9],
                              [2.1, 8.7],
                              [3.4, 6.7],
                              [4.9, 4.9],
                              [6.7, 3.4],
                              [8.7, 2.1],
                              [10.9, 1.2],
                              [13.2, 0.7],
                              [15.6, 0.5],
                            ],
                          ],
                        },
                        symbol: {
                          type: "CIMPolygonSymbol",
                          symbolLayers: [
                            {
                              type: "CIMSolidFill",
                              enable: true,
                              color: [65, 75, 140, 255],
                            },
                          ],
                        },
                      },
                      {
                        type: "CIMMarkerGraphic",
                        geometry: {
                          rings: [
                            [
                              [0.5, 15.6],
                              [0.7, 13.2],
                              [1.2, 10.9],
                              [2.1, 8.7],
                              [3.4, 6.7],
                              [4.9, 4.9],
                              [6.7, 3.4],
                              [8.7, 2.1],
                              [10.9, 1.2],
                              [13.2, 0.7],
                              [15.6, 0.5],
                              [18, 0.7],
                              [20.3, 1.2],
                              [22.5, 2.1],
                              [24.5, 3.4],
                              [26.3, 4.9],
                              [27.8, 6.7],
                              [29.1, 8.7],
                              [30, 10.9],
                              [30.5, 13.2],
                              [30.7, 15.6],
                              [30.5, 18],
                              [30, 20.3],
                              [29.1, 22.5],
                              [27.8, 24.5],
                              [26.3, 26.3],
                              [24.5, 27.8],
                              [22.5, 29.1],
                              [20.3, 30],
                              [18, 30.5],
                              [15.6, 30.7],
                              [15.6, 30.7],
                              [13.2, 30.5],
                              [10.9, 30],
                              [8.7, 29.1],
                              [6.7, 27.8],
                              [4.9, 26.3],
                              [3.4, 24.5],
                              [2.1, 22.5],
                              [1.2, 20.3],
                              [0.7, 18],
                              [0.5, 15.6],
                            ],
                          ],
                        },
                        symbol: {
                          type: "CIMPolygonSymbol",
                          symbolLayers: [
                            {
                              type: "CIMSolidStroke",
                              enable: false,
                              capStyle: "Butt",
                              joinStyle: "Miter",
                              lineStyle3D: "Strip",
                              miterLimit: 10,
                              width: 1,
                              color: [0, 0, 0, 255],
                            },
                          ],
                        },
                      },
                      {
                        type: "CIMMarkerGraphic",
                        geometry: {
                          rings: [
                            [
                              [17.6, 5.6],
                              [17.6, 18.8],
                              [23.6, 12.5],
                              [23.6, 17.6],
                              [15.6, 25.6],
                              [7.6, 17.6],
                              [7.6, 12.5],
                              [13.6, 18.7],
                              [13.6, 5.6],
                              [17.6, 5.6],
                            ],
                          ],
                        },
                        symbol: {
                          type: "CIMPolygonSymbol",
                          symbolLayers: [
                            {
                              type: "CIMSolidFill",
                              enable: false,
                              colorLocked: true,
                              color: [0, 0, 0, 255],
                            },
                          ],
                        },
                      },
                    ],
                    scaleSymbolsProportionally: true,
                    respectFrame: true,
                    clippingPath: {
                      type: "CIMClippingPath",
                      clippingType: "Intersect",
                      path: {
                        rings: [
                          [
                            [0, 0],
                            [31.2, 0],
                            [31.2, 31.2],
                            [0, 31.2],
                            [0, 0],
                          ],
                        ],
                      },
                    },
                    rotation: 0,
                  },
                ],
                haloSize: 1,
                scaleX: 1,
                angleAlignment: "Display",
                angle: 0,
              },
            },
            value: "i",
          },
          {
            label: "Decrease",
            symbol: {
              type: "CIMSymbolReference",
              symbol: {
                type: "CIMPointSymbol",
                symbolLayers: [
                  {
                    type: "CIMVectorMarker",
                    enable: true,
                    colorLocked: true,
                    anchorPointUnits: "Relative",
                    dominantSizeAxis3D: "Y",
                    size: 9.69,
                    billboardMode3D: "FaceNearPlane",
                    frame: {
                      xmin: 0,
                      ymin: 0,
                      xmax: 31.2,
                      ymax: 31.2,
                    },
                    markerGraphics: [
                      {
                        type: "CIMMarkerGraphic",
                        geometry: {
                          rings: [
                            [
                              [15.6, 0.5],
                              [17, 0.6],
                              [17.6, 0.6],
                              [18.3, 0.8],
                              [19.1, 0.9],
                              [19.8, 1.1],
                              [20.5, 1.3],
                              [21.2, 1.6],
                              [21.9, 1.9],
                              [22.5, 2.2],
                              [23.2, 2.6],
                              [23.9, 3],
                              [24.5, 3.4],
                              [25.1, 3.9],
                              [25.7, 4.4],
                              [26.3, 4.9],
                              [26.8, 5.5],
                              [27.3, 6.1],
                              [27.8, 6.7],
                              [28.2, 7.3],
                              [28.6, 8],
                              [29, 8.7],
                              [29.3, 9.3],
                              [29.6, 10],
                              [29.9, 10.7],
                              [30.1, 11.4],
                              [30.3, 12.1],
                              [30.4, 12.9],
                              [30.6, 13.6],
                              [30.6, 14.2],
                              [30.7, 14.9],
                              [30.7, 15.6],
                              [30.5, 18],
                              [30, 20.3],
                              [29.1, 22.5],
                              [27.8, 24.5],
                              [26.3, 26.3],
                              [24.5, 27.8],
                              [22.5, 29.1],
                              [20.3, 30],
                              [18, 30.5],
                              [15.6, 30.7],
                              [12.7, 30.4],
                              [10, 29.6],
                              [7.4, 28.3],
                              [5.2, 26.5],
                              [3.3, 24.3],
                              [1.8, 21.9],
                              [0.9, 19.1],
                              [0.5, 16.3],
                              [0.5, 15.6],
                              [0.7, 13.2],
                              [1.2, 10.9],
                              [2.1, 8.7],
                              [3.4, 6.7],
                              [4.9, 4.9],
                              [6.7, 3.4],
                              [8.7, 2.1],
                              [10.9, 1.2],
                              [13.2, 0.7],
                              [15.6, 0.5],
                            ],
                          ],
                        },
                        symbol: {
                          type: "CIMPolygonSymbol",
                          symbolLayers: [
                            {
                              type: "CIMSolidFill",
                              enable: false,
                              color: [255, 255, 255, 255],
                            },
                          ],
                        },
                      },
                      {
                        type: "CIMMarkerGraphic",
                        geometry: {
                          rings: [
                            [
                              [0.5, 15.6],
                              [0.7, 13.2],
                              [1.2, 10.9],
                              [2.1, 8.7],
                              [3.4, 6.7],
                              [4.9, 4.9],
                              [6.7, 3.4],
                              [8.7, 2.1],
                              [10.9, 1.2],
                              [13.2, 0.7],
                              [15.6, 0.5],
                              [18, 0.7],
                              [20.3, 1.2],
                              [22.5, 2.1],
                              [24.5, 3.4],
                              [26.3, 4.9],
                              [27.8, 6.7],
                              [29.1, 8.7],
                              [30, 10.9],
                              [30.5, 13.2],
                              [30.7, 15.6],
                              [30.5, 18],
                              [30, 20.3],
                              [29.1, 22.5],
                              [27.8, 24.5],
                              [26.3, 26.3],
                              [24.5, 27.8],
                              [22.5, 29.1],
                              [20.3, 30],
                              [18, 30.5],
                              [15.6, 30.7],
                              [15.6, 30.7],
                              [13.2, 30.5],
                              [10.9, 30],
                              [8.7, 29.1],
                              [6.7, 27.8],
                              [4.9, 26.3],
                              [3.4, 24.5],
                              [2.1, 22.5],
                              [1.2, 20.3],
                              [0.7, 18],
                              [0.5, 15.6],
                            ],
                          ],
                        },
                        symbol: {
                          type: "CIMPolygonSymbol",
                          symbolLayers: [
                            {
                              type: "CIMSolidStroke",
                              enable: true,
                              capStyle: "Butt",
                              joinStyle: "Miter",
                              lineStyle3D: "Strip",
                              miterLimit: 10,
                              width: 1,
                              color: [255, 255, 255, 255],
                            },
                          ],
                        },
                      },
                      {
                        type: "CIMMarkerGraphic",
                        geometry: {
                          rings: [
                            [
                              [17.6, 5.6],
                              [17.6, 18.8],
                              [23.6, 12.5],
                              [23.6, 17.6],
                              [15.6, 25.6],
                              [7.6, 17.6],
                              [7.6, 12.5],
                              [13.6, 18.7],
                              [13.6, 5.6],
                              [17.6, 5.6],
                            ],
                          ],
                        },
                        symbol: {
                          type: "CIMPolygonSymbol",
                          symbolLayers: [
                            {
                              type: "CIMSolidFill",
                              enable: true,
                              colorLocked: true,
                              color: [255, 255, 255, 255],
                            },
                          ],
                        },
                      },
                    ],
                    scaleSymbolsProportionally: true,
                    respectFrame: true,
                    clippingPath: {
                      type: "CIMClippingPath",
                      clippingType: "Intersect",
                      path: {
                        rings: [
                          [
                            [0, 0],
                            [31.2, 0],
                            [31.2, 31.2],
                            [0, 31.2],
                            [0, 0],
                          ],
                        ],
                      },
                    },
                    rotation: 180,
                  },
                  {
                    type: "CIMVectorMarker",
                    enable: true,
                    anchorPointUnits: "Relative",
                    dominantSizeAxis3D: "Y",
                    size: 9.69,
                    billboardMode3D: "FaceNearPlane",
                    frame: {
                      xmin: 0,
                      ymin: 0,
                      xmax: 31.2,
                      ymax: 31.2,
                    },
                    markerGraphics: [
                      {
                        type: "CIMMarkerGraphic",
                        geometry: {
                          rings: [
                            [
                              [15.6, 0.5],
                              [17, 0.6],
                              [17.6, 0.6],
                              [18.3, 0.8],
                              [19.1, 0.9],
                              [19.8, 1.1],
                              [20.5, 1.3],
                              [21.2, 1.6],
                              [21.9, 1.9],
                              [22.5, 2.2],
                              [23.2, 2.6],
                              [23.9, 3],
                              [24.5, 3.4],
                              [25.1, 3.9],
                              [25.7, 4.4],
                              [26.3, 4.9],
                              [26.8, 5.5],
                              [27.3, 6.1],
                              [27.8, 6.7],
                              [28.2, 7.3],
                              [28.6, 8],
                              [29, 8.7],
                              [29.3, 9.3],
                              [29.6, 10],
                              [29.9, 10.7],
                              [30.1, 11.4],
                              [30.3, 12.1],
                              [30.4, 12.9],
                              [30.6, 13.6],
                              [30.6, 14.2],
                              [30.7, 14.9],
                              [30.7, 15.6],
                              [30.5, 18],
                              [30, 20.3],
                              [29.1, 22.5],
                              [27.8, 24.5],
                              [26.3, 26.3],
                              [24.5, 27.8],
                              [22.5, 29.1],
                              [20.3, 30],
                              [18, 30.5],
                              [15.6, 30.7],
                              [12.7, 30.4],
                              [10, 29.6],
                              [7.4, 28.3],
                              [5.2, 26.5],
                              [3.3, 24.3],
                              [1.8, 21.9],
                              [0.9, 19.1],
                              [0.5, 16.3],
                              [0.5, 15.6],
                              [0.7, 13.2],
                              [1.2, 10.9],
                              [2.1, 8.7],
                              [3.4, 6.7],
                              [4.9, 4.9],
                              [6.7, 3.4],
                              [8.7, 2.1],
                              [10.9, 1.2],
                              [13.2, 0.7],
                              [15.6, 0.5],
                            ],
                          ],
                        },
                        symbol: {
                          type: "CIMPolygonSymbol",
                          symbolLayers: [
                            {
                              type: "CIMSolidFill",
                              enable: true,
                              color: [153, 144, 92, 255],
                            },
                          ],
                        },
                      },
                      {
                        type: "CIMMarkerGraphic",
                        geometry: {
                          rings: [
                            [
                              [0.5, 15.6],
                              [0.7, 13.2],
                              [1.2, 10.9],
                              [2.1, 8.7],
                              [3.4, 6.7],
                              [4.9, 4.9],
                              [6.7, 3.4],
                              [8.7, 2.1],
                              [10.9, 1.2],
                              [13.2, 0.7],
                              [15.6, 0.5],
                              [18, 0.7],
                              [20.3, 1.2],
                              [22.5, 2.1],
                              [24.5, 3.4],
                              [26.3, 4.9],
                              [27.8, 6.7],
                              [29.1, 8.7],
                              [30, 10.9],
                              [30.5, 13.2],
                              [30.7, 15.6],
                              [30.5, 18],
                              [30, 20.3],
                              [29.1, 22.5],
                              [27.8, 24.5],
                              [26.3, 26.3],
                              [24.5, 27.8],
                              [22.5, 29.1],
                              [20.3, 30],
                              [18, 30.5],
                              [15.6, 30.7],
                              [15.6, 30.7],
                              [13.2, 30.5],
                              [10.9, 30],
                              [8.7, 29.1],
                              [6.7, 27.8],
                              [4.9, 26.3],
                              [3.4, 24.5],
                              [2.1, 22.5],
                              [1.2, 20.3],
                              [0.7, 18],
                              [0.5, 15.6],
                            ],
                          ],
                        },
                        symbol: {
                          type: "CIMPolygonSymbol",
                          symbolLayers: [
                            {
                              type: "CIMSolidStroke",
                              enable: false,
                              capStyle: "Butt",
                              joinStyle: "Miter",
                              lineStyle3D: "Strip",
                              miterLimit: 10,
                              width: 1,
                              color: [0, 0, 0, 255],
                            },
                          ],
                        },
                      },
                      {
                        type: "CIMMarkerGraphic",
                        geometry: {
                          rings: [
                            [
                              [17.6, 5.6],
                              [17.6, 18.8],
                              [23.6, 12.5],
                              [23.6, 17.6],
                              [15.6, 25.6],
                              [7.6, 17.6],
                              [7.6, 12.5],
                              [13.6, 18.7],
                              [13.6, 5.6],
                              [17.6, 5.6],
                            ],
                          ],
                        },
                        symbol: {
                          type: "CIMPolygonSymbol",
                          symbolLayers: [
                            {
                              type: "CIMSolidFill",
                              enable: false,
                              colorLocked: true,
                              color: [0, 0, 0, 255],
                            },
                          ],
                        },
                      },
                    ],
                    scaleSymbolsProportionally: true,
                    respectFrame: true,
                    clippingPath: {
                      type: "CIMClippingPath",
                      clippingType: "Intersect",
                      path: {
                        rings: [
                          [
                            [0, 0],
                            [31.2, 0],
                            [31.2, 31.2],
                            [0, 31.2],
                            [0, 0],
                          ],
                        ],
                      },
                    },
                    rotation: 180,
                  },
                ],
                haloSize: 1,
                scaleX: 1,
                angleAlignment: "Display",
                angle: 180,
              },
            },
            value: "d",
          },
          {
            label: "Unknown",
            symbol: {
              type: "CIMSymbolReference",
              symbol: {
                type: "CIMPointSymbol",
                symbolLayers: [
                  {
                    type: "CIMVectorMarker",
                    enable: true,
                    colorLocked: true,
                    anchorPointUnits: "Relative",
                    dominantSizeAxis3D: "Y",
                    size: 9.69,
                    billboardMode3D: "FaceNearPlane",
                    frame: {
                      xmin: 0,
                      ymin: 0,
                      xmax: 31.2,
                      ymax: 31.2,
                    },
                    markerGraphics: [
                      {
                        type: "CIMMarkerGraphic",
                        geometry: {
                          rings: [
                            [
                              [15.6, 0.5],
                              [17, 0.6],
                              [17.6, 0.6],
                              [18.3, 0.8],
                              [19.1, 0.9],
                              [19.8, 1.1],
                              [20.5, 1.3],
                              [21.2, 1.6],
                              [21.9, 1.9],
                              [22.5, 2.2],
                              [23.2, 2.6],
                              [23.9, 3],
                              [24.5, 3.4],
                              [25.1, 3.9],
                              [25.7, 4.4],
                              [26.3, 4.9],
                              [26.8, 5.5],
                              [27.3, 6.1],
                              [27.8, 6.7],
                              [28.2, 7.3],
                              [28.6, 8],
                              [29, 8.7],
                              [29.3, 9.3],
                              [29.6, 10],
                              [29.9, 10.7],
                              [30.1, 11.4],
                              [30.3, 12.1],
                              [30.4, 12.9],
                              [30.6, 13.6],
                              [30.6, 14.2],
                              [30.7, 14.9],
                              [30.7, 15.6],
                              [30.5, 18],
                              [30, 20.3],
                              [29.1, 22.5],
                              [27.8, 24.5],
                              [26.3, 26.3],
                              [24.5, 27.8],
                              [22.5, 29.1],
                              [20.3, 30],
                              [18, 30.5],
                              [15.6, 30.7],
                              [12.7, 30.4],
                              [10, 29.6],
                              [7.4, 28.3],
                              [5.2, 26.5],
                              [3.3, 24.3],
                              [1.8, 21.9],
                              [0.9, 19.1],
                              [0.5, 16.3],
                              [0.5, 15.6],
                              [0.7, 13.2],
                              [1.2, 10.9],
                              [2.1, 8.7],
                              [3.4, 6.7],
                              [4.9, 4.9],
                              [6.7, 3.4],
                              [8.7, 2.1],
                              [10.9, 1.2],
                              [13.2, 0.7],
                              [15.6, 0.5],
                            ],
                          ],
                        },
                        symbol: {
                          type: "CIMPolygonSymbol",
                          symbolLayers: [
                            {
                              type: "CIMSolidFill",
                              enable: false,
                              color: [255, 255, 255, 255],
                            },
                          ],
                        },
                      },
                      {
                        type: "CIMMarkerGraphic",
                        geometry: {
                          rings: [
                            [
                              [0.5, 15.6],
                              [0.7, 13.2],
                              [1.2, 10.9],
                              [2.1, 8.7],
                              [3.4, 6.7],
                              [4.9, 4.9],
                              [6.7, 3.4],
                              [8.7, 2.1],
                              [10.9, 1.2],
                              [13.2, 0.7],
                              [15.6, 0.5],
                              [18, 0.7],
                              [20.3, 1.2],
                              [22.5, 2.1],
                              [24.5, 3.4],
                              [26.3, 4.9],
                              [27.8, 6.7],
                              [29.1, 8.7],
                              [30, 10.9],
                              [30.5, 13.2],
                              [30.7, 15.6],
                              [30.5, 18],
                              [30, 20.3],
                              [29.1, 22.5],
                              [27.8, 24.5],
                              [26.3, 26.3],
                              [24.5, 27.8],
                              [22.5, 29.1],
                              [20.3, 30],
                              [18, 30.5],
                              [15.6, 30.7],
                              [15.6, 30.7],
                              [13.2, 30.5],
                              [10.9, 30],
                              [8.7, 29.1],
                              [6.7, 27.8],
                              [4.9, 26.3],
                              [3.4, 24.5],
                              [2.1, 22.5],
                              [1.2, 20.3],
                              [0.7, 18],
                              [0.5, 15.6],
                            ],
                          ],
                        },
                        symbol: {
                          type: "CIMPolygonSymbol",
                          symbolLayers: [
                            {
                              type: "CIMSolidStroke",
                              enable: true,
                              capStyle: "Butt",
                              joinStyle: "Miter",
                              lineStyle3D: "Strip",
                              miterLimit: 10,
                              width: 1,
                              color: [255, 255, 255, 255],
                            },
                          ],
                        },
                      },
                      {
                        type: "CIMMarkerGraphic",
                        geometry: {
                          rings: [
                            [
                              [17.6, 5.6],
                              [17.6, 18.8],
                              [23.6, 12.5],
                              [23.6, 17.6],
                              [15.6, 25.6],
                              [7.6, 17.6],
                              [7.6, 12.5],
                              [13.6, 18.7],
                              [13.6, 5.6],
                              [17.6, 5.6],
                            ],
                          ],
                        },
                        symbol: {
                          type: "CIMPolygonSymbol",
                          symbolLayers: [
                            {
                              type: "CIMSolidFill",
                              enable: true,
                              colorLocked: true,
                              color: [255, 255, 255, 255],
                            },
                          ],
                        },
                      },
                    ],
                    scaleSymbolsProportionally: true,
                    respectFrame: true,
                    clippingPath: {
                      type: "CIMClippingPath",
                      clippingType: "Intersect",
                      path: {
                        rings: [
                          [
                            [0, 0],
                            [31.2, 0],
                            [31.2, 31.2],
                            [0, 31.2],
                            [0, 0],
                          ],
                        ],
                      },
                    },
                    rotation: 0,
                  },
                  {
                    type: "CIMVectorMarker",
                    enable: true,
                    anchorPointUnits: "Relative",
                    dominantSizeAxis3D: "Y",
                    size: 9.69,
                    billboardMode3D: "FaceNearPlane",
                    frame: {
                      xmin: 0,
                      ymin: 0,
                      xmax: 31.2,
                      ymax: 31.2,
                    },
                    markerGraphics: [
                      {
                        type: "CIMMarkerGraphic",
                        geometry: {
                          rings: [
                            [
                              [15.6, 0.5],
                              [17, 0.6],
                              [17.6, 0.6],
                              [18.3, 0.8],
                              [19.1, 0.9],
                              [19.8, 1.1],
                              [20.5, 1.3],
                              [21.2, 1.6],
                              [21.9, 1.9],
                              [22.5, 2.2],
                              [23.2, 2.6],
                              [23.9, 3],
                              [24.5, 3.4],
                              [25.1, 3.9],
                              [25.7, 4.4],
                              [26.3, 4.9],
                              [26.8, 5.5],
                              [27.3, 6.1],
                              [27.8, 6.7],
                              [28.2, 7.3],
                              [28.6, 8],
                              [29, 8.7],
                              [29.3, 9.3],
                              [29.6, 10],
                              [29.9, 10.7],
                              [30.1, 11.4],
                              [30.3, 12.1],
                              [30.4, 12.9],
                              [30.6, 13.6],
                              [30.6, 14.2],
                              [30.7, 14.9],
                              [30.7, 15.6],
                              [30.5, 18],
                              [30, 20.3],
                              [29.1, 22.5],
                              [27.8, 24.5],
                              [26.3, 26.3],
                              [24.5, 27.8],
                              [22.5, 29.1],
                              [20.3, 30],
                              [18, 30.5],
                              [15.6, 30.7],
                              [12.7, 30.4],
                              [10, 29.6],
                              [7.4, 28.3],
                              [5.2, 26.5],
                              [3.3, 24.3],
                              [1.8, 21.9],
                              [0.9, 19.1],
                              [0.5, 16.3],
                              [0.5, 15.6],
                              [0.7, 13.2],
                              [1.2, 10.9],
                              [2.1, 8.7],
                              [3.4, 6.7],
                              [4.9, 4.9],
                              [6.7, 3.4],
                              [8.7, 2.1],
                              [10.9, 1.2],
                              [13.2, 0.7],
                              [15.6, 0.5],
                            ],
                          ],
                        },
                        symbol: {
                          type: "CIMPolygonSymbol",
                          symbolLayers: [
                            {
                              type: "CIMSolidFill",
                              enable: true,
                              color: [137, 166, 166, 255],
                            },
                          ],
                        },
                      },
                      {
                        type: "CIMMarkerGraphic",
                        geometry: {
                          rings: [
                            [
                              [0.5, 15.6],
                              [0.7, 13.2],
                              [1.2, 10.9],
                              [2.1, 8.7],
                              [3.4, 6.7],
                              [4.9, 4.9],
                              [6.7, 3.4],
                              [8.7, 2.1],
                              [10.9, 1.2],
                              [13.2, 0.7],
                              [15.6, 0.5],
                              [18, 0.7],
                              [20.3, 1.2],
                              [22.5, 2.1],
                              [24.5, 3.4],
                              [26.3, 4.9],
                              [27.8, 6.7],
                              [29.1, 8.7],
                              [30, 10.9],
                              [30.5, 13.2],
                              [30.7, 15.6],
                              [30.5, 18],
                              [30, 20.3],
                              [29.1, 22.5],
                              [27.8, 24.5],
                              [26.3, 26.3],
                              [24.5, 27.8],
                              [22.5, 29.1],
                              [20.3, 30],
                              [18, 30.5],
                              [15.6, 30.7],
                              [15.6, 30.7],
                              [13.2, 30.5],
                              [10.9, 30],
                              [8.7, 29.1],
                              [6.7, 27.8],
                              [4.9, 26.3],
                              [3.4, 24.5],
                              [2.1, 22.5],
                              [1.2, 20.3],
                              [0.7, 18],
                              [0.5, 15.6],
                            ],
                          ],
                        },
                        symbol: {
                          type: "CIMPolygonSymbol",
                          symbolLayers: [
                            {
                              type: "CIMSolidStroke",
                              enable: false,
                              capStyle: "Butt",
                              joinStyle: "Miter",
                              lineStyle3D: "Strip",
                              miterLimit: 10,
                              width: 1,
                              color: [0, 0, 0, 255],
                            },
                          ],
                        },
                      },
                      {
                        type: "CIMMarkerGraphic",
                        geometry: {
                          rings: [
                            [
                              [17.6, 5.6],
                              [17.6, 18.8],
                              [23.6, 12.5],
                              [23.6, 17.6],
                              [15.6, 25.6],
                              [7.6, 17.6],
                              [7.6, 12.5],
                              [13.6, 18.7],
                              [13.6, 5.6],
                              [17.6, 5.6],
                            ],
                          ],
                        },
                        symbol: {
                          type: "CIMPolygonSymbol",
                          symbolLayers: [
                            {
                              type: "CIMSolidFill",
                              enable: false,
                              colorLocked: true,
                              color: [0, 0, 0, 255],
                            },
                          ],
                        },
                      },
                    ],
                    scaleSymbolsProportionally: true,
                    respectFrame: true,
                    clippingPath: {
                      type: "CIMClippingPath",
                      clippingType: "Intersect",
                      path: {
                        rings: [
                          [
                            [0, 0],
                            [31.2, 0],
                            [31.2, 31.2],
                            [0, 31.2],
                            [0, 0],
                          ],
                        ],
                      },
                    },
                    rotation: 0,
                  },
                ],
                haloSize: 1,
                scaleX: 1,
                angleAlignment: "Display",
                angle: 0,
              },
            },
            value: "u",
          },
        ],
      },
    },
  },
  customParameters: {
    returnM: "true",
  },
  effect: null,
  featureEffect: null,
  id: 0,
  showLabels: true,
  popupInfo: {
    popupElements: [
      {
        type: "fields",
        description: "",
        fieldInfos: [
          {
            fieldName: "ARM",
            format: {
              digitSeparator: false,
              places: 3,
            },
            isEditable: true,
            label: "ARM",
            visible: true,
          },
          {
            fieldName: "expression/expr2",
            isEditable: true,
            visible: true,
          },
          {
            fieldName: "expression/expr9",
            isEditable: true,
            visible: true,
          },
          {
            fieldName: "expression/expr10",
            isEditable: true,
            visible: true,
          },
          {
            fieldName: "expression/expr11",
            isEditable: true,
            visible: true,
          },
          {
            fieldName: "expression/expr13",
            isEditable: true,
            visible: true,
          },
        ],
        title: "",
      },
      {
        type: "expression",
        expressionInfo: {
          title: "App Links",
          expression:
            'function getSearchDict() {\n  var output = {\n    SR: $feature.StateRouteNumber,\n    SRMP: Round($feature.SRMP, 2),\n    DIR: Upper($feature.Direction)\n  };\n\n  if (Upper($feature.AheadBackInd) == "B") {\n    output.SRMP = Text(output.SRMP) + "B";\n  }\n  if (!IsEmpty($feature.RelRouteType)) {\n    output.RRT = $feature.RelRouteType;\n    if (!IsEmpty($feature.RelRouteQual)) {\n      output.RRQ = $feature.RelRouteQual;\n    }\n  }\n\n  return output;\n}\n\nfunction getUrlSearch() {\n  var searchParams = getSearchDict();\n  var parts = [];\n  for (var key in searchParams) {\n    var kvp = Map([key, searchParams[key]], UrlEncode);\n    Push(parts, Concatenate(kvp, "="));\n  }\n  return Concatenate(parts, "&");\n}\n\nvar urlSearch = getUrlSearch();\n\nvar urls = {\n  "Internal QA": "https://webappsqa.wsdot.loc/data/tools/Locatemp/" + `?${urlSearch}`,\n  "Public QA":\n    "https://remoteappsqa.wsdot.wa.gov/data/tools/Locatemp/" + `?${urlSearch}`,\n  "Internal Production": "https://webapps.wsdot.loc/data/tools/Locatemp/" + `?${urlSearch}`,\n  "External Production": "https://remoteapps.wsdot.wa.gov/data/tools/Locatemp/" + `?${urlSearch}`,\n\n};\n\n// Create the field infos.\nvar fieldInfos = []\nfor (var key in urls) {\n  Push(fieldInfos, { fieldName: key })\n}\n\nreturn {\n    type: \'fields\',\n    title: \'Application Links\',\n    description : \'Links to external applications\',\n    fieldInfos,\n    attributes: urls\n  }',
          returnType: "dictionary",
        },
      },
      {
        type: "fields",
        description: "",
        fieldInfos: [
          {
            fieldName: "expression/expr5",
            isEditable: true,
            visible: true,
          },
          {
            fieldName: "expression/expr1",
            isEditable: true,
            visible: true,
          },
          {
            fieldName: "expression/expr4",
            isEditable: true,
            visible: true,
          },
          {
            fieldName: "expression/expr7",
            isEditable: true,
            visible: true,
          },
          {
            fieldName: "expression/expr3",
            isEditable: true,
            visible: true,
          },
        ],
        title: "External links",
      },
      {
        type: "expression",
        expressionInfo: {
          title: "Speed Limits",
          expression:
            'function getSpeedLimit() {\n  var agolId = "7bd8c700fa8b47d08a57c49b77b34ad5";\n  var rid = `${$feature.RouteID}${$feature.Direction}`;\n  var arm = $feature.ARM;\n\n  var fields = ["SpeedLimit", "SnapshotDate"];\n\n  var closest = Top(\n    Filter(\n      FeatureSetByPortalItem(\n        Portal("https://www.arcgis.com"),\n        agolId,\n        4,\n        fields\n      ),\n      "RouteIdentifier = @rid AND BeginAccumulatedRouteMile <= @arm AND EndAccumulatedRouteMile >= @arm"\n    ),\n    1\n  );\n\n  for (var item in closest) {\n    var speedLimit = item.SpeedLimit;\n    var snapshotDate = item.SnapshotDate;\n    return {\n      "Speed Limit": `${speedLimit} mph`,\n      "Snapshot Date": Text(snapshotDate, "Y-MM-DD")\n    };\n  }\n}\n\nvar fieldInfos = [];\n\nvar attributes = getSpeedLimit();\n\nvar fieldNames = ["Speed Limit", "Snapshot Date"];\n\nfor (var key in fieldNames) {\n\tPush(fieldInfos, { fieldName: fieldNames[key] })\n}\n\nreturn {\n    type: \'fields\',\n    title: \'Speed Limits\',\n    //description : \'\',\n    fieldInfos,\n    attributes\n  }',
          returnType: "dictionary",
        },
      },
      {
        type: "text",
      },
    ],
    expressionInfos: [
      {
        name: "expr1",
        title: "Google Street View",
        expression:
          '// Create links to Google Street View Images. \n\nvar lat = $feature.Latitude;\nvar lon = $feature.Longitude;\nvar az = $feature.Azimuth;\nvar dir = $feature.Direction;\nvar url_start = "https://www.google.com/maps/@?api=1&map_action=pano&viewpoint=";\nvar url_mid = "%2c";\nvar url_end = "&heading=";\n\nif (dir == \'d\') {\n  if (az >= 180) {\n    az -= 180;\n  } else {\n    az += 180;\n  }\n} else {\n  az == az;\n}\nreturn url_start + lat + url_mid + lon + url_end + az\n',
        returnType: "string",
      },
      {
        name: "expr2",
        title: "LRS Date",
        expression:
          '// Write a script to return a value to show in the pop-up. \n// For example, get the average of 4 fields:\n// Average($feature.SalesQ1, $feature.SalesQ2, $feature.SalesQ3, $feature.SalesQ4)\nDateOnly($feature.LRS_Date, "YMMDD")',
        returnType: "string",
      },
      {
        name: "expr3",
        title: "GeoURI",
        expression: '"geo:" + $feature.Latitude + "," + $feature.Longitude',
        returnType: "string",
      },
      {
        name: "expr4",
        title: "Bing Maps",
        expression:
          '"https://bing.com/maps/default.aspx?where=" + $feature.Latitude + "," + $feature.Longitude',
        returnType: "string",
      },
      {
        name: "expr5",
        title: "GeoHack",
        expression:
          '"https://geohack.toolforge.org/geohack.php?params=" +\n  $feature.Latitude +\n  ";" +\n  $feature.Longitude + "&pagename=" + $feature.RouteID + "+@+" + Round($feature.SRMP, 3) + IIf($feature.AheadBackInd == \'B\', "B", "")',
        returnType: "string",
      },
      {
        name: "expr7",
        title: "SRView URL",
        expression:
          '/*\nSample location w/ back mileage:\n002i @ 60.50B\n\nhttps://srview3i.wsdot.loc/stateroute/picturelog/v3/client/SRview.Windows.Viewer.application?srnum=002&srmp=60.50B&year=2022&dir=I\nhttps://srview3i.wsdot.loc/stateroute/picturelog/v3/client/SRview.Windows.Viewer.application?srnum=002&srmp=60.50B&dir=I\n\nExample with RRT & RRQ:\nSRview 3 at 007 LX 05778 Inc 2022: SRMP 0.00 / ARM 0.00\nhttps://srview3i.wsdot.loc/stateroute/picturelog/v3/client/SRview.Windows.Viewer.application?srnum=007&rrt=LX&rrq=05778&srmp=0.000&year=2022&dir=I\n*/\n\nvar srnum = $feature.StateRouteNumber;\nvar srmp = Text(Round($feature.SRMP, 3));\nif ($feature.AheadBackInd == "B") {\n  srmp += "B";\n}\nvar dir = Upper($feature.Direction);\n\nvar rrt = $feature.RelRouteType;\nvar rrq = $feature.RelRouteQual;\n\nvar d = { srnum, srmp, rrt, rrq, dir };\n\nvar parts = [];\n\nfor (var i in d) {\n  var value = d[i];\n\n  if (value == null || value == "") {\n    continue;\n  }\n\n  var output = Concatenate([i, value], "=");\n  Push(parts, output);\n}\nvar search = Concatenate(parts, "&");\n\nreturn (\n  "https://srview3i.wsdot.loc/stateroute/picturelog/v3/client/SRview.Windows.Viewer.application?" +\n  search\n);',
        returnType: "string",
      },
      {
        name: "expr8",
        title: "Milepost Label",
        expression:
          '/**\n * Creates the label text for an SRMP point.\n * - Rounds SRMP to three digits.\n * - Appends an "S" for spurs (RRT = "SP")\n * - Appends a "B" for back mileage\n */\nfunction createSrmpLabel() {\n  var label = Text($feature.SRMP, "####.###");\n\n  if ($feature.RelRouteType == "SP") {\n    label += Left($feature.RelRouteType, 1)\n  }\n\n  if (Upper($feature.AheadBackInd) == "B") {\n    label += "B";\n  }\n\n  return label;\n}\n\ncreateSrmpLabel();',
        returnType: "string",
      },
      {
        name: "expr9",
        title: "County",
        expression:
          'var agolId = "47143194b52f4eea8e8235a7d7bb703c";\nvar p = Portal("https://wsdot.maps.arcgis.com");\n\nvar match = Top(\n  Within(\n    $userInput,\n    FeatureSetByPortalItem(\n      p,\n      agolId,\n      11,\n      ["JURISDICT_LABEL_NM"],\n      false\n    )\n  ),\n  1\n)\n\nfor (var f in match) {\n  return f.JURISDICT_LABEL_NM\n}',
        returnType: "string",
      },
      {
        name: "expr10",
        title: "City",
        expression:
          'var agolId = "0d96ba84b802425aa1d2b9a99e422c5d";\nvar p = Portal("https://wsdot.maps.arcgis.com");\n\nvar fields = ["NAME", "LastUpdate"];\n\nvar features = FeatureSetByPortalItem(p, agolId, null, fields, true);\n\nvar output = [];\n\nfor (var f in features) {\n  var d = Distance(f, $userInput, "miles");\n  Push(output, [d, f]);\n}\n\nfunction orderByDistance(a, b) {\n  return a[0] - b[0];\n}\n\nfunction getFeature(f) {\n  return { name: f[1].Name, lastUpdate: f[1].LastUpdate, distance: f[0] };\n}\n\noutput = Top(Map(Sort(output, orderByDistance), getFeature), 1)[0];\n\nvar outText = output.name;\n\nif (output.distance > 0) {\n  outText += ` is about ${Round(output.distance)} mi away`;\n}\nif (output.lastUpdate != null) {\n  outText += ` (as of ${Text(output.lastUpdate, "Y-MM-DD")})`;\n}\n\noutText += "."\n\nreturn outText',
        returnType: "string",
      },
      {
        name: "expr11",
        title: "Access Control",
        expression:
          'var agolId = "35cd6f1a5e2f4553a8e83c9ff6edbfa7";\nvar routeIdentifier = `${$feature.RouteID}${Lower($feature.Direction)}`;\nvar codeField = "AccessControlTypeDescription"\nvar snapshotDateField = "SnapshotDate"\n\nvar fs = Filter(\n  FeatureSetByPortalItem(Portal("https://arcgis.com"), agolId, 1, [codeField, snapshotDateField]),\n  "RouteIdentifier = @routeIdentifier"\n);\n\nvar shortestDistanceCode = null;\nvar shortestDistance = null;\nvar shortestDistanceSnapshotDate = null;\n\nvar i = 0\nfor (var f in fs) {\n  Console(f)\n  var d = Distance(f, $feature)\n  var c = f[codeField]\n  var sd = f[snapshotDateField];\n  if (i == 0 || d < shortestDistance) {\n    shortestDistance = d;\n    shortestDistanceCode = c;\n    shortestDistanceSnapshotDate = sd;\n  }\n  i++;\n}\n\nif (shortestDistanceCode == null) {\n  return null;\n}\n\nvar output = `${shortestDistanceCode} (as of ${Text(shortestDistanceSnapshotDate, "Y-MM-DD")})`;\n\nreturn output',
        returnType: "string",
      },
      {
        name: "expr12",
        title: "Region",
        expression:
          'var regionId = "1c0845bd12944017b3bb3631bda09e0e"\nvar p = Portal("https://wsdot.maps.arcgis.com")\n\nvar nameField = "RegionName"\n\nfor (var current in Top(Within($feature, FeatureSetByPortalItem(p, regionId, null, [nameField])), 1)) {\n  return current[nameField];\n}',
        returnType: "string",
      },
      {
        name: "expr13",
        title: "Speed Limit",
        expression:
          'var agolId = "7bd8c700fa8b47d08a57c49b77b34ad5";\nvar rid = `${$feature.RouteID}${$feature.Direction}`;\nvar arm = $feature.ARM;\n\nvar fields = ["SpeedLimit", "SnapshotDate"];\n\nvar closest = Top(\n  Filter(\n    FeatureSetByPortalItem(Portal("https://www.arcgis.com"), agolId, 4, fields),\n    "RouteIdentifier = @rid AND BeginAccumulatedRouteMile <= @arm AND EndAccumulatedRouteMile >= @arm"\n  ),\n  1\n);\n\nfor (var item in closest) {\n  var speedLimit = item.SpeedLimit;\n  var snapshotDate = item.SnapshotDate\n  return {\n    "Speed Limit": `${speedLimit} mph`,\n    "Snapshot Date": Text(snapshotDate, "Y-MM-DD"),\n  }\n}',
        returnType: "string",
      },
    ],
    fieldInfos: [
      {
        fieldName: "OBJECTID",
        format: {
          digitSeparator: false,
          places: 0,
        },
        isEditable: false,
        label: "OBJECTID",
        visible: false,
      },
      {
        fieldName: "RouteID",
        isEditable: true,
        label: "Route ID",
        visible: false,
      },
      {
        fieldName: "ARM",
        format: {
          digitSeparator: false,
          places: 3,
        },
        isEditable: true,
        label: "ARM",
        visible: true,
      },
      {
        fieldName: "SRMP",
        format: {
          digitSeparator: false,
          places: 3,
        },
        isEditable: true,
        label: "SRMP",
        visible: false,
      },
      {
        fieldName: "Direction",
        isEditable: true,
        label: "Direction",
        visible: false,
      },
      {
        fieldName: "Easting",
        format: {
          digitSeparator: true,
          places: 6,
        },
        isEditable: true,
        label: "Easting",
        visible: false,
      },
      {
        fieldName: "Northing",
        format: {
          digitSeparator: true,
          places: 6,
        },
        isEditable: true,
        label: "Northing",
        visible: false,
      },
      {
        fieldName: "Longitude",
        format: {
          digitSeparator: false,
          places: 6,
        },
        isEditable: true,
        label: "Longitude",
        visible: false,
      },
      {
        fieldName: "Latitude",
        format: {
          digitSeparator: false,
          places: 6,
        },
        isEditable: true,
        label: "Latitude",
        visible: false,
      },
      {
        fieldName: "Azimuth",
        format: {
          digitSeparator: false,
          places: 0,
        },
        isEditable: true,
        label: "Azimuth",
        visible: false,
      },
      {
        fieldName: "StateRouteNumber",
        isEditable: true,
        label: "State Route Number",
        visible: false,
      },
      {
        fieldName: "RelRouteType",
        isEditable: true,
        label: "Rel Route Type",
        visible: false,
      },
      {
        fieldName: "RelRouteQual",
        isEditable: true,
        label: "Rel Route Qual",
        visible: false,
      },
      {
        fieldName: "AheadBackInd",
        isEditable: true,
        label: "Ahead Back Ind",
        visible: false,
      },
      {
        fieldName: "LRS_Date",
        isEditable: true,
        label: "LRS Date",
        visible: false,
      },
      {
        fieldName: "expression/expr1",
        isEditable: true,
        visible: true,
      },
      {
        fieldName: "expression/expr2",
        isEditable: true,
        visible: true,
      },
      {
        fieldName: "expression/expr3",
        isEditable: true,
        visible: true,
      },
      {
        fieldName: "expression/expr4",
        isEditable: true,
        visible: true,
      },
      {
        fieldName: "expression/expr5",
        isEditable: true,
        visible: true,
      },
      {
        fieldName: "expression/expr7",
        isEditable: true,
        visible: true,
      },
      {
        fieldName: "expression/expr8",
        isEditable: true,
        visible: false,
      },
      {
        fieldName: "expression/expr9",
        isEditable: true,
        visible: true,
      },
      {
        fieldName: "expression/expr10",
        isEditable: true,
        visible: true,
      },
      {
        fieldName: "expression/expr11",
        isEditable: true,
        visible: true,
      },
      {
        fieldName: "expression/expr12",
        isEditable: true,
        visible: false,
      },
      {
        fieldName: "expression/expr13",
        isEditable: true,
        visible: true,
      },
    ],
    title: "{RouteID} ({Direction}) @ {expression/expr8}",
  },
} as const;

export default data;
