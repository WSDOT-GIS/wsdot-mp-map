...
import type { Components } from "@esri/calcite-components";
...
const interceptLogs: __esri.LogInterceptor = function (
  level,
  module,
  ...args: unknown[]
): false {
  /* __PURE__ */ console.group(interceptLogs.name, { level, module, args });
  /* __PURE__ */ console.debug("log interceptor", {
    level,
    module,
    args,
  });
  let alertKind: Components.CalciteAlert["kind"] | null;
  switch (level) {
    case "error":
      alertKind = "danger";
      break;
  ...
  alertTitleDiv.slot = "title";
  alertTitleDiv.append(module);
  alert.append(alertTitleDiv);

  const alertMessageDiv = document.createElement("div");
  alertMessageDiv.slot = "message";

  function objectToDL(
    obj: object,
    objectToNode: (obj: unknown) => string | Element | null,
  ) {
  ...
  function objectToNode(obj: unknown): string | Element | null {
    ...
    if (typeof obj === "string") {
      ...
      return objectToDL(obj, objectToNode);
    }
  ...
  for (const arg of args) {
    const message = objectToNode(arg);

    if (message) {
      const p = document.createElement("p");
      p.append(message);
      alertMessageDiv.append(p);
    }
  }

  alert.append(alertMessageDiv);
...
async function setupMap() {
  ...
  config.log.interceptors.push(interceptLogs);
    ...
    const callElc = async () => {
      ...
      if (elcGraphics) {
        ...
        view
          ...
          .catch((reason: unknown) => {
            console.error("Failed to go to a viewpoint", reason);
          });
        view
          .openPopup({
