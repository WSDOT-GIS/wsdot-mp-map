...
return transformWorldFromViewTL + vPositionWorldCameraRelative;
}`)}let Ps=class extends Ct{constructor(){super(...arguments),this.transformWorldFromViewTH=P(),this.transformWorldFromViewTL=P(),this.transformViewFromCameraRelativeRS=Yt(),this.transformProjFromView=Xt()}};function li(t,e){switch(e.normalType){case H.Attribute:case H.Compressed:t.include(er,e),t.varyings.add("vNormalWorld","vec3"),t.varyings.add("vNormalView","vec3"),t.vertex.uniforms.add(new ni("transformNormalGlobalFromModel",r=>r.transformNormalGlobalFromModel),new Me("transformNormalViewFromGlobal",r=>r.transformNormalViewFromGlobal)),t.vertex.code.add(s`void forwardNormal() {
...
return proj * (view * vec4(pos, 1.0));
    ...
    `),e.output===x.Normal&&(Ys(a),a.code.add(s`
...
return max(c.x, max(c.y, c.z));
  ...
  `)}function Yl(t){const e=new or,{vertex:r,fragment:o,varyings:a}=e;if(Tt(r,t),e.include(Gr),a.add("vpos","vec3"),e.include(_t,t),e.include(xi,t),e.include(Ei,t),t.hasColorTextureTransform&&e.include(Ul),t.output===x.Color||t.output===x.Alpha){t.hasNormalTextureTransform&&e.include(Hl),t.hasEmissionTextureTransform&&e.include(jl),t.hasOcclusionTextureTransform&&e.include(Wl),t.hasMetallicRoughnessTextureTransform&&e.include(kl),bt(r,t),e.include(er,t),e.include(xt,t);const i=t.normalType===H.Attribute||t.normalType===H.Compressed;i&&t.offsetBackfaces&&e.include(mi),e.include(hl,t),e.include(li,t),t.instancedColor&&e.attributes.add(p.INSTANCECOLOR,"vec4"),a.add("vPositionLocal","vec3"),e.include(Ze,t),e.include(hi,t),e.include(_i,t),e.include(bi,t),r.uniforms.add(new re("externalColor",n=>n.externalColor)),a.add("vcolorExt","vec4"),t.multipassEnabled&&a.add("depth","float"),r.code.add(s`
    ...
    `)),t.output===x.Color&&(e.include(Ue,t),e.include(Di,t),e.include(Wr,t),e.include(He,t),e.include(t.instancedDoublePrecision?Fi:Bi,t),e.include(Wt,t),bt(e.fragment,t),kr(o),Yr(o),Xr(o),o.uniforms.add(r.uniforms.get("localOrigin"),r.uniforms.get("view"),new Z("ambient",i=>i.ambient),new Z("diffuse",i=>i.diffuse),new te("opacity",i=>i.opacity),new te("layerOpacity",i=>i.layerOpacity)),t.hasColorTexture&&o.uniforms.add(new K("tex",i=>i.texture)),e.include(Zo,t),e.include(qr,t),o.include(kt),ir(o),o.code.add(s`
      void main() {
        discardBySlice(vpos);
        ${t.multipassEnabled?s`terrainDepthTest(depth);`:""}
        ${t.hasColorTexture?s`
